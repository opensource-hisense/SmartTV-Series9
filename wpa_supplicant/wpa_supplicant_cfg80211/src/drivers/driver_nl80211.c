...
extern int wpa_driver_nl80211_get_acs_rank_list(void *priv, u8 *buf, int buf_len);
...
#ifndef CONFIG_SHARED_P2P_MAC_ADDR
		if (nl80211_addr_in_use(drv->global, new_addr)) {
...
#endif /* CONFIG_SHARED_P2P_MAC_ADDR */
...
#ifdef ANDROID
#ifdef CONFIG_SHARED_P2P_MAC_ADDR 
	if (os_strstr(param, "use_shared_p2p_mac=1")) {
		wpa_driver_nl80211_driver_cmd(priv, "SET_FEATURE_ENABLE use_shared_p2p_mac", "", 0);
		wpa_printf(MSG_ERROR, "nl80211: Feature use_shared_p2p_mac is enabled");
	}	
#endif /* CONFIG_SHARED_P2P_MAC_ADDR  */
#endif /* ANDROID */
#endif /* CONFIG_P2P */
...
#ifdef ANDROID
	if (os_strstr(param, "use_customized_bw_setting=1")) {
		wpa_driver_nl80211_driver_cmd(priv, "SET_FEATURE_ENABLE use_customized_bw_setting", "", 0);
		wpa_printf(MSG_ERROR, "nl80211: Feature use_customized_bw_setting is enabled");
	}	
#endif /* ANDROID */
...
int linux_driver_mtk_set_oid(int ioctl_sock, const char *ifname, int oid, u8 *data, int len)
{
	...
	if ( (err = ioctl(ioctl_sock, RT_PRIV_IOCTL, &iwr)) < 0)
	{
		wpa_printf(MSG_ERROR, "ERR(%d)::  %s in 0x%04x [%s]", err, __FUNCTION__, oid, strerror(errno));
		return -1;
	}
...
int linux_set_ptk(int sock, const char *ifname, u8 *data, int len)
{
	...
	linux_driver_mtk_set_oid(sock, "wlan0", OID_HE_SET_PTK, (u8*)data, len);
	close(sock);
	return 0;
}
...
{
	sock = socket(AF_INET, SOCK_DGRAM, 0);
	linux_driver_mtk_set_oid(sock, "wlan0", OID_HE_SET_REPLAY_COUNTER, (u8*)data, len);
	close(sock);
	return 0;	
}

int wpa_driver_nl80211_set_ptk(void *priv, u8 *ptk, int ptk_len)
{
    struct i802_bss *bss = priv;
    struct wpa_driver_nl80211_data *drv = bss->drv;
    return linux_set_ptk(drv->global->ioctl_sock, bss->ifname, ptk, ptk_len);
}
int wpa_driver_nl80211_set_replay_counter(void *priv, u8 *replay_counter, int replay_counter_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	return linux_set_replay_counter(drv->global->ioctl_sock, bss->ifname, replay_counter, replay_counter_len);
}
#endif /* CONFIG_WOW_SUPPORT */
					 ...
					 size_t buf_len)
{
	...
	if (os_strcasecmp(cmd, "STOP") == 0) {
		 if (linux_set_iface_flags(drv->global->ioctl_sock, bss->ifname, 0)) {
			wpa_msg(drv->ctx, MSG_ERROR, "Failed to set interface flags (ifname %s)", bss->ifname);
			return -1;
		 }
...
#endif
		/* Currently this command only requires events from the 
		 * kernel. Put in a more generic way later
		 */
#ifdef BCM_GENL
		if((ret = wpa_driver_register_genl(priv)) < 0)
			return ret;
#endif /* BCM_GENL */
#ifdef BCM_GENL_UNICAST
		if((ret = wpa_driver_set_supplicant_pid(priv)) < 0)
			return ret;
#endif /* BCM_GENL_UNICAST */
	} else if (os_strncasecmp(cmd, "P2P_SD_CANCEL_REQ", 18) == 0) {
#ifdef BCM_GENL
		wpa_driver_unregister_genl(priv);
#endif /* BCM_GENL */
	...
	ifr.ifr_data = &priv_cmd;
	wpa_printf(MSG_ERROR, "[%s]: Send to Driver Cmd: %s\n", __func__, cmd);
	if ((ret = ioctl(drv->global->ioctl_sock, SIOCDEVPRIVATE + 1, &ifr)) < 0) {
		...
		if ((os_strcasecmp(cmd, "LINKSPEED") == 0) ||
			...
			wpa_supplicant_event(drv->ctx, EVENT_CHANNEL_LIST_CHANGED,
			     NULL);
		} else if (os_strncasecmp(cmd, "GET_CS_INFO", 11) == 0) {
			os_memcpy(buf, (char *)priv_cmd.buf, priv_cmd.used_len);
		}
		wpa_printf(MSG_DEBUG, "[%s] buf=%s cmd=%s len = %d, %d", __func__, buf, cmd, ret, strlen(buf));
	}
	return ret;
}
#endif
...
#ifdef CONFIG_ACS_SUPPORT
    .get_acs_rank_list = wpa_driver_nl80211_get_acs_rank_list,
#endif /* CONFIG_ACS_SUPPORT */
};
