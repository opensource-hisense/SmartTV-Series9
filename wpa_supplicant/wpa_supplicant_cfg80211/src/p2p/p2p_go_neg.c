...
#if defined (CONFIG_SET_5G_HIGHER_PRIORITY)
static int p2p_check_channel_flag_active(struct wpa_supplicant *wpa_s,
					      int op_channel,
					      int op_class)
{
	...
	int i, j;
    int freq;
    freq = p2p_channel_to_freq(op_class, op_channel);
        
    wpa_printf(MSG_DEBUG,"op_class:%d,op_channel:%d\n", op_class, op_channel);

	for (j = 0; j < wpa_s->global->p2p_init_wpa_s->hw.num_modes; j++)
    {
		chnl = wpa_s->global->p2p_init_wpa_s->hw.modes[j].channels;
		for (i = 0; i < wpa_s->global->p2p_init_wpa_s->hw.modes[j].num_channels; i++)
        {
            if(freq == chnl[i].freq)
            {
                if ((chnl[i].flag & HOSTAPD_CHAN_DISABLED) ||
                    (chnl[i].flag & HOSTAPD_CHAN_PASSIVE_SCAN) ||
                    (chnl[i].flag & HOSTAPD_CHAN_NO_IBSS) ||
                    (chnl[i].flag & HOSTAPD_CHAN_RADAR))
                {
    				wpa_printf(MSG_DEBUG,"\t%d %x(Inactive)", chnl[i].chan, chnl[i].flag);
                    return 0;
                }
    			else
                {
    				wpa_printf(MSG_DEBUG,"\t%d %x(Active)", chnl[i].chan, chnl[i].flag);
                    return 1;
                }
            }
    ...
    wpa_printf(MSG_DEBUG,"cannot find the channel:%d\n", op_channel);

	return 1;
}

/*check channel flag(Active,Passive,Disabled,DFS) at this function*/
static int p2p_intersection_5g_num(struct p2p_data *p2p,
                            struct p2p_channels *intersection,
                            int p2p_5g_channel[2][160],
                            int p2p_24g_channel[2][14])
{
    wpa_printf(MSG_DEBUG,"+++into %s+++",__FUNCTION__);
    //return 0;
    size_t i;
    size_t j;
    int index_5g = 0;
    int index_24g = 0;
    int count = 0; //count 5g number
    if(NULL == intersection || NULL == p2p_5g_channel || NULL == p2p_24g_channel)
    {
        wpa_printf(MSG_DEBUG,"invalid pointer @ %s",__FUNCTION__);
        return -1;
    }
    for(i=0; i<intersection->reg_classes; i++)
    {
        const struct p2p_reg_class *reg = &intersection->reg_class[i];
        for(j=0; j<reg->channels; j++)
        {
            if(!p2p_check_channel_flag_active(p2p->cfg->cb_ctx, reg->channel[j], reg->reg_class))
                continue;
            if(reg->channel[j] > 14)
            {
                //wpa_printf(MSG_DEBUG, "support 5g\n");
                p2p_5g_channel[0][index_5g] = reg->reg_class;
                p2p_5g_channel[1][index_5g++] = reg->channel[j];
                count++;
            }
            else
            {
                p2p_24g_channel[0][index_24g] = reg->reg_class;
                p2p_24g_channel[1][index_24g++] = reg->channel[j];
            }
        }
    }
    wpa_printf(MSG_DEBUG,"p2p_5g_channel_num:%d", count);
    return count;
}
			  ...
			  struct p2p_channels *intersection)
{
#if defined (CONFIG_SET_5G_HIGHER_PRIORITY)
    wpa_printf(MSG_DEBUG, "+++into %s+++", __FUNCTION__);
    wpa_printf(MSG_DEBUG, "P2P: Selected operating "
            "channel (reg_class %u channel %u) not acceptable to the "
            "peer", p2p->op_reg_class, p2p->op_channel);

    int i,j;
    int p2p_5g_channel[2][160] = {0};
    int p2p_24g_channel[2][14] = {0};
    int numb_5g;
    int rank_channel_len = 0;
    u8 channel_buf[64] = {0x00};
    
    numb_5g = p2p_intersection_5g_num(p2p, intersection, p2p_5g_channel, p2p_24g_channel);
    if(NULL != p2p->cfg->channel_rank)
    {
        rank_channel_len = p2p->cfg->channel_rank(p2p->cfg->cb_ctx, channel_buf, (int)sizeof(channel_buf));
        wpa_hexdump(MSG_DEBUG, "P2P: rank channels", channel_buf, rank_channel_len);
    }
    else
    {
        wpa_printf(MSG_DEBUG, "p2p: null function pointer");
        return;
    }

    /*check intersection include 5G or not*/
    /*TV&peer devcie support 5G*/
    if(numb_5g > 1)
    {
        if(0 == rank_channel_len)
        {
            p2p->op_reg_class = p2p_5g_channel[0][0];
            p2p->op_channel = p2p_5g_channel[1][0];
            wpa_printf(MSG_DEBUG, "P2P: no rank channel,Pick 5 GHz "
    			"channel (reg_class %u channel %u) from intersection",
    			p2p->op_reg_class, p2p->op_channel);
            return;
        }

        /*find the best 5G channel*/
        for(i=0; i<rank_channel_len; i++)
        {
            if(14 < channel_buf[i])
            {
                for(j=0; j<160; j++)
                {
                    if(0 == p2p_5g_channel[1][j])
                    {
                        //wpa_printf(MSG_DEBUG, "P2P: no 5G");
                        break;
                    }
                    else if(p2p_5g_channel[1][j] == channel_buf[i])
                    {
                        p2p->op_reg_class = p2p_5g_channel[0][j];
                        p2p->op_channel = p2p_5g_channel[1][j];
                        wpa_printf(MSG_DEBUG, "P2P: Pick 5 GHz "
                			"channel (reg_class %u channel %u) from intersection",
                			p2p->op_reg_class, p2p->op_channel);
                        return;
                    }
    ...
    else if(numb_5g == 1)
    {
        ...
        for(i=0; i<rank_channel_len; i++)
        {
            if(14 < channel_buf[i])
            {
                ...
                if(p2p_5g_channel[1][0] == channel_buf[i])
                {
                    p2p->op_reg_class = p2p_5g_channel[0][0];
                    p2p->op_channel = p2p_5g_channel[1][0];
                    wpa_printf(MSG_DEBUG, "P2P: Pick 5 GHz "
        ...
        #endif
        /*try to select the 5G channel*/
		p2p->op_reg_class = p2p_5g_channel[0][0];
		p2p->op_channel = p2p_5g_channel[1][0];
        wpa_printf(MSG_DEBUG, "P2P: Pick the only one 5 GHz "
			"channel (reg_class %u channel %u) from intersection",
			p2p->op_reg_class, p2p->op_channel);
    }
    else
    {
        if(0 == rank_channel_len)
        {
            p2p->op_reg_class = p2p_24g_channel[0][0];
            p2p->op_channel = p2p_24g_channel[1][0];
            wpa_printf(MSG_DEBUG, "P2P: no rank channel,Pick 2.4 GHz "
    			"channel (reg_class %u channel %u) from intersection",
    			p2p->op_reg_class, p2p->op_channel);
            return;
        }
        /*check best channel from 2.4G*/
        for(i=0; i<rank_channel_len; i++)
        {
            if(14 >= channel_buf[i])
            {
                for(j=0; j<20; j++)
                {
                    if(0 == p2p_24g_channel[1][j])
                    {
                        wpa_printf(MSG_DEBUG, "P2P: no 2.4G");
                        break;
                    }
                    else if(p2p_24g_channel[1][j] == channel_buf[i])
                    {
                        p2p->op_reg_class = p2p_24g_channel[0][j];
                        p2p->op_channel = p2p_24g_channel[1][j];
                        wpa_printf(MSG_DEBUG, "P2P: Pick 2.4 GHz "
                			"channel (reg_class %u channel %u) from intersection",
                			p2p->op_reg_class, p2p->op_channel);
                        return;
                    }
...
#ifdef CONFIG_P2P_FORCE_GO
	if(dev != NULL)
	{
		u8 peer_device_intent = (*(msg.go_intent)) >> 1;
		if(peer_device_intent <= P2P_MAX_GO_INTENT)
		{
			dev->go_intent = peer_device_intent;
			dev->go_intent_recored = 1;
			wpa_printf(MSG_DEBUG, "peer go_intent=%d", peer_device_intent);
		}
	}
#endif /*CONFIG_P2P_FORCE_GO*/
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		default:
			...
			if (!p2p->cfg->cfg_op_channel ||
                !p2p_channels_includes(&intersection, p2p->op_reg_class, p2p->op_channel))
            {
                p2p_reselect_channel(p2p, &intersection);
            }
