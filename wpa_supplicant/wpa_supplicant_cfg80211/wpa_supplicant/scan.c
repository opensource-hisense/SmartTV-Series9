...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->last_scan_req != MANUAL_SCAN_REQ &&
		 ...
		 */
		wpa_s->reattach = 0;
	}else if (1 == wpa_s->params_hidden_ssid.num_ssids){
	    params.ssids[0].ssid = wpa_s->params_hidden_ssid.ssids[0].ssid;
		params.ssids[0].ssid_len = wpa_s->params_hidden_ssid.ssids[0].ssid_len;
		params.num_ssids = 1;
		os_memset(&(wpa_s->params_hidden_ssid), 0, sizeof(wpa_s->params_hidden_ssid));		
	}else {
