...
static int wpa_cli_cmd_scan_only(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;
	int i;
	...
	if (argc != 2) {
		printf("Invalid SCAN command: needs two arguments (SCAN "
		       "cmd and value)\n");
		return -1;
	}
	...
	if((*argv[1] != '0') && (*argv[1] != '1')){
		printf("Invalid SCAN command: value should be 0 or 1\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "SCAN %s %s",
			  argv[0], argv[1]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long SCAN command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}
...
static int wpa_cli_cmd_p2p_set_go_passphrase(struct wpa_ctrl *ctrl, int argc,
					  char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_SET_GO_PASSPHRASE", 1, argc, argv);
}
#endif
...
static int wpa_cli_cmd_p2p_get_device_addr(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_GET_DEVICE_ADDR", 0, argc, argv);
}
static int wpa_cli_cmd_p2p_get_working_mode(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_GET_WORKING_MODE", 0, argc, argv);
}
static int wpa_cli_cmd_p2p_get_if_addr(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_GET_IF_ADDR", 0, argc, argv);
}

static int wpa_cli_cmd_get_p2p_information(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_GET_INFO", 0, argc, argv);
}

#endif
#ifdef CONFIG_SUPPORT_GET_LISTEN_FREQ
static int wpa_cli_cmd_p2p_get_listen_channel(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_GET_LISTEN_CHANNEL", 0, argc, argv);
}
#endif
...
static int wpa_cli_cmd_p2p_get_all_keys(struct wpa_ctrl *ctrl, int argc,
                                               char *argv[])
{
    return wpa_cli_cmd(ctrl, "P2P_GET_ALL_KEYS", 0, argc, argv);
}
...
static int wpa_cli_cmd_tdls_status(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "TDLS_STATUS", 0, argc, argv);
}


static int wpa_cli_cmd_tdls_scan(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "TDLS_SCAN", 1, argc, argv);
}


static int wpa_cli_cmd_tdls_start(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "TDLS_START", 0, argc, argv);
}


static int wpa_cli_cmd_tdls_stop(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "TDLS_STOP", 0, argc, argv);
}
...
static struct wpa_cli_cmd wpa_cli_commands[] = {
...
#ifdef CONFIG_BSS_HANDLE
	{ "scan_only", wpa_cli_cmd_scan_only, NULL,
	  cli_cmd_flag_none,
	  "= set variables " },
#endif	  
...
#ifdef CONFIG_MANUAL_GO_PASSPHRASE_SUPPORT
	{ "p2p_set_go_passphrase", wpa_cli_cmd_p2p_set_go_passphrase, NULL,
	  cli_cmd_flag_none,
	  "= set the GO passphrase for a group (GO only)" },
#endif
...
#ifdef CONFIG_SUPPORT_GET_P2P_INFOR
	{ "p2p_get_device_addr", wpa_cli_cmd_p2p_get_device_addr, NULL,
	  cli_cmd_flag_none,
	  "= get p2p device address" },
  	{ "p2p_get_working_mode", wpa_cli_cmd_p2p_get_working_mode, NULL,
	  cli_cmd_flag_none,
	  "= get p2p working mode" },
	{ "p2p_get_if_addr", wpa_cli_cmd_p2p_get_if_addr, NULL,
	  cli_cmd_flag_none,
	  "= get p2p interface address" },
	{ "get_p2p_info", wpa_cli_cmd_get_p2p_information, NULL,
	  cli_cmd_flag_none,
	  "= get p2p related information" },
#endif
#ifdef CONFIG_SUPPORT_GET_LISTEN_FREQ
  	{ "p2p_get_listen_channel", wpa_cli_cmd_p2p_get_listen_channel, NULL,
  	  cli_cmd_flag_none,
	  "= get p2p listen freq" },
#endif
...
#ifdef CONFIG_P2P_GET_ALL_KEYS_SUPPORT
	{ "p2p_get_all_keys", wpa_cli_cmd_p2p_get_all_keys, NULL, 
	  cli_cmd_flag_none,
	  "get keys of all p2p groups" },
#endif /* CONFIG_P2P_GET_ALL_KEYS_SUPPORT */
#ifdef CONFIG_P2P_DISCONNECT_SUPPORT
	{ "p2p_disconnect", wpa_cli_cmd_p2p_disconnect, 
	   wpa_cli_complete_p2p_peer, cli_cmd_flag_none,
	   "<address|iface=address> = p2p disconnect" },
#endif
#endif /* CONFIG_P2P */
#ifdef CONFIG_WIFI_DISPLAY
	...
	{ "tdls_status", wpa_cli_cmd_tdls_status, NULL,
	  cli_cmd_flag_none,
	  "= show TDLS status" },
	{ "tdls_scan", wpa_cli_cmd_tdls_scan, NULL,
	  cli_cmd_flag_none,
	  "<addr> = send tunneld probe request to <addr>" },
	{ "tdls_start", wpa_cli_cmd_tdls_start, NULL,
	  cli_cmd_flag_none,
	  "= start TDLS over Miracast functionality" },
	{ "tdls_stop", wpa_cli_cmd_tdls_stop, NULL,
	  cli_cmd_flag_none,
	  "= stop TDLS over Miracast functionality" },
...
static void wpa_cli_action_process(const char *msg)
{
	...
	if (str_match(pos, WPA_EVENT_CONNECTED)) {
		...
		wpa_cli_exec(action_file, ifname, pos);
#ifdef CONFIG_AUTOMATIC_SCAN_RESULT
