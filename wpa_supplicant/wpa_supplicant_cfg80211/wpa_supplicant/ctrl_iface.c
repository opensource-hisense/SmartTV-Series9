...
#ifdef CONFIG_HANDLE_P2P_INVITATION_RSP_TX_STATUS
#include "p2p/p2p_i.h"
#endif /*CONFIG_HANDLE_P2P_INVITATION_RSP_TX_STATUS*/
...
#ifdef CONFIG_BLOCK_STA_SCAN_CMD_SUPPORT
static int block_scan = 0;
#endif /* CONFIG_BLOCK_STA_SCAN_CMD_SUPPORT */

#ifdef CONFIG_BSS_HANDLE
int b_scan_only = 0;
int b_connect = 0;
#endif
					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "EAPOL::heldPeriod") == 0) {
    ...
    } else if (os_strcasecmp(cmd, "wfd_devType") == 0) {
        wifi_display_set_wfd_dev_info(wpa_s->global, cmd, !!atoi(value));
        ...
        wifi_display_set_wfd_dev_info(wpa_s->global, cmd, !!atoi(value));
	} else if (os_strcasecmp(cmd, "wfd_bssid") == 0) {
		wifi_display_set_wfd_bssid(wpa_s->global, cmd, value);
...
#endif /* CONFIG_NO_CONFIG_BLOBS */
#ifdef CONFIG_BLOCK_STA_SCAN_CMD_SUPPORT
	} else if (os_strcasecmp(cmd, "block_scan") == 0) {
		block_scan = atoi(value);
		wpa_printf(MSG_DEBUG, "block_scan=%d", block_scan);
#endif /* CONFIG_BLOCK_STA_SCAN_CMD_SUPPORT */
	} else {
		value[-1] = '=';
...
#ifdef CONFIG_WIFI_DISPLAY
static int wpa_supplicant_ctrl_iface_tdls_status(
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	...
	ret = os_snprintf(pos, end - pos, "TDLS status:\n"
			"tdls_external_control=%d\n"
			"tdls_disabled=%d\n"
			"tdls_prohibited=%d\n"
			"tdls_supported=%d\n"
			"tdls_external_setup=%d\n"
			"tdls_peer_ip=%s\n"
			"tdls_dev_info=%s\n"
			"tdls_link_state=%d\n",
			wpa_s->conf->tdls_external_control,
			wpa_tdls_is_disabled(wpa_s->wpa),
			wpa_tdls_is_prohibited(wpa_s->wpa),
			wpa_tdls_is_supported(wpa_s->wpa),
			wpa_tdls_is_external_setup(wpa_s->wpa),
			wpa_tdls_peer_ip(wpa_s->wpa),
			wpa_tdls_dev_info(wpa_s->wpa),
			wpa_tdls_link_state(wpa_s->wpa));
...
static int wpa_supplicant_ctrl_iface_tdls_probe_request(
	struct wpa_supplicant *wpa_s, char *addr)
{
	u8 peer[ETH_ALEN];
	int ret;
	...
	if (hwaddr_aton(addr, peer)) {
		wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_PROBE_REQUEST: invalid "
			   "address '%s'", addr);
		return -1;
	}

	wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_PROBE_REQUEST " MACSTR,
		   MAC2STR(peer));

	if (wpa_tdls_is_external_setup(wpa_s->wpa))
		ret = wpa_tdls_send_probe_request(wpa_s->wpa, peer);
	else
		ret = -1; /*tunneled probe request is not a native support procedure*/

	return ret;
}


static int wpa_supplicant_ctrl_iface_tdls_wfd_init(
	struct wpa_supplicant *wpa_s)
{
	int ret;

	wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_WFD_INIT");

	if (wpa_tdls_is_external_setup(wpa_s->wpa))
		ret = wpa_tdls_wfd_init(wpa_s);
	else
		ret = -1; /*tdls enable is not a native support procedure*/

	return ret;
}


static int wpa_supplicant_ctrl_iface_tdls_wfd_exit(
	struct wpa_supplicant *wpa_s)
{
	int ret;

	wpa_printf(MSG_DEBUG, "CTRL_IFACE TDLS_WFD_EXIT");

	if (wpa_tdls_is_external_setup(wpa_s->wpa))
		ret = wpa_tdls_wfd_exit(wpa_s);
	else
		ret = -1; /*tdls disable is not a native support procedure*/
...
{
#ifdef CONFIG_MANUAL_GO_PASSPHRASE_SUPPORT
	if (os_strcmp(name, "passphrase") == 0)
	{
		wpa_printf(MSG_DEBUG, "<mtk94097>  original passphrase is <%s>",ssid->passphrase);

//first.  update the value in ram
		ssid->passphrase = os_strdup(value);

//second. update the value in p2p_supplicant.conf
		wpa_printf(MSG_DEBUG, "<mtk94097>  confname is <%s>",wpa_s->confname);
		//wpa_supplicant_ctrl_iface_save_config(wpa_s);
	}
#endif
...
#ifdef CONFIG_MANUAL_GO_PASSPHRASE_SUPPORT
	else if ((os_strcmp(name, "passphrase") == 0 &&
		 value[0] == '"' && ssid->ssid_len) ||
		(os_strcmp(name, "ssid") == 0 && ssid->passphrase))
					wpa_config_update_psk(ssid);
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT			
			if((i > 0) && (chnl[i].chan < chnl[i-1].chan))
				continue;
			else if (chnl[i].flag & HOSTAPD_CHAN_DISABLED)
				ret = os_snprintf(pos, end - pos, "\n\t%d (Disabled)", chnl[i].chan);
			else if (chnl[i].flag & HOSTAPD_CHAN_PASSIVE_SCAN)
				ret = os_snprintf(pos, end - pos, "\n\t%d (Passive)", chnl[i].chan);	
			else
				ret = os_snprintf(pos, end - pos, "\n\t%d (Active)", chnl[i].chan);
#else
...
#ifdef CONFIG_HANDLE_P2P_INVITATION_RSP_TX_STATUS
	if(wpa_s->global->p2p->pending_action_state != 0)
	{
		wpa_printf(MSG_ERROR, "skip p2p_listen since wpa_s->global->p2p->pending_action_state(%d) != P2P_NO_PENDING_ACTION", wpa_s->global->p2p->pending_action_state);
		return 0;
	}
#endif /*CONFIG_HANDLE_P2P_INVITATION_RSP_TX_STATUS*/
...
#ifdef CONFIG_MANUAL_GO_PASSPHRASE_SUPPORT
static int p2p_set_go_passphrase(struct wpa_supplicant *wpa_s, char *cmd, char *buf,
			      size_t buflen)
{
	struct wpa_ssid *ssid;
	struct wpa_global *global = wpa_s->global;
	struct wpa_supplicant *prev = global->ifaces;

	while (prev) {
		wpa_printf(MSG_ERROR,"%s:%d, %d  >>> ssid check ",__FUNCTION__,__LINE__,prev->current_ssid);
		wpa_printf(MSG_ERROR,"%s:%d, %s  >>> ifname check",__FUNCTION__,__LINE__,prev->ifname);
	...
	while (prev) {
		if (os_strcmp(prev->ifname, "p2p1") == 0)
		{
			wpa_s = prev;
			break;
		}
	...
	{
		wpa_printf(MSG_ERROR,"%s:%d  >>> ssid is NULL ",__FUNCTION__,__LINE__);
		return -1;
	}

#ifdef CONFIG_DEBUG_MANUAL_GO_PASSPHRASE_SUPPORT
		wpa_hexdump(MSG_DEBUG,"<mtk94097> psk old",(u8 const *)ssid->psk,32);
		wpa_hexdump(MSG_DEBUG,"<mtk94097> phs old",(u8 const *)ssid->passphrase,16);
#endif
	...
	if( cmd == NULL || cmd[0] == '\0' )
	{
		wpa_printf(MSG_ERROR,"%s:%d  >>> cmd is NULL",__FUNCTION__,__LINE__);
		return -1;
	}

	wpa_printf(MSG_ERROR, "Start >%s<",cmd);
	...
	if (len < 8 || len > 63) 
	{
		wpa_printf(MSG_ERROR, "Invalid passphrase : length %lu (expected: 8..63) '%s'.",
		   (unsigned long) len, cmd);
		return -1;
	}
	...
	{
		wpa_hexdump_ascii_key(MSG_DEBUG, "PSK (ASCII passphrase)",(u8 *) cmd, len);
		
		if (ssid->passphrase && os_strlen(ssid->passphrase) == len &&
			os_memcmp(ssid->passphrase, cmd , len) == 0)
		{
			wpa_printf(MSG_ERROR, "Same passphrase %s:%u", cmd , (unsigned int) len);
			return 0;
		}
		...
		{
			wpa_printf(MSG_ERROR, "os_malloc failed ..");
			return -1;
		}
		os_memcpy(ssid->passphrase, cmd , len);
		ssid->passphrase[len] = '\0';

		wpa_printf(MSG_ERROR, "Done >%s<",ssid->passphrase);

		wpa_config_update_psk(ssid);

		// update passphrase & psk in ap mode, without this ,4-way handshake will failed
		wpa_supplicant_associate(wpa_s, NULL, ssid);

#ifdef CONFIG_DEBUG_MANUAL_GO_PASSPHRASE_SUPPORT
	wpa_hexdump(MSG_DEBUG,"<mtk94097> psk new",(u8 const *)ssid->psk,32);
	wpa_hexdump(MSG_DEBUG,"<mtk94097> phs new",(u8 const *)ssid->passphrase,16);
#endif
	...
	{
		wpa_printf(MSG_ERROR,"%s:%d  >>> MODE is invalid",__FUNCTION__,__LINE__);
		return -1;
	}
}
#endif
...
#ifdef CONFIG_MTK_HE_SUPPORT
	if (os_strncmp(cmd, "STA_REMOVE ", 11) == 0) {
        u8 addr[ETH_ALEN];
        if (hwaddr_aton(param+1, addr))
            return -1;
...
#ifdef CONFIG_SUPPORT_GET_P2P_INFOR
static int p2p_get_device_addr(struct wpa_supplicant *wpa_s,char *buf, size_t buflen)
{
	int ret = -1;
    if (wpa_s->global->p2p)
    {
        ret = os_snprintf(buf, buflen, "p2p_device_address=" MACSTR
                          "\n", MAC2STR(wpa_s->global->p2p_dev_addr));
    }
	return ret;
}
static int p2p_get_working_mode(struct wpa_supplicant *wpa_s,char *buf, size_t buflen)
{
	int ret = -1;
	int freq = 0;
	struct wpa_ssid *ssid;
	//p2p_type 0: p2p device 1: p2p go 2: p2p client
	struct wpa_global *global = wpa_s->global;
	struct wpa_supplicant *prev = global->ifaces;

		while (prev) {
		wpa_printf(MSG_ERROR,"%s:%d, %d  p2p_get_working_mode>>> ssid check ",__FUNCTION__,__LINE__,prev->current_ssid);
		wpa_printf(MSG_ERROR,"%s:%d, %s  >>> ifname check",__FUNCTION__,__LINE__,prev->ifname);
    ...
    if (ssid != NULL &&
            ssid->p2p_group)
    {
    wpa_dbg(wpa_s, MSG_ERROR, "p2p_get_working_mode '%d'", ssid->mode);
        if (ssid->mode == WPAS_MODE_P2P_GO)
        {
        	freq = ssid->frequency;
            ret = os_snprintf(buf, buflen, "p2p_type=1 oper_freq=%d\n",freq);
			wpa_printf(MSG_ERROR,"p2p_type = %s  ",buf);
        }
        else if (wpa_s->current_ssid->mode == WPAS_MODE_INFRA)
        {
            freq = wpa_s->current_bss ? wpa_s->current_bss->freq :
                   (int) wpa_s->assoc_freq;
            ret = os_snprintf(buf, buflen, "p2p_type=2 oper_freq=%d\n",freq);
			wpa_printf(MSG_ERROR,"p2p_type = %s  ",buf);
        }
    }
	else
	{
	   wpa_dbg(wpa_s, MSG_ERROR, "p2p_get_working_mode p2p_type=0 '%s'", buf);
        ret = os_snprintf(buf, buflen, "p2p_type=0 oper_freq=0\n");
	}
	return ret;
}
static int p2p_get_own_if_addr(struct wpa_supplicant *wpa_s,char *buf, size_t buflen)
{
	int ret = -1;
	u8 interface_address[ETH_ALEN] = {0};
	struct p2p_data *p2p = wpa_s->global->p2p;
    if (p2p)
    {
    	p2p_get_own_interface_address(p2p, interface_address);
        ret = os_snprintf(buf, buflen, "p2p_interface_address=" MACSTR
                          "\n", MAC2STR(interface_address));
    }
	return ret;
}

static int p2p_get_related_info(struct wpa_supplicant *wpa_s,char *buf, size_t buflen)
{
	int ret = -1;
	u8 interface_address[ETH_ALEN] = {0};
    ...
    {
    	p2p_get_own_interface_address(p2p, interface_address);
		p2p_get_own_device_name(p2p, device_name);
		p2p_get_own_pri_dev_type(p2p, pri_dev_type);
		config_methods = p2p_get_own_config_methods(p2p);
		dev_capab = p2p_get_own_dev_capab(p2p);
		group_capab = p2p_get_own_group_capab(p2p);
        ret = os_snprintf(buf, buflen, "device_address=" MACSTR 
									   ", interface_address=" MACSTR
                          			   ", device_name=%s"
                          			   ", pri_dev_type=%s"
                          			   ", config_methods=0x%x"
                          			   ", dev_capab=0x%x"
                          			   ...
                          			   MAC2STR(interface_address),
                          			   device_name,
                          			   wps_dev_type_bin2str(pri_dev_type,
					       						devtype, sizeof(devtype)),
					       			  config_methods,
					       			  dev_capab,
					       			  group_capab);
...
#ifdef CONFIG_SUPPORT_GET_LISTEN_FREQ
static int p2p_ctrl_get_listen_channel(struct wpa_supplicant *wpa_s,char *buf, size_t buflen)
{
	int ret = -1;
    int freq=0;
	struct p2p_data *p2p = wpa_s->global->p2p;
    if (p2p)
    {
    	freq = p2p_get_listen_freq(p2p);
		if(freq < 0)
		{
			freq = 0;
		}
		ret = os_snprintf(buf, buflen, "listen_freq=%d\n",freq);
    }
	return ret;
}
#endif
...
static int p2p_ctrl_disconnect(struct wpa_supplicant *wpa_s, const char *cmd)
{
	...
	struct wpa_supplicant *prev = global->ifaces;
		...
		while (prev) {
		wpa_printf(MSG_ERROR,"%s:%d, %d  p2p_ctrl_disconnect>>> ssid check ",__FUNCTION__,__LINE__,prev->current_ssid);
		wpa_printf(MSG_ERROR,"%s:%d, %s  >>> ifname check",__FUNCTION__,__LINE__,prev->ifname);
    ...
    if (ssid != NULL &&
            ssid->p2p_group)
    {
        wpa_dbg(wpa_s, MSG_ERROR, "p2p_ctrl_disconnect  mode = %d", ssid->mode);
        if (ssid->mode == WPAS_MODE_P2P_GO)
        {
	p2p_ctrl_remove_client(wpa_s,cmd);
			wpa_printf(MSG_ERROR,"WPAS_MODE_P2P_GO  disconnect");
    }
        else 
        {
            wpas_p2p_group_remove(wpa_s,wpa_s->ifname);
			wpa_printf(MSG_ERROR,"P2P_GC disconnect");
    }
    }
...
#ifdef CONFIG_P2P_GET_ALL_KEYS_SUPPORT
static int p2p_ctrl_get_all_keys(struct wpa_supplicant *wpa_s)
{
    wpa_printf(MSG_ERROR, "CTRL_IFACE P2P_GET_ALL_KEYS");
    
    wpa_printf(MSG_DEBUG, "All Group Keys = ( @ [ %s ] @ [ line:%d ] @ [ %s ] ) ", __FUNCTION__, __LINE__, __FILE__);
    struct wpa_ssid *ps;
    struct psk_list_entry *psk, *tmp;
    int i = 0, j = 0, k = 0;
    u8 ssid_tmp[ 33 ] = { '\0' };

    for (ps = wpa_s->parent->conf->ssid; ps; ps = ps->next) {
        memset(ssid_tmp, 0, sizeof(ssid_tmp));
        memcpy( ssid_tmp, ps->ssid, ps->ssid_len );
        wpa_printf(MSG_DEBUG, "Group[%d]: bssid=" MACSTR " ssid=%s passphrase=%s", 
            i, MAC2STR(ps->bssid), ssid_tmp, ps->passphrase);
        wpa_hexdump(MSG_DEBUG, "PSK", (u8 const *)ps->psk, 32);
        i++;

        j = 0;
        dl_list_for_each_safe(psk, tmp, &ps->psk_list, struct psk_list_entry, list) {
            wpa_printf(MSG_DEBUG, "client[%d]: " MACSTR " psk=", j, MAC2STR(psk->addr));
            wpa_hexdump(MSG_DEBUG, "Per-Client PSK", (u8 const *)psk->psk, 32);
            j++;
	    }
	}
...
#endif /* CONFIG_P2P_GET_ALL_KEYS_SUPPORT */
				     ...
				     char *buf, size_t buflen)
{
	...
	if (os_strncasecmp(cmd, "ACTIVE_SCAN", 11) == 0) {
		/* Set the active scan flag first in wpa_drv_driver_cmd */
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (params) {
...
#ifdef CONFIG_BSS_HANDLE		
		pos = os_strstr(params, "scan_only ");
		if (pos){
			b_scan_only = atoi(pos+10);
			wpa_printf(MSG_DEBUG, "CTRL_IFACE: SCAN scan_only=%d", b_scan_only);
		}

		pos = os_strstr(params, "connect ");
		if (pos){
			if((atoi(pos+8) == 1) && (b_scan_only == 1)){
				b_connect = 1;
			}
			else 
				b_connect = 0;
			wpa_printf(MSG_DEBUG, "CTRL_IFACE: SCAN connect=%d", b_connect);
		}
#endif
		...
		pos = os_strstr(params, "ssid=\"");
		if (pos){
			struct wpa_driver_scan_params scan_params;
			char ssid[32]={0};
			char *pos_end = os_strstr(params, " \"");
							
			if(pos_end){
				int len_tmp = strlen("ssid=\"");					
				int ssid_len = pos_end - pos -len_tmp;
				os_memcpy(ssid,pos+len_tmp,ssid_len);			
				os_memset(&scan_params, 0, sizeof(scan_params));
				scan_params.ssids[0].ssid = ssid;
				scan_params.ssids[0].ssid_len = ssid_len;
				scan_params.num_ssids = 1;
				os_memcpy(&(wpa_s->params_hidden_ssid),&scan_params,
							sizeof(scan_params));				
			}
       }
	} else {
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		/* For Google cast command "SET country XX", translate it
		*   to driver command "SETCOUNTRYREV XX".
		*/
		char cmd[32] = { 0 };
		os_snprintf(cmd, sizeof(cmd), "SETCOUNTRYREV ");
		os_memcpy(cmd + 14, buf + 12, 2);
		cmd[sizeof(cmd) - 1] = '\0';
		reply_len = wpa_supplicant_driver_cmd(wpa_s, cmd, reply,
					      reply_size);
#endif
...
#ifdef CONFIG_MANUAL_GO_PASSPHRASE_SUPPORT
	} else if (os_strncmp(buf, "P2P_SET_GO_PASSPHRASE ", 22) == 0) {
		reply_len = p2p_set_go_passphrase(wpa_s, buf + 22, reply, reply_size);
#endif
		...
		if (p2p_ctrl_ext_listen(wpa_s, "") < 0)
			reply_len = -1;
#ifdef CONFIG_SUPPORT_GET_P2P_INFOR
	} else if (os_strcmp(buf, "P2P_GET_DEVICE_ADDR") == 0) {
		reply_len = p2p_get_device_addr(wpa_s,reply,reply_size);
	} else if (os_strcmp(buf, "P2P_GET_WORKING_MODE") == 0) {
		reply_len = p2p_get_working_mode(wpa_s,reply,reply_size);
	} else if (os_strcmp(buf, "P2P_GET_IF_ADDR") == 0) {
		reply_len = p2p_get_own_if_addr(wpa_s, reply, reply_size);
	} else if (os_strcmp(buf, "P2P_GET_INFO") == 0) {
		reply_len = p2p_get_related_info(wpa_s, reply, reply_size);
#endif
#ifdef CONFIG_SUPPORT_GET_LISTEN_FREQ
	} else if (os_strcmp(buf, "P2P_GET_LISTEN_CHANNEL") == 0) {
		reply_len = p2p_ctrl_get_listen_channel(wpa_s,reply,reply_size);
#endif
	} else if (os_strncmp(buf, "P2P_REMOVE_CLIENT ", 18) == 0) {
		if (p2p_ctrl_remove_client(wpa_s, buf + 18) < 0)
			reply_len = -1;
#ifdef CONFIG_P2P_GET_ALL_KEYS_SUPPORT
	} else if (os_strncmp(buf, "P2P_GET_ALL_KEYS", 16) == 0) {
		if (p2p_ctrl_get_all_keys(wpa_s) < 0)
			reply_len = -1;			
#endif /* CONFIG_P2P_GET_ALL_KEYS_SUPPORT */
#ifdef CONFIG_P2P_DISCONNECT_SUPPORT		
	} else if (os_strncmp(buf, "P2P_DISCONNECT ", 15) == 0) {
		if (p2p_ctrl_disconnect(wpa_s, buf + 15) < 0)
			reply_len = -1;
#endif
#endif /* CONFIG_P2P */
#ifdef CONFIG_WIFI_DISPLAY
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (block_scan)
		{
			wpa_printf(MSG_DEBUG, "Block Scan!");
			os_memcpy(reply, "OK\n", 3);
			reply_len = 3;
		}
		else
		...
		if (wpa_supplicant_ctrl_iface_tdls_teardown(wpa_s, buf + 14))
			reply_len = -1;
	} else if (os_strncmp(buf, "TDLS_STATUS", 11) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_tdls_status(wpa_s, reply, reply_size);
	} else if (os_strncmp(buf, "TDLS_SCAN ", 10) == 0) {
		if (wpa_supplicant_ctrl_iface_tdls_probe_request(wpa_s, buf + 10))
			reply_len = -1;
	} else if (os_strncmp(buf, "TDLS_START", 10) == 0) {
		if (wpa_supplicant_ctrl_iface_tdls_wfd_init(wpa_s))
			reply_len = -1;
	} else if (os_strncmp(buf, "TDLS_STOP", 9) == 0) {
		if (wpa_supplicant_ctrl_iface_tdls_wfd_exit(wpa_s))
			reply_len = -1;
#endif /* CONFIG_TDLS */
					       ...
					       char *buf, size_t *resp_len)
{
	...
	static const char * cmd[] = {
		...
		"P2P_PRESENCE_REQ",
		"P2P_EXT_LISTEN",
		"P2P_GET_WORKING_MODE",
		"P2P_GET_INFO",
		"P2P_GET_DEVICE_ADDR",
		"P2P_GET_LISTEN_CHANNEL",
		"P2P_GET_WPS_PIN",	
		NULL
	};
	static const char * prefix[] = {
		...
		"NFC_REPORT_HANDOVER ",
		"WPS_PIN ",
		"P2P_REMOVE_CLIENT_PER_STA_KEY ",
		"WPS_NFC_TOKEN ",
		"WPS_NFC ",
		"GET_CAPABILITY ",
		"P2P_SET_GO_PASSPHRASE ",
		"SET_NETWORK ",
		"P2P_DISCONNECT ",
		NULL
	};
