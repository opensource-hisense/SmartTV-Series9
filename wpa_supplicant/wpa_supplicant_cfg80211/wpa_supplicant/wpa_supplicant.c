...
#if defined(CONFIG_P2P) && defined(CONFIG_ANDROID_LOG) && defined(CONFIG_P2P_LISTEN_CH_FOLLOW_AP)
    /* make p2p listen channel as associated AP channel */
    if ((os_strncmp(wpa_s->ifname, "wlan0", 5) == 0) && wpa_s->global->p2p)
    {
        u8 reg_class = 0, new_channel = 0, cfg_op_channel = 0;
        unsigned int r;
		int p2p_freq_to_channel(unsigned int freq, u8 *op_class, u8 *channel);
        struct p2p_data *p2p = wpa_s->global->p2p;
        
        if (state == WPA_COMPLETED && old_state != WPA_COMPLETED)
        {
            if (wpa_s->assoc_freq)
            {
                if (!p2p_freq_to_channel(wpa_s->assoc_freq, &reg_class, &new_channel) &&
					(new_channel == 1 || new_channel == 6|| new_channel == 11) &&
                    !p2p_set_listen_channel(p2p, reg_class, new_channel, 1))
                    wpa_printf(MSG_DEBUG, "Set P2P Listen Channel to AP Channel (%d-ch%d)\n",
                               wpa_s->assoc_freq, new_channel);
                if (!p2p_set_oper_channel(p2p, reg_class, new_channel, cfg_op_channel))
                    wpa_printf(MSG_DEBUG, "Set P2P Operation Channel to AP Channel (%d-ch%d)\n",
                               wpa_s->assoc_freq, new_channel);
            }
        }
        else if (state != WPA_COMPLETED && old_state == WPA_COMPLETED)
        {
            wpa_printf(MSG_DEBUG, "Reset P2P Listen/Oper Channel\n");
            if (wpa_s->conf->p2p_listen_reg_class &&
                wpa_s->conf->p2p_listen_channel) 
            {
                reg_class = wpa_s->conf->p2p_listen_reg_class;
                new_channel = wpa_s->conf->p2p_listen_channel;
            }
            else
            {
                reg_class = 81;
                ...
                new_channel = 1 + (r % 3) * 5;
#ifdef CONFIG_FIXED_P2P_LISTEN_CHANNEL
                new_channel = CONFIG_FIXED_P2P_LISTEN_CHANNEL;
                wpa_printf(MSG_DEBUG, "P2P: Fix Listen Channel to %d", new_channel);
#endif
            }
            p2p_set_listen_channel(p2p, reg_class, new_channel, 1);
            /* OP channel */
            ...
            {
                reg_class = wpa_s->conf->p2p_oper_reg_class;
                new_channel = wpa_s->conf->p2p_oper_channel;
                cfg_op_channel = 1;
            } else {
                reg_class = 81;
                /* set to the same ch as listen channel */
                cfg_op_channel = 0;
#ifdef CONFIG_FIXED_P2P_OP_CHANNEL
                new_channel = CONFIG_FIXED_P2P_OP_CHANNEL;
                wpa_printf(MSG_DEBUG, "P2P: Fix Operation Channel to %d", new_channel);
#endif
            }
            if (!p2p_set_oper_channel(p2p, reg_class, new_channel, cfg_op_channel))
                wpa_printf(MSG_DEBUG, "P2P: %s: Operation Channel = %d\n", __FUNCTION__, new_channel);
...
#if defined(ANDROID) || defined(PRIVATE_CMD)
	wpa_msg(wpa_s, MSG_ERROR, "[%s] set country_region %s", __func__, iface->country_code);
	if (iface->country_code) {
		reply = os_malloc(reply_size);

		snprintf(cmd, sizeof(cmd), "SETCOUNTRYREV %s", iface->country_code);
		if (wpa_drv_driver_cmd(wpa_s, cmd, reply, reply_size) < 0)
			wpa_msg(wpa_s, MSG_ERROR, "Failed to set country code '%s'", iface->country_code);
