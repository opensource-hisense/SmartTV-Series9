...
static inline int wpa_drv_get_acs_rank_list(struct wpa_supplicant *wpa_s,
				   u8 *buf, int buf_len)
{
	if (!wpa_s->driver->get_acs_rank_list)
		return -1;
	return wpa_s->driver->get_acs_rank_list(wpa_s->drv_priv, buf, buf_len);
}
