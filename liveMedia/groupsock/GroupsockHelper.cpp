...
static unsigned increaseBufferTo(UsageEnvironment& env, int bufOptName,
  ...
  env << "<LiveMedia>Current sock buf size = " << curSize << "\n";
  ...
  while (requestedSize > curSize) {
    ...
    if (setsockopt(socket, SOL_SOCKET, bufOptName,
      ...
      // success
      socketErr(env, "[LIVEMIDEA] Setsocket opt success: ");
      return requestedSize;
    }
	socketErr(env, "[LIVEMIDEA] Setsocket opt error: ");
...
netAddressBits ourIPAddress(UsageEnvironment& env) {
  ...
  /* sarick modify: Fix sometimes stuck at sending WFD M6 (SETUP) */
  return 0;
