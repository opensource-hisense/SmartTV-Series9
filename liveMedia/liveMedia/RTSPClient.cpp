...
#ifdef MTK_WFD_RTSP
    sprintf(userAgentName, "%s", applicationName);
#else
...
int RTSPClient::openConnection()
{
    do
    {
        ...
        // modified by wen.tang: parameter "False" -> to use blocked socket, to keep tcp connection sync
        fInputSocketNum = fOutputSocketNum = setupStreamSocket(envir(), 0, true); 
...
int RTSPClient::registerWfdCallback(responseHandler * handler)
{
    printf("RTSPClient::registerWfdCallback handler = %p \n", handler);
    if(!handler)
    {
        printf("Error RTSPClient::registerWfdCallback handler is NULL \n");
        return -1;
    }
    mWfdResponseHandler = handler;
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum)
{
	...
	//printf("connectToServer begin Time:(%lu.%03lu)s \n", timestamp.tv_sec, timestamp.tv_usec/1000);
	    if (1)
    {
        envir() << "Opening connection to " << our_inet_ntoa(remoteName.sin_addr) << ", port " << remotePortNum << ", socketNum " << socketNum << "...\n";
		//envir() << "connectToServer begin Time: " <<  timestamp.tv_sec <<  timestamp.tv_usec/1000 << "s...\n";
    }
    if (connect(socketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0)
    {
        if (envir().getErrno() == EINPROGRESS)
        {
          ...
          /*  envir().taskScheduler().setBackgroundHandling(socketNum, SOCKET_WRITABLE|SOCKET_EXCEPTION,
			...
			envir() << "...connectToServer:envir().getErrno() == EINPROGRESS\n";
		    ...
		    int res;
			int flag;
		    res = select(socketNum + 1, &rset, &set, NULL, &timeout);
			...
			if(res!=1)
				...
				{
                  if (FD_ISSET(socketNum, &set))
                  	{
                     flag =~O_NONBLOCK;
					 fcntl(socketNum, F_SETFL,flag);
					 envir() << "...local connection opened\n";
					 return socketNum;
				  }
				  else
				  	{
				  	 envir() << "connectToServer:other error happen!\n";
                     return -1;
				   }
			}
            return 0;
	...
	gettimeofday(&timestamp, NULL);
	//printf("connectToServer end Time:(%lu.%03lu)s \n", timestamp.tv_sec, timestamp.tv_usec/1000);
    if (1) envir() << "...local connection opened\n";
...
unsigned RTSPClient::sendRequest(RequestRecord* request)
{
    ...
    do
    {
        ...
        {
            extraHeaders = (char *)"Require: org.wfa.wfd1.0\r\n";
            cmdURL = (char *)"*";
        }
		...
		else if (strcmp(request->commandName(), "SET_PARAMETER") == 0)
        {
		    if(fLastSessionId!=NULL)
            {
				...
				char* headertype = (char*)"Content-Type: text/parameters\r\n";
				extraHeaders = new char[strlen(sessionStr)+strlen(headertype) + 1];
			    sprintf(extraHeaders, "%s%s", sessionStr, headertype);
				extraHeadersWereAllocated = True;
				delete[] sessionStr;
            }
			else
			{
                 extraHeaders = (char*)"Content-Type: text/parameters\r\n";
			}
#ifdef MTK_WFD_RTSP
            if (request->session())
                if (request->session()->controlPath())
                {
                    char *cmdURLtmp = new char[strlen(request->session()->controlPath()) + 1];
                    sprintf(cmdURLtmp, "%s", request->session()->controlPath());
                    /* The cmdURL we get here is:
                            SET_PARAMETER rtsp://192.168.43.1/wfd1.0/streamid=0 RTSP/1.0
                       But according to WFD spec, the url format is:
                            SET_PARAMETER rtsp://192.168.43.1/wfd1.0 RTSP/1.0
                       So we need to strip the "/streamid=0".
                     */
                    cmdURL = new char[strlen(request->session()->controlPath()) + 1];
                    cmdURLWasAllocated = True;
                    char *tmpptr = cmdURLtmp;
                    if (tmpptr = strstr(cmdURLtmp, "/streamid=0"))
                    {
                        /* need to strip */
                        strncpy(cmdURL, cmdURLtmp, (tmpptr - cmdURLtmp));
                        tmpptr = strchr(tmpptr, ' ');
                        if (tmpptr)
                        {
                            strncat(cmdURL, tmpptr, strlen(tmpptr));
                        }
                    }
                    else
                    {
                        /* don't need to strip */
                        sprintf(cmdURL, "%s", cmdURLtmp);
                    }
                    delete[] cmdURLtmp; 
                }
#endif
...
#ifdef MTK_WFD_RTSP
            /* We shall carry "wfd_presentation_URL" in the M6 req as described in spec. */
            /* !!! In D1.47 spec, the wfd_presentation_URL is not sent in M6 Req anymore !!! */
            /*
            char presentationUrlStr[128];
            memset(presentationUrlStr, 0, sizeof(presentationUrlStr));
            sprintf(presentationUrlStr, "wfd_presentation_URL: %s none\r\n", suffix);
            */
#endif
...
#ifdef MTK_WFD_RTSP
                transportFmt = "Transport: RTP/AVP/UDP%s%s%s=%d\r\n";
#else
...
#ifdef MTK_WFD_RTSP
            if (request->subsession())
                if (request->subsession()->controlPath())
                {
                    cmdURL = new char[strlen(request->subsession()->controlPath()) + 1];
                    cmdURLWasAllocated = True;
                    sprintf(cmdURL, "%s", request->subsession()->controlPath());
                }
#else
...
#ifdef MTK_WFD_RTSP
            /* in D1.47, the wfd_presentation_URL is not sent with M6 */
            //extraHeaders = new char[transportSize + strlen(sessionStr) + strlen(presentationUrlStr)];
            //sprintf(extraHeaders, "%s%s%s", presentationUrlStr, transportStr, sessionStr);
        ...
        else     // "PLAY", "PAUSE", "TEARDOWN", "RECORD", "SET_PARAMETER", "GET_PARAMETER"
        {
            ...
            if (strcmp(request->commandName(), "PLAY") == 0)
            {
                ...
                extraHeadersWereAllocated = True;
#ifdef MTK_WFD_RTSP
                sprintf(extraHeaders, "%s%s", sessionStr, scaleStr);
                if (request->session())
                    if (request->session()->controlPath())
                    {
                        cmdURL = new char[strlen(request->session()->controlPath()) + 1];
                        cmdURLWasAllocated = True;
                        sprintf(cmdURL, "%s", request->session()->controlPath());
                    }
#else
            ...
            else if((strcmp(request->commandName(), "TEARDOWN") == 0) ||
                    (strcmp(request->commandName(), "PAUSE") == 0))
            {
...
//#endif
        ...
        /* Dump RTSP message content */
        fprintf(stderr, "Dumping RTSP Request message\n");
        fprintf(stderr, "------------------------------\n");
...
void RTSPClient::handleIncomingRequest()
{
    ...
    {
        // modified by wen.tang
        if(mWfdResponseHandler)
        {
            fprintf(stderr, "Received incoming RTSP message\n");
            fprintf(stderr, "------------------------------\n");
            fprintf(stderr, "%s", fResponseBuffer? fResponseBuffer:"<NULL>");
            fprintf(stderr, "------------------------------\n");
            mWfdResponseHandler(this, atoi(cseq), fResponseBuffer);
        }
        else
        {
...
#ifdef MTK_WFD_RTSP
static char* getNextLine(char* startOfLine)
{
...
//Handle M2&M3 receive together
#ifdef MTK_WFD_RTSP
	if (bytesRead > 0)
	{
		char* parseCSeqDataCopy = new char[responseBufferSize];
		strncpy(parseCSeqDataCopy, &fResponseBuffer[fResponseBytesAlreadySeen], bytesRead);
		parseCSeqDataCopy[bytesRead] = '\0';
		fprintf(stderr, "*********************\n");
		fprintf(stderr, "Receive Message: %s\n", parseCSeqDataCopy);
		fprintf(stderr, "*********************\n");

		char* preMessageStart = parseCSeqDataCopy;
		char* nextLineStart = getNextLine(preMessageStart);

		while (1)
		{
			if (nextLineStart == NULL || nextLineStart == '\0')
			{
				strncpy(&fResponseBuffer[fResponseBytesAlreadySeen], preMessageStart, parseCSeqDataCopy + bytesRead - preMessageStart);
				handleResponseBytes(parseCSeqDataCopy + bytesRead - preMessageStart);
				break;
			}
			
			if ((_strncasecmp(nextLineStart, "GET_PARAMETER", 13) == 0) ||
						(_strncasecmp(nextLineStart, "SET_PARAMETER", 13) == 0))
			{
				handleResponseBytes(nextLineStart - preMessageStart);
				preMessageStart = nextLineStart;
			}
			
			nextLineStart = getNextLine(nextLineStart);
		}
		delete[] parseCSeqDataCopy;
	}
	else
	{
#endif
...
void RTSPClient::handleResponseBytes(int newBytesRead)
{
    ...
    if (foundRequest != NULL && foundRequest->handler() != NULL)
    {
        ...
        if (responseSuccess)
        {
            fprintf(stderr, "Dumping RTSP Response message\n");
            fprintf(stderr, "------------------------------\n");
            fprintf(stderr, "%s", fResponseBuffer);
            fprintf(stderr, "------------------------------\n");            
            if (responseCode == 200)
            {
                resultCode = 0;
...
#ifdef ENABLE_MTK_RTSP
void RTSPClient::setCustomData(std::string &data)
{
    //strCustomData.erase();
    //strCustomData = data;
}
