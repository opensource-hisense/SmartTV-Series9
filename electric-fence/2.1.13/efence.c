...
efence_free(void * address)
{
	...
	if ( allocationList == 0 )
		EF_Abort("efence_free() called before first efence_malloc().");
	...
	if ( !slot )
		EF_Abort("efence_free(%a): address not from efence_malloc().", address);
	...
	if ( slot->mode != ALLOCATED ) {
		if ( internalUse && slot->mode == INTERNAL_USE )
			...
			EF_Abort(
			 "efence_free(%a): freeing efence_free memory."
			,address);
		}
	}
...
efence_malloc(size_t size)
{
        ...
        lock();
        allocation=efence_memalign(EF_ALIGNMENT, size); 
...
efence_valloc (size_t size)
{
        ...
        lock();
        allocation= efence_memalign(bytesPerPage, size);
        unlock();
...
{
    efence_memalign(alignment, userSize);
}
...
{
    efence_realloc(oldBuffer, newSize);
}
