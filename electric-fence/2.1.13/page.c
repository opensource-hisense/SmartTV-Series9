...
Page_AllowAccess(void * address, size_t size)
{
    ...
    {
        EF_Print("Page_AllowAccess fail : address=%x size=%d", (unsigned int)address, size);
		mprotectFailed();
	}
}
...
{
	if ( mprotect((caddr_t)address, size, PROT_READ) < 0 )
    {
        EF_Print("Page_DenyAccess fail : address=%x size=%d", (unsigned int)address, size);
		mprotectFailed();
	}
}
