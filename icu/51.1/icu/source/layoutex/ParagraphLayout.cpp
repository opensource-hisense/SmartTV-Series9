    ...
    while (currentRun[0] < styleRunArrays[0]->getCount()) {
        ...
        for (style = 0; style < styleCount; style += 1) {
            if (styleRunArrays[style]->getLimit(currentRun[style]) < fRunLimits[run]) {
                if(styleRunArrays[style]->getLimit(currentRun[style]) < 0)
                {
                    continue;
                }
        ...
        for (style = 0; style < styleCount; style += 1) {
            if(runStyle > (maxRunCount * styleCount - 1))
            {
                break;
            }
                                   ...
                                   fFirstVisualRun(-1), fLastVisualRun(-1),*/ fVisualRunLastX(0), fVisualRunLastY(0)
{
    ...
    computeSubFonts(fontRuns, crntFontID, status);
...
ParagraphLayout::Line *ParagraphLayout::nextLine(float width)
{
    ...
    if (width > 0) {
        ...
        while (glyph < fGlyphCount && widthSoFar + fGlyphWidths[glyph] <= width) {
            ...
            i4_idx = fGlyphToCharMap[glyph];            
            if((0x000A == fChars[i4_idx]) ||
                0x000D == fChars[i4_idx])
            {
                break;
            }
            glyph++;
...
void ParagraphLayout::computeSubFonts(const FontRuns *fontRuns, le_int32 crntFontID, LEErrorCode &status)
{
    if (LE_FAILURE(status)) {
        return;
    }
    ...
    const LEFontInstance    *runFontDefault = fontRuns->getFont(crntFontID);
    ...
    if((offset != fCharCount) || (subFontRuns->getFont(0) == NULL))
    {
    	//printf("%s.%d, offset:%d,fCharCount:%d,sub font runs add font:%x\n", __FUNCTION__, __LINE__,offset, fCharCount, runFontDefault);
        subFontRuns->add(runFontDefault, fCharCount);	
    }
...
le_int32 ParagraphLayout::previousBreak(le_int32 charIndex)
{
    ...
    while (charIndex < fCharCount && (fChars[charIndex] != 0x20) &&
...
ParagraphLayout::Line *ParagraphLayout::computeVisualRuns()
{
    ...
    if(U_ZERO_ERROR != bidiStatus)
    {
        printf("=====Qing: EEEEEEEEEEEEEEEERROR! bidiStatus: %d \n", bidiStatus);
    }
...
void ParagraphLayout::appendRun(ParagraphLayout::Line *line, le_int32 run, le_int32 firstChar, le_int32 lastChar)
{
	...
	if (leftGlyph < 0 || leftGlyph > fGlyphCount) {
		leftGlyph = fGlyphCount;
	} 
