...
int32_t trusty_init(unsigned int boot_entry, unsigned int boot_args)
{
    ...
    cm_set_context(mpidr, &ctx->cpu_ctx, SECURE);
    ...
    if (is_caller_secure(flags)) {
        switch(smc_fid){
            ...
            case SMC_SC_MTK_LEGACY_TA_DONE:
                sig = read_ctx_reg(get_sysregs_ctx(cm_get_context(read_mpidr(), NON_SECURE)), CTX_SP_EL1);
                spin_lock(&sigMapLock);
...
static void trusty_cpu_suspend(void)
{
    ...
    if (ret.r0 != 0) {
        printf("%s: cpu %d, SMC_FC_CPU_SUSPEND returned unexpected value, %d\n",
...
static void trusty_cpu_resume(void)
{
    ...
    if (ret.r0 != 0) {
        printf("%s: cpu %d, SMC_FC_CPU_RESUME returned unexpected value, %d\n",
                __func__, linear_id, ret.r0);
    }
}
