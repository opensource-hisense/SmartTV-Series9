...
void tbase_setup_entry_nwd( cpu_context_t *ns_context, uint32_t call_offset ) {
  ...
  // Set up registers
  cpu_context_t *s_context = (cpu_context_t *) cm_get_context(read_mpidr(), SECURE);
  gp_regs_t *s_gpregs = get_gpregs_ctx(s_context);  
...
void print_fastcall_params( char *msg, uint32_t secure ) {
  gp_regs_t *ns_gpregs = get_gpregs_ctx((cpu_context_t *) cm_get_context(read_mpidr(),secure)); 
...
// ************************************************************************************
// Trigger SGIs to all cores, except current
          ...
          void *cookie)
{
  ...
  assert(&tbase_ctx->cpu_ctx == cm_get_context(mpidr, SECURE));
  ...
  if (id == FIQ_SMP_CALL_SGI) {
    ...
    {
        unsigned int iar;
        iar = gicc_read_IAR(mt_get_cfgvar(CONFIG_GICC_ADDR));
