...
static void delete_current(void)
{
    ...
    edit_clear_line();
    memmove(cmdbuf + cmdbuf_pos, cmdbuf + cmdbuf_pos + 1,
           cmdbuf_len - cmdbuf_pos);
    cmdbuf_len--;
    edit_redraw();
}
...
static void delete_word(void)
{
    ...
    while (pos > 0 && cmdbuf[pos - 1] != ' ')
        pos--;
    memmove(cmdbuf + pos, cmdbuf + cmdbuf_pos, cmdbuf_len - cmdbuf_pos);
...
static void clear_left(void)
{
    ...
    edit_clear_line();
    memmove(cmdbuf, cmdbuf + cmdbuf_pos, cmdbuf_len - cmdbuf_pos);
    cmdbuf_len -= cmdbuf_pos;
    cmdbuf_pos = 0;
    edit_redraw();
}
...
static void history_add(const char *str)
{
    ...
    len = strlen(str);
    h = util_zalloc(sizeof(*h) + len);
    if (h == NULL)
        return;
    dl_list_add(&history_list, &h->list);
    util_strlcpy(h->str, str, len + 1);
    history_curr = h;
}
...
{
    edit_clear_line();
    cmdbuf_len = cmdbuf_pos = strlen(history_curr->str);
    memcpy(cmdbuf, history_curr->str, cmdbuf_len);
    edit_redraw();
}
...
static void history_prev(void)
{
        ...
        dl_list_first(&history_list, struct edit_history, list))
    {
        ...
        {
            cmdbuf[cmdbuf_len] = '\0';
            memcpy(currbuf, cmdbuf, cmdbuf_len + 1);
            currbuf_valid = 1;
            history_use();
            return;
        }
...
static void history_next(void)
{
        ...
        dl_list_first(&history_list, struct edit_history, list))
    {
        if (currbuf_valid)
        {
            currbuf_valid = 0;
            edit_clear_line();
            cmdbuf_len = cmdbuf_pos = strlen(currbuf);
            memcpy(cmdbuf, currbuf, cmdbuf_len);
            edit_redraw();
        }
        return;
    }
...
static void complete(int list)
{
    ...
    memmove(cmdbuf + cmdbuf_pos + len + add_space, cmdbuf + cmdbuf_pos,
           cmdbuf_len - cmdbuf_pos);
    memcpy(&cmdbuf[cmdbuf_pos - plen], c[0], plen + len);
    if (add_space)
...
static void search_stop(void)
{
    ...
    if (match)
    {
        util_strlcpy(cmdbuf, match, CMD_BUF_LEN);
...
void edit_redraw(void)
{
    char tmp;
    cmdbuf[cmdbuf_len] = '\0';
    printf("\r%s> %s", CLI_PREFIX, cmdbuf);
    if (cmdbuf_pos != cmdbuf_len)
    {
