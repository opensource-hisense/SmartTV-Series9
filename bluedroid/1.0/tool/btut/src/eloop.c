                                     ...
                                     void *eloop_data, void *user_data)
{
    ...
    tmp = (struct eloop_sock *) realloc(table->table,
                    ...
                    void *eloop_data, void *user_data)
{
    ...
    timeout = util_zalloc(sizeof(*timeout));
    if (timeout == NULL)
        return -1;
    if (util_get_time(&timeout->time) < 0)
    {
        free(timeout);
        return -1;
    }
    ...
    {
        if (util_time_before(&timeout->time, &tmp->time))
        {
...
void eloop_run(void)
{
        ...
        eloop.writers.count > 0 || eloop.exceptions.count > 0))
    {
        ...
        {
            util_get_time(&now);
            if (util_time_before(&now, &timeout->time))
                util_time_sub(&timeout->time, &now, &tv);
            else
                tv.sec = tv.usec = 0;
            _tv.tv_sec = tv.sec;
            _tv.tv_usec = tv.usec;
