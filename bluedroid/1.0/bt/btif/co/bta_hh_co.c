...
#if !defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
#define UHID_EVENT_SIZE_3_10    4108
#define UHID_EVENT_SIZE_3_18    4380
        // 4108 is the length of kernel 3.10 struct uhid_event.,
        // solution only for Android project, because Linux would include correct uhid.h
        if (ret < (ssize_t)sizeof(ev) && ret < UHID_EVENT_SIZE_3_10) {
            APPL_TRACE_ERROR("%s: Invalid size read from uhid-dev: %ld < %lu or %ld",
                         __FUNCTION__, ret, sizeof(ev), UHID_EVENT_SIZE_3_10);
#else
...
#if (defined(MTK_HID_DRIVER_KERNEL_LARGER_THAN_3_10) && (MTK_HID_DRIVER_KERNEL_LARGER_THAN_3_10 == TRUE))
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    /* To fix some HID device can't work. */
    if (p_dev->fd >= 0) {
#else
                             ...
                             int dscp_len, UINT8 *p_dscp)
{
    int result;
    struct uhid_event ev;
#if (defined(MTK_HID_IOT_MOUSE_IME) && (MTK_HID_IOT_MOUSE_IME == TRUE))
    UINT8 temp_buffer_IOT[1024];
#endif
