...
#if defined(MTK_DUN_GW_12) && (MTK_DUN_GW_12 == TRUE)
#define RESERVED_SCN_DUN 25
#endif // MTK_DUN_GW_12 == TRUE
...
#if defined(MTK_DUN_GW_12) && (MTK_DUN_GW_12 == TRUE)
// Registers a service with the given |name| and |channel| in the SDP
// database as a DUN protocol.
static int add_dun_sdp(const char *name, const int channel) {
    UINT16 services[] = {UUID_SERVCLASS_DIALUP_NETWORKING, UUID_SERVCLASS_GENERIC_NETWORKING};

    APPL_TRACE_DEBUG("add_dun_sdp: scn %d, service_name %s", channel, name);

    int handle = SDP_CreateRecord();
    if (handle == 0) {
        APPL_TRACE_ERROR("add_dun_sdp: failed to create sdp record, service_name: %s", name);
        return 0;
    }
    ...
    stage = "service_class";
    if (!SDP_AddServiceClassIdList(handle, sizeof(services)/sizeof(services[0]), services))
        goto error;

    // Add the Dial-up networking profile descriptor
    stage = "profile_descriptor_list";
...
error:
    SDP_DeleteRecord(handle);
    APPL_TRACE_ERROR("add_spp_sdp: failed to register DUN service, stage: %s, service_name: %s", stage, name);
    return 0;
}
#endif //MTK_DUN_GW_12 == TRUE
