...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    BTIF_TRACE_DEBUG("%s: store remote information to config database", __func__);
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    if (p_pin_req->min_16_digit == FALSE)
#else
        ...
        case BTA_DM_DISC_RES_EVT:
        {
            ...
            if (strlen((const char *) p_search_data->disc_res.bd_name))
            {
                ...
                if (cod != 0) {
                    ...
                    BTIF_STORAGE_FILL_PROPERTY(&properties[2], BT_PROPERTY_CLASS_OF_DEVICE, sizeof(UINT32), &cod);
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                BTIF_TRACE_WARNING("%s:Ensure SDP is stopped before attempting service discovery", __FUNCTION__);
                btif_dm_cancel_discovery();
#endif
...
#if defined(MTK_LINUX_GAP) && (MTK_LINUX_GAP == TRUE)
            HAL_CBACK(bt_gap_ex_cbacks, acl_disconnect_reason, &bd_addr,
...
#endif
#if defined(MTK_LINUX_GAP) && (MTK_LINUX_GAP == TRUE)
        case BTA_DM_READ_RSSI_EVT:
        {
            btif_read_rssi_event_t *p_btif_cb = (btif_read_rssi_event_t*) p_param;
            if (NULL == p_btif_cb)
            {
                return;
            }
            HAL_CBACK(bt_gap_ex_cbacks, get_rssi_cb, p_btif_cb->status, &p_btif_cb->bd_addr, p_btif_cb->rssi);
            break;
        }
#endif
...
void btif_dm_proc_loc_oob(BOOLEAN valid, BT_OCTET16 c, BT_OCTET16 r)
{
    FILE *fp;
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    char *path_a = BT_CACHE_PATH"bluedroid/LOCAL/a.key";
    char *path_b = BT_CACHE_PATH"bluedroid/LOCAL/b.key";
#else
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == FALSE)
            BTA_GATTC_Refresh(bd_addr.address);
#endif
#if defined(MTK_LINUX_GAP_PTS_TEST) && (MTK_LINUX_GAP_PTS_TEST == TRUE)
            if (stack_config_get_interface()->get_pts_disable_get_remote_services())
            {
                /*do nothing*/
                BTIF_TRACE_DEBUG("GAP PTS, do not get remote services");
            }
            else
            {
...
/*******************************************************************************
**
** Function         btif_dm_read_rssi_cb
**
** Description      read rssi callback
**
** Returns          bt_status_t
**
*******************************************************************************/
static void btif_dm_read_rssi_cb (tBTM_RSSI_RESULTS *p_result)
{
    btif_read_rssi_event_t btif_cb;

    bdcpy(btif_cb.bd_addr.address, p_result->rem_bda);
    btif_cb.rssi = p_result->rssi;
    btif_cb.status = p_result->status;
    BTIF_TRACE_DEBUG("%s: get rssi cb,rssi is %d", __FUNCTION__,btif_cb.rssi);
    BTIF_TRACE_DEBUG("%s: get rssi cb,addr is %s", __FUNCTION__,btif_cb.bd_addr.address);
    BTIF_TRACE_DEBUG("%s: get rssi cb,status is %s", __FUNCTION__,btif_cb.status);
    btif_transfer_context(btif_dm_upstreams_evt, BTA_DM_READ_RSSI_EVT,
            (char *) &btif_cb, sizeof(btif_read_rssi_event_t), NULL);
}
...
**
** Description      get remote device's rssi
**
** Returns          bt_status_t
**
*******************************************************************************/
bt_status_t btif_dm_get_rssi(const bt_bdaddr_t *bd_addr)
{
    BTIF_TRACE_DEBUG("%s: start get rssi", __FUNCTION__);
    BTM_ReadRSSI((UINT8 *)bd_addr->address, (tBTM_CMPL_CB *)btif_dm_read_rssi_cb);
    return BT_STATUS_SUCCESS;
}
#endif
