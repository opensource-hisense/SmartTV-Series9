...
#define KEYSTATE_FILEPATH BT_CACHE_PATH"bluedroid/bt_hh_ks"
#else
...
#if defined(MTK_LINUX_HID) && (MTK_LINUX_HID == TRUE)
const bthh_ex_callbacks_t *btif_hh_ex_callbacks = NULL;

static bt_status_t btif_hh_init_ex( const bthh_ex_callbacks_t *callbacks )
{
    btif_hh_ex_callbacks = callbacks;

    return BT_STATUS_SUCCESS;
}

static const bthh_ex_interface_t bthhExInterface = {
    sizeof(bthhExInterface),
    btif_hh_init_ex,
};
...
/*******************************************************************************
**
** Function         btif_hh_ex_get_interface
**
** Description      Get the hh external callback interface
**
** Returns          bthh_ex_interface_t
**
*******************************************************************************/
const bthh_ex_interface_t *btif_hh_ex_get_interface()
{
    BTIF_TRACE_EVENT("%s", __FUNCTION__);
    return &bthhExInterface;
}
...
static void btif_hh_upstreams_evt(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTA_HH_DISABLE_EVT:
            btif_hh_cb.status = BTIF_HH_DISABLED;
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
            btif_clean_service_id(BTA_HID_SERVICE_ID);
#endif
            if (p_data->status == BTA_HH_OK) {
                int i;
        ...
        case BTA_HH_GET_DSCP_EVT:
            ...
            }
            {
                ...
                {
#if defined(MTK_LINUX_HID_SET_UHID_NAME) && (MTK_LINUX_HID_SET_UHID_NAME == TRUE)
                    HAL_CBACK(btif_hh_ex_callbacks, get_remote_name_cb, &(p_dev->bd_addr), (char *)bdname.name);
                    cached_name = (char *)bdname.name;
