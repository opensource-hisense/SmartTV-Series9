...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
static UINT32 mtk_btif_hf_features = 0;
#endif
...
#if defined(MTK_STACK_CONFIG_BL) && (MTK_STACK_CONFIG_BL == TRUE)
extern void btif_avrcp_key_state_reset();
#endif
...
************************************************************************************/
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
void bt_hf_mtk_supported_feature()
{
        mtk_btif_hf_features |= (BTIF_HF_FEATURES & ~BTA_AG_FEAT_BTRH);
#if defined(MTK_HFP_eSCO_S4) && (MTK_HFP_eSCO_S4 == TRUE)
        mtk_btif_hf_features |= BTA_AG_FEAT_eSCO_S4;
#endif
#if defined(MTK_HFP_HF_IND) && (MTK_HFP_HF_IND == TRUE)
        mtk_btif_hf_features |= BTA_AG_FEAT_HF_IND;
#endif
#if defined(MTK_HFP_INBAND_RING) && (MTK_HFP_INBAND_RING == TRUE)
       mtk_btif_hf_features |= BTA_AG_FEAT_INBAND;
#endif
}
#endif
                                            ...
                                            const char *number, bthf_call_addrtype_t type)
{
    ...
    if (interop_mtk_match_addr_name(INTEROP_MTK_HFP_DEALY_OPEN_SCO, (const bt_bdaddr_t *)&(btif_hf_cb[idx].connected_bda))) {
    ...
    if (btif_hf_cb[idx].call_setup_state != BTHF_CALL_STATE_IDLE) {
        btif_avrcp_key_state_reset();
    }
#endif

#if defined(MTK_HFP_DUAL_HF) && (MTK_HFP_DUAL_HF == TRUE)
    BTIF_TRACE_DEBUG("phone_state_change: should_open_sco=%d", should_open_sco);
#endif /* MTK_HFP_DUAL_HF */
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
          bt_hf_mtk_supported_feature();
#endif
              for (i = 0; i < btif_max_hf_clients; i++)
              {
                  BTA_AgRegister(BTIF_HF_SERVICES, BTIF_HF_SECURITY,
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                      mtk_btif_hf_features, p_service_names, bthf_hf_id[i]);
#else
