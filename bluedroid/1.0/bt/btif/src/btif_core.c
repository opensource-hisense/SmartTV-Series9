...
#define BTE_DID_CONF_FILE BT_CONF_PATH"bt_did.conf"
#else
...
#if defined(MTK_LINUX_RESET) && (MTK_LINUX_RESET == TRUE)
void hci_status_cback(UINT8 stutus)
{
    BTIF_TRACE_WARNING("hci_status callback");
    if(stutus == BT_HCI_TIMEOUT)
    {
        BTIF_TRACE_WARNING("hci_timeout,need reset BT...");
        HAL_CBACK(bt_hal_cbacks, adapter_state_changed_cb, BT_STATE_HCI_TIMEOUT);
    }
}
#endif
...
#if defined(MTK_LINUX_RESET) && (MTK_LINUX_RESET == TRUE)
        BTIF_TRACE_WARNING("BT is ready, set HCI_State callback");
        sleep(1);
        /*set hci status callback */
        hci = hci_layer_get_interface();
        if (!hci)
           BTIF_TRACE_WARNING("%s could not get hci layer interface.", __func__);

        hci->set_hci_state_callback(hci_status_cback);
...
bt_status_t btif_clean_service_id(tBTA_SERVICE_ID service_id)
{
    btif_enabled_services &=  (tBTA_SERVICE_MASK)(~(1<<service_id));
    BTIF_TRACE_DEBUG("%s: current services:0x%x", __FUNCTION__, btif_enabled_services);
    return BT_STATUS_SUCCESS;
