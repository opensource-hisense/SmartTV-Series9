...
void btif_gatt_check_encrypted_link (BD_ADDR bd_addr, tBTA_GATT_TRANSPORT transport_link)
{
        ...
        && !btif_gatt_is_link_encrypted(bd_addr))
    {
        ...
        tBTM_LE_PENC_KEYS ble_key;
        memcpy(&ble_key, buf, sizeof(tBTM_LE_PENC_KEYS));
        BTIF_TRACE_DEBUG ("%d: transport = %d", __func__, ble_key.sec_level);
        if (ble_key.sec_level == BTM_LE_SEC_UNAUTHENTICATE)
        {
            BTA_DmSetEncryption(bd_addr,transport_link,
                                &btif_gatt_set_encryption_cb, BTM_BLE_SEC_ENCRYPT_NO_MITM);
        }
        else if (ble_key.sec_level == BTM_LE_SEC_AUTHENTICATED)
        {
            BTA_DmSetEncryption(bd_addr,transport_link,
                                &btif_gatt_set_encryption_cb, BTM_BLE_SEC_ENCRYPT_MITM);
        }
#else
        BTA_DmSetEncryption(bd_addr,transport_link,
