...
#endif
#if defined(MTK_STACK_BA2BD_CONVERT) && (MTK_STACK_BA2BD_CONVERT == TRUE)
...
static const char *CONFIG_FILE_PATH = BT_CACHE_PATH"bluedroid/bt_config.conf";
static const char *CONFIG_BACKUP_PATH = BT_CACHE_PATH"bluedroid/bt_config.bak";
static const char *CONFIG_LEGACY_FILE_PATH = BT_CACHE_PATH"bluedroid/bt_config.xml";
...
static future_t *init(void) {
  ...
  if (!config) {
    ...
    if (!config) {
      ...
      LOG_WARN(LOG_TAG, "%s attempting to transcode Blueangel db", __func__);
      config = load_blueangel_cfg();
      if (!config) {
        LOG_WARN(LOG_TAG, "%s unable to transcode Blueangel db, starting unconfigured.", __func__);
        }
      }
  ...
  while (snode != config_section_end(conf)) {
    ...
    if (string_is_bdaddr(section)) {
      if (!config_has_key(conf, section, "LinkKey") &&
        ...
        //workaround for Jawbone APP, should be removed after APP bug fixed
        //The Jawbone app does not call ble scan before connect after BT restart
        //So the address type need to be saved
        if(config_has_key(conf, section, "Name") &&
            (!strcmp(config_get_string(conf, section, "Name", NULL), "UP24") ||
            !strcmp(config_get_string(conf, section, "Name", NULL), "UP2")))
            continue;
 #endif
