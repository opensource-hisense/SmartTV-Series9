...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
void bta_hh_le_deregister_notif(tBTA_HH_DEV_CB *p_dev_cb)
{
    APPL_TRACE_DEBUG("bta_hh_le_deregister_notif");
    BTA_GATTC_DeregisterAllNotifications(bta_hh_cb.gatt_if, p_dev_cb->addr);
}
#endif
...
void bta_hh_le_api_disc_act(tBTA_HH_DEV_CB *p_cb)
{
    if (p_cb->conn_id != BTA_GATT_INVALID_CONN_ID)
    {
        gatt_op_queue_clean(p_cb->conn_id);
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
        bta_hh_le_deregister_notif(p_cb);
#endif
