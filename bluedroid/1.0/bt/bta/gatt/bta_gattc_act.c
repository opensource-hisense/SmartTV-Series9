...
static UINT16 bta_gattc_opcode_to_int_evt[] =
{
    ...
    BTA_GATTC_API_READ_UISNG_CHAR_UUID_EVT,
    BTA_GATTC_API_READ_LONG_EVT,
...
void bta_gattc_set_local_le_name(tBTA_GATTC_DATA * p_msg)
{
    BTM_SetLocalDeviceLeName((char*)p_msg->set_name.name);
}
#endif

#if defined(MTK_LINUX_GATTC_DISC_MODE) && (MTK_LINUX_GATTC_DISC_MODE == TRUE)
void bta_gattc_set_disc_mode(tBTA_GATTC_DATA * p_msg)
{
    BTM_SetLocalDeviceDiscMode((UINT32)p_msg->set_disc_mode.disc_mode);
}
#endif
...
#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
void bta_gattc_read_using_char_uuid(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    if (!bta_gattc_enqueue(p_clcb, p_data))
        return;
    tGATT_READ_PARAM    read_param;
    memset (&read_param, 0, sizeof(tGATT_READ_PARAM));
    read_param.by_handle.auth_req = p_data->api_read_using_char_uuid.auth_req;
    read_param.service.auth_req = p_data->api_read_using_char_uuid.auth_req;
    read_param.service.s_handle = p_data->api_read_using_char_uuid.s_handle;
    read_param.service.e_handle = p_data->api_read_using_char_uuid.e_handle;
    if (NULL == p_data->api_read_using_char_uuid.p_char_uuid)
    {
        APPL_TRACE_ERROR("bta_gattc_read_using_char_uuid failed, uuid is NULL!");
        return;
    }
    memcpy(&(read_param.service.uuid), p_data->api_read_using_char_uuid.p_char_uuid, sizeof(tBT_UUID));
...
void bta_gattc_read_long(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    if (!bta_gattc_enqueue(p_clcb, p_data))
        return;
    tGATT_READ_PARAM    read_param;
    memset (&read_param, 0, sizeof(tGATT_READ_PARAM));
    read_param.by_handle.auth_req = p_data->api_read_long_char.auth_req;
    read_param.by_handle.handle = p_data->api_read_long_char.handle;
    read_param.partial.auth_req = p_data->api_read_long_char.auth_req;
    read_param.partial.handle = p_data->api_read_long_char.handle;
    read_param.partial.offset = p_data->api_read_long_char.offset;

    tBTA_GATT_STATUS status = GATTC_Read(p_clcb->bta_conn_id, GATT_READ_PARTIAL, &read_param);
...
void bta_gattc_set_pts_flag(tBTA_GATTC_DATA * p_msg)
{
    GATTC_Set_Pts_Flag(p_msg->api_set_pts_flag.pts_flag);
}
#endif
/*******************************************************************************
**
...
void bta_gattc_read_cmpl(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_OP_CMPL *p_data)
{
...
#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
        switch (p_data->p_cmpl->op_subtype)
        {
            case GATT_READ_BY_HANDLE:
                cb_data.read.handle = p_clcb->p_q_cmd->api_read.handle;
                break;
            case GATT_READ_MULTIPLE:
                cb_data.read.handle = p_clcb->p_q_cmd->api_read_multi.handles[0];
                break;
            case GATT_READ_PARTIAL:
                cb_data.read.handle = p_clcb->p_q_cmd->api_read_long_char.handle;
                break;
            case GATT_READ_BY_TYPE:
            case GATT_READ_CHAR_VALUE:
                cb_data.read.handle = p_clcb->p_q_cmd->api_read_using_char_uuid.s_handle;
                break;
            default:
                break;
        }
    ...
    switch (p_data->p_cmpl->op_subtype)
    {
        ...
        case GATT_READ_MULTIPLE:
            ...
            break;
        case GATT_READ_PARTIAL:
            event = p_clcb->p_q_cmd->api_read_long_char.cmpl_evt;
            break;
        case GATT_READ_BY_TYPE:
        case GATT_READ_CHAR_VALUE:
            event = p_clcb->p_q_cmd->api_read_using_char_uuid.cmpl_evt;
            break;
        default:
            break;
    }
...
void  bta_gattc_op_cmpl(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    ...
    else if (op >= GATTC_OPTYPE_READ)
    {
...
#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
        if (p_clcb->p_q_cmd->hdr.event >= BTA_GATTC_API_READ_EVT)
        {
            if (p_clcb->p_q_cmd->hdr.event == BTA_GATTC_API_READ_MULTI_EVT
                && p_clcb->p_q_cmd->hdr.event != bta_gattc_opcode_to_int_evt[op + 2 - GATTC_OPTYPE_READ])
            {
                mapped_op = p_clcb->p_q_cmd->hdr.event - BTA_GATTC_API_READ_UISNG_CHAR_UUID_EVT + GATTC_OPTYPE_READ;
            ...
            else if(p_clcb->p_q_cmd->hdr.event != BTA_GATTC_API_READ_MULTI_EVT
                    && p_clcb->p_q_cmd->hdr.event != bta_gattc_opcode_to_int_evt[op + 3 - GATTC_OPTYPE_READ])
            {
                mapped_op = p_clcb->p_q_cmd->hdr.event - BTA_GATTC_API_READ_UISNG_CHAR_UUID_EVT + GATTC_OPTYPE_READ;
                if ( mapped_op > GATTC_OPTYPE_INDICATION)   mapped_op = 0;
        ...
        else if (p_clcb->p_q_cmd->hdr.event < BTA_GATTC_API_READ_EVT && (p_clcb->p_q_cmd->hdr.event != bta_gattc_opcode_to_int_evt[op - GATTC_OPTYPE_READ]
                 && p_clcb->p_q_cmd->hdr.event != bta_gattc_opcode_to_int_evt[op + 1 - GATTC_OPTYPE_READ]))
        {
            mapped_op = p_clcb->p_q_cmd->hdr.event - BTA_GATTC_API_READ_UISNG_CHAR_UUID_EVT + GATTC_OPTYPE_READ;
            if ( mapped_op > GATTC_OPTYPE_INDICATION)   mapped_op = 0;
