...
enum
{
    ...
    ,BTA_GATTC_API_SET_LE_NAME_EVT
#endif
#if defined(MTK_LINUX_GATTC_DISC_MODE) && (MTK_LINUX_GATTC_DISC_MODE == TRUE)
    ,BTA_GATTC_API_SET_DISC_MODE_EVT
#endif
#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
    ,BTA_GATTC_API_SET_PTS_FLAG_EVT
#endif
};
...
{
    BT_HDR              hdr;
    BD_LE_NAME          name; /* max 31 bytes name, plus must be Null terminated */
}tBTA_GATTC_API_SET_LE_NAME;
#endif
...
{
    BT_HDR              hdr;
    UINT32              disc_mode; /* discoverable mode */
}tBTA_GATTC_API_SET_DISC_MODE;
#endif
...
typedef struct
{
    ...
    UINT16                  e_handle;
    tBTA_GATTC_EVT          cmpl_evt;
} tBTA_GATTC_API_READ_USING_CHAR_UUID;
...
typedef struct
{
    ...
    UINT16                  offset;
    tBTA_GATTC_EVT          cmpl_evt;
} tBTA_GATTC_API_READ_LONG_CHAR;

typedef struct
{
    BT_HDR              hdr;
    UINT8               pts_flag;
}tBTA_GATTC_API_SET_PTS_FLAG;
#endif
...
#if defined(MTK_LINUX_GATTC_LE_NAME) && (MTK_LINUX_GATTC_LE_NAME == TRUE)
    tBTA_GATTC_API_SET_LE_NAME set_name;
#endif

#if defined(MTK_LINUX_GATTC_DISC_MODE) && (MTK_LINUX_GATTC_DISC_MODE == TRUE)
    tBTA_GATTC_API_SET_DISC_MODE set_disc_mode;
#endif

#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
    tBTA_GATTC_API_READ_USING_CHAR_UUID api_read_using_char_uuid;
    tBTA_GATTC_API_READ_LONG_CHAR       api_read_long_char;
    tBTA_GATTC_API_SET_PTS_FLAG         api_set_pts_flag;
#endif
} tBTA_GATTC_DATA;
...
#if defined(MTK_LINUX_GATTC_LE_NAME) && (MTK_LINUX_GATTC_LE_NAME == TRUE)
extern void bta_gattc_set_local_le_name(tBTA_GATTC_DATA * p_msg);
#endif

#if defined(MTK_LINUX_GATTC_DISC_MODE) && (MTK_LINUX_GATTC_DISC_MODE == TRUE)
extern void bta_gattc_set_disc_mode(tBTA_GATTC_DATA * p_msg);
#endif

#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
extern void bta_gattc_read_using_char_uuid(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data);
extern void bta_gattc_read_long(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data);
extern void bta_gattc_set_pts_flag(tBTA_GATTC_DATA * p_msg);
#endif
