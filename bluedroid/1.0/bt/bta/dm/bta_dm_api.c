...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    bta_sys_custom_eir_register(bta_dm_custom_eir_update_uuid);
#endif
...
void BTA_BleGetAdvRPA(UINT8 inst_id, tBTA_BLE_GET_ADV_RPA_CBACK *p_cback)
{
    tBTA_DM_API_BLE_GET_ADV_RPA *p_msg =
        (tBTA_DM_API_BLE_GET_ADV_RPA *)osi_calloc(sizeof(tBTA_DM_API_BLE_GET_ADV_RPA));

    APPL_TRACE_API("%s: %d", __func__, inst_id);

    p_msg->hdr.event = BTA_DM_API_BLE_GET_ADV_RPA_EVT;
    p_msg->inst_id = inst_id;
    p_msg->p_cback = p_cback;
    bta_sys_sendmsg(p_msg);
}
...
void BTA_DmSetEIRConfig (tBTA_DM_EIR_CONF *p_eir_cfg)
{
    tBTA_DM_API_SET_EIR_CONFIG *p_msg =
        (tBTA_DM_API_SET_EIR_CONFIG *)osi_calloc(sizeof(tBTA_DM_API_SET_EIR_CONFIG));
