...
#endif
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
    ,UUID_SERVCLASS_3D_DISPLAY
    ,UUID_SERVCLASS_3D_GLASSES
    ,UUID_SERVCLASS_3D_SYNCHRONIZATION
#endif
};
...
#endif
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
    ,BTM_SEC_SERVICE_3D_DISPLAY
    ,BTM_SEC_SERVICE_3D_GLASSES
    ,BTM_SEC_SERVICE_3D_SYNCHRONIZATION
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    APPL_TRACE_DEBUG(" bta_dm_pin_cback() min_16_digit=%d", min_16_digit);
#endif
...
static void bta_dm_set_eir (char *local_name)
{
...
#if defined(MTK_B3DS_LEGACY_B3DS_SHORTEN_EIR) && (MTK_B3DS_LEGACY_B3DS_SHORTEN_EIR == TRUE)
    UINT8   use_legacy_b3ds = bta_b3ds_eir_cfg.bta_dm_eir_manufac_spec_len ? TRUE : FALSE;
#endif
    ...
    )
    {
        ...
        if ( use_legacy_b3ds )
        {
            APPL_TRACE_WARNING("BTA EIR: local name is shortened due to b3ds");
            local_name_len = 10;    // 27 - 4 - 7 - 3 - 2
...
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
    if( p_bta_dm_eir_cfg->bta_dm_eir_3d_info )
    {
        APPL_TRACE_DEBUG("bta_dm_set_eir: p_bta_dm_eir_cfg->bta_dm_eir_3d_info");
        p_length = p;

        UINT8_TO_STREAM(p, 2 + 1);      /* Length field */
        UINT8_TO_STREAM(p, BTM_EIR_3D_INFORMATION_TYPE);
        memcpy(p, p_bta_dm_eir_cfg->bta_dm_eir_3d_info, 2);
        p += 2;
        free_eir_length -= 2 + 2;
#if defined(MTK_LEGACY_B3DS_SUPPORT) && (MTK_LEGACY_B3DS_SUPPORT == TRUE)
        if(free_eir_length < 7)
#else
        if(free_eir_length < 0)
#endif
            APPL_TRACE_ERROR("BTA EIR: B3DS info is truncated");
    }
    else
    {
        p_length = NULL;
    ...
    {
        #if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
        APPL_TRACE_DEBUG("bta_dm_set_eir: p_bta_dm_eir_cfg->bta_dm_eir_manufac_spec");
        #endif
        p_length = p;
...
/*******************************************************************************
**
** Function         bta_dm_custom_eir_update_uuid
**
...
**
*******************************************************************************/
BOOLEAN bta_dm_custom_eir_update_uuid(UINT8 * uuid, UINT16 len, BOOLEAN adding)
{
    UINT16 idx = 0;
    BOOLEAN has_existed = FALSE;
    tBT_UUID* table = bta_dm_cb.custom_uuid;

    APPL_TRACE_DEBUG("%s",__func__);
    /* check whether uuid input is vaild */
    if(uuid == NULL)
        return FALSE;
    ...
    {
        case LEN_UUID_16:
            APPL_TRACE_DEBUG("%s len: %d, content: %02x%02x",__func__,len,
                uuid[1],uuid[0]);
            break;
        case LEN_UUID_32:
            APPL_TRACE_DEBUG("%s len: %d, content: %02x%02x%02x%02x",__func__,len,
                uuid[3],uuid[2],uuid[1],uuid[0]);
            break;
        case LEN_UUID_128:
            APPL_TRACE_DEBUG("%s len: %d, content: %02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
                __func__,len,
                uuid[15],uuid[14],uuid[13],uuid[12],uuid[11],uuid[10],uuid[9],uuid[8],
                uuid[7],uuid[6],uuid[5],uuid[4],uuid[3],uuid[2],uuid[1],uuid[0]);
            break;
        default:
            APPL_TRACE_ERROR("%s UUID length is wrong!",__func__);
            return FALSE;
    }

    /* check whether uuid input already exists in the custom uuid table */
    for(idx = 0; idx < BTA_EIR_SERVER_NUM_CUSTOM_UUID; idx++)
    {
        if(len == table[idx].len && !memcmp(uuid, table[idx].uu.uuid128, len))
        {
            APPL_TRACE_DEBUG("%s UUID exists in the table.",__func__);
            has_existed = TRUE;
            break;
        }
    }
    ...
    if(!adding)
    {
        ...
        {
            APPL_TRACE_DEBUG("%s UUID did not exist, no need to remove",__func__);
            return TRUE;
        }
    }
    /*Add UUID */
    else
    {
        if(has_existed)
        {
            APPL_TRACE_DEBUG("%s UUID has exists, no need to add.",__func__);
            return TRUE;
        }
        else
        {
            ...
            if(idx == BTA_EIR_SERVER_NUM_CUSTOM_UUID)
            {
                APPL_TRACE_WARNING("%s Custom UUID table is full.",__func__);
                return FALSE;
            }
            else
            {
                table[idx].len = len;
                memcpy(table[idx].uu.uuid128, uuid,len);
            }
        }
    }
    bta_dm_set_eir(NULL);
...
void btm_dm_ble_get_adv_rpa(tBTA_DM_MSG *p_data)
{
    tBTM_STATUS btm_status = 0;
    void *p_ref = NULL;
    bt_bdaddr_t rpa;
    memset(&rpa, 0, sizeof(bt_bdaddr_t));
    bta_dm_cb.p_get_adv_rpa_cback = p_data->ble_get_adv_rpa.p_cback;
    if(BTM_BleMaxMultiAdvInstanceCount() > 0 && p_data->ble_get_adv_rpa.inst_id > 0
        && p_data->ble_get_adv_rpa.inst_id < BTM_BleMaxMultiAdvInstanceCount())
    {
        btm_status = BTM_BleGetAdvRPA(p_data->ble_get_adv_rpa.inst_id, &rpa);
    }

    p_ref = btm_ble_multi_adv_get_ref(p_data->ble_multi_adv_disable.inst_id);
    if(BTM_SUCCESS != btm_status)
    {
       bta_dm_cb.p_get_adv_rpa_cback(BTA_BLE_GET_ADV_RPA_EVT,
                                   p_data->ble_get_adv_rpa.inst_id, &rpa, p_ref, BTA_FAILURE);
    }
    else
    {
        bta_dm_cb.p_get_adv_rpa_cback(BTA_BLE_GET_ADV_RPA_EVT,
                                   p_data->ble_get_adv_rpa.inst_id, &rpa, p_ref, BTA_SUCCESS);
    }
}
