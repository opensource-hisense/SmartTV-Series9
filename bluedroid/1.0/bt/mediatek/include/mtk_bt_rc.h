...
/** BT-RC Controller ext callback structure. */
typedef struct {
    /** set to sizeof(btrc_ctrl_ext_callbacks_t) */
    size_t      size;

    btrc_passthrough_cmd_callback                               passthrough_volume_cmd_cb;
} btrc_ctrl_ext_callbacks_t;
...
typedef struct {
    ...
    /** set to sizeof(btrc_ctrl_ex_interface_t) */
    size_t          size;

    /**
     * Register the BtRc ext callbacks
     */
    bt_status_t (*init)( btrc_ctrl_ext_callbacks_t* ext_callbacks );

    /** send get_playstatus_cmd to the target  */
    void  (*send_get_playstatus_cmd)( void );

} btrc_ctrl_ex_interface_t;
