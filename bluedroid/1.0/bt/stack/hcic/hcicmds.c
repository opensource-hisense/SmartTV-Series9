...
BOOLEAN btsnd_hcic_send_hci (UINT8 *buff, UINT8 len)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(sizeof(BT_HDR) + sizeof(void *) + len);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len    = len;
    p->offset = sizeof(void *);

    *((void **)pp) = NULL;  /* Store command complete callback in buffer */
    pp += sizeof(void *);   /* Skip over callback pointer */

    ARRAY_TO_STREAM(pp, buff, len);
    btu_hcif_send_cmd (LOCAL_BR_EDR_CONTROLLER_ID, p);
    return (TRUE);
}
#endif
...
/* up_Synchronous_Connection command  */
/* HFP 1.6 */
BOOLEAN btsnd_enh_hcic_setup_esco_conn (UINT16 handle,
                                           UINT32 tx_bw, UINT32 rx_bw, BtAudioCodingFormat txCoding,
                                           BtAudioCodingFormat rxCoding, UINT16 txCodecFrameSize,
                                           UINT16 rxCodecFrameSize, UINT32 inputBandwith,
                                           UINT32 outputBandwith, BtAudioCodingFormat inputCoding,
                                           BtAudioCodingFormat outputCoding, UINT16 inputCodedDataSize,
                                           UINT16 outputCodedDataSize, UINT8 inputPCMDataFormat,
                                           UINT8 outputPCMDataFormat, UINT8 inputPCMSamplePayloadMSBPos,
                                           UINT8 outputPCMSamplePayloadMSBPos, UINT8 inputDataPath,
                                           UINT8 outputDataPath, UINT8 inputTransportUnitSize,
                                           UINT8 outputTransportUnitSize, UINT16 max_latency,
                                           UINT16 packet_types, UINT8 retrans_effort)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_ENH_SETUP_ESCO;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_ENH_SETUP_ESCO_CONNECTION);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_ENH_SETUP_ESCO);

    UINT16_TO_STREAM (pp, handle);
    UINT32_TO_STREAM (pp, tx_bw);
    UINT32_TO_STREAM (pp, rx_bw);
    UINT8_TO_STREAM  (pp, txCoding.id);
    UINT16_TO_STREAM (pp, txCoding.vendor.comanyId);
    UINT16_TO_STREAM (pp, txCoding.vendor.id);
    UINT8_TO_STREAM  (pp, rxCoding.id);
    UINT16_TO_STREAM (pp, rxCoding.vendor.comanyId);
    UINT16_TO_STREAM (pp, rxCoding.vendor.id);
    UINT16_TO_STREAM (pp, txCodecFrameSize);
    UINT16_TO_STREAM (pp, rxCodecFrameSize);
    UINT32_TO_STREAM (pp, inputBandwith);
    UINT32_TO_STREAM (pp, outputBandwith);
    UINT8_TO_STREAM  (pp, inputCoding.id);
    UINT16_TO_STREAM (pp, inputCoding.vendor.comanyId);
    UINT16_TO_STREAM (pp, inputCoding.vendor.id);
    UINT8_TO_STREAM  (pp, outputCoding.id);
    UINT16_TO_STREAM (pp, outputCoding.vendor.comanyId);
    UINT16_TO_STREAM (pp, outputCoding.vendor.id);
    UINT16_TO_STREAM (pp, inputCodedDataSize);
    UINT16_TO_STREAM (pp, outputCodedDataSize);
    UINT8_TO_STREAM  (pp, inputPCMDataFormat);
    UINT8_TO_STREAM  (pp, outputPCMDataFormat);
    UINT8_TO_STREAM  (pp, inputPCMSamplePayloadMSBPos);
    UINT8_TO_STREAM  (pp, outputPCMSamplePayloadMSBPos);
    UINT8_TO_STREAM  (pp, inputDataPath);
    UINT8_TO_STREAM  (pp, outputDataPath);
    UINT8_TO_STREAM  (pp, inputTransportUnitSize);
    UINT8_TO_STREAM  (pp, outputTransportUnitSize);
    UINT16_TO_STREAM (pp, max_latency);
    UINT16_TO_STREAM (pp, packet_types);
    UINT8_TO_STREAM  (pp, retrans_effort);
...
/* HCI_Enhanced_Accept_Synchronous_Connection command  */
/* HFP 1.6 */
BOOLEAN btsnd_enh_hcic_accept_esco_conn (BD_ADDR bd_addr,
                                           UINT32 tx_bw, UINT32 rx_bw, BtAudioCodingFormat txCoding,
                                           BtAudioCodingFormat rxCoding, UINT16 txCodecFrameSize,
                                           UINT16 rxCodecFrameSize, UINT32 inputBandwith,
                                           UINT32 outputBandwith, BtAudioCodingFormat inputCoding,
                                           ...
                                           UINT16 packet_types, UINT8 retrans_effort)
{
    ...
    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_ENH_ACCEPT_ESCO;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_ENH_ACCEPT_ESCO_CONNECTION);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_ENH_ACCEPT_ESCO);
...
BOOLEAN btsnd_hcic_set_psm_state (BOOLEAN state)
{
    btu_hcif_send_psm_cmd(state);
    return (TRUE);
}
#endif /* MTK_COMMON == TRUE */
...
BOOLEAN btsnd_hcic_write_synchronization_train_parameter(UINT16 interval_min, UINT16 interval_max,
            UINT32 synchronization_train_to, UINT8 service_data)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_WRITE_SYNC_TRAIN_PARAM;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_WRITE_SYNC_TRAIN_PARAM);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_WRITE_SYNC_TRAIN_PARAM);
    UINT16_TO_STREAM (pp, interval_min);
    UINT16_TO_STREAM (pp, interval_max);
    UINT32_TO_STREAM (pp, synchronization_train_to);
...
BOOLEAN btsnd_hcic_start_synchronization_train(void)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);
...
BOOLEAN btsnd_hcic_set_triggered_clock_capture(UINT16 handle, UINT8 enable,
            UINT8 which_clock, UINT8 lpo_allowed, UINT8 num_clock_captures_to_filter)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_SET_TRIGGERED_CLOCK_CAPTURE;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_SET_TRIGGERED_CLOCK_CAPTURE);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_SIZE_SET_TRIGGERED_CLOCK_CAPTURE);
    UINT16_TO_STREAM (pp, handle);
    UINT8_TO_STREAM (pp, enable);
    UINT8_TO_STREAM (pp, which_clock);
    UINT8_TO_STREAM (pp, lpo_allowed);
    UINT8_TO_STREAM (pp, num_clock_captures_to_filter);
...
BOOLEAN btsnd_hcic_set_connectionless_slave_broadcast(UINT8 enable, UINT8 lt_addr,
            UINT8 lpo_allowed, UINT16 packet_type, UINT16 interval_min, UINT16 interval_max, UINT16 csb_supervision_to)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);
    ...
    UINT16_TO_STREAM (pp, interval_min);
    UINT16_TO_STREAM (pp, interval_max);
    UINT16_TO_STREAM (pp, csb_supervision_to);

    btu_hcif_send_cmd (LOCAL_BR_EDR_CONTROLLER_ID,  p);
    return (TRUE);
}

BOOLEAN btsnd_hcic_set_connectionless_slave_broadcast_data(UINT8 lt_addr, UINT8 fragment, UINT8 data_length, UINT8 *p_data)
{
    BT_HDR *p = (BT_HDR *)osi_malloc(HCI_CMD_BUF_SIZE);
    UINT8 *pp = (UINT8 *)(p + 1);

    p->len    = HCIC_PREAMBLE_SIZE + HCIC_PARAM_PARTIAL_SIZE_SET_CLB_DATA + data_length;
    p->offset = 0;

    UINT16_TO_STREAM (pp, HCI_WRITE_CLB_DATA);
    UINT8_TO_STREAM  (pp, HCIC_PARAM_PARTIAL_SIZE_SET_CLB_DATA + data_length);
    UINT8_TO_STREAM (pp, lt_addr);
    UINT8_TO_STREAM (pp, fragment);
    UINT8_TO_STREAM (pp, data_length);
    ARRAY_TO_STREAM  (pp, p_data, data_length);
