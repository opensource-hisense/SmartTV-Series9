...
static void btu_ble_proc_enhanced_conn_cmpl (UINT8 *p, UINT16 evt_len);
#endif
#endif /* (defined(MTK_COMMON) && MTK_COMMON == TRUE) */

    #endif

#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
static void btu_hcif_triggered_clock_capture_evt(UINT8 *p);
static void btu_hcif_sync_train_comp_evt(UINT8 *p);
static void btu_hcif_start_sync_train_status_evt(UINT8 status);
static void btu_hcif_csb_timeout_evt(UINT8 *p);
static void btu_hcif_inquiry_response_notify_evt(UINT8 *p);
static void btu_hcif_csb_channel_map_change_evt(UINT8 *p);
static void btu_hcif_slave_page_response_timeout_evt(void);
#if defined(MTK_LEGACY_B3DS_SUPPORT) && (MTK_LEGACY_B3DS_SUPPORT == TRUE)
static void btu_hcif_3d_ref_proto_assoc_notify_evt(UINT8 *p);
#endif
#endif
...
void btu_hcif_process_event (UNUSED_ATTR UINT8 controller_id, BT_HDR *p_msg)
{
    ...
    switch (hci_evt_code)
    {
        ...
        case HCI_SET_TRIGGERED_CLOCK_CAPTURE_EVT:
            btu_hcif_triggered_clock_capture_evt(p);
            break;
        case HCI_SYNC_TRAIN_COMP_EVT:
            btu_hcif_sync_train_comp_evt(p);
            break;
        case HCI_CSB_TIMEOUT_EVT:
            btu_hcif_csb_timeout_evt(p);
            break;
        case HCI_INQUIRY_RESPONSE_NOTIFY_EVT:
            btu_hcif_inquiry_response_notify_evt(p);
            break;
        case HCI_CSB_CHANNEL_MAP_CHANGE_EVT:
            btu_hcif_csb_channel_map_change_evt(p);
            break;
        case HCI_SLAVE_PAGE_RESPONSE_TIMEOUT_EVT:
            btu_hcif_slave_page_response_timeout_evt();
            break;
#if defined(MTK_LEGACY_B3DS_SUPPORT) && (MTK_LEGACY_B3DS_SUPPORT == TRUE)
        case HCI_3D_REF_PROTO_ASSOC_NOTIFY:
            btu_hcif_3d_ref_proto_assoc_notify_evt(p);
            break;
#endif
#endif
...
#if defined(MTK_COMMON) && (MTK_COMMON == TRUE)
void btu_hcif_send_psm_cmd(BOOLEAN state)
{
    int val;
    ...
    {
        val = 0;
        hci_layer_get_interface()->send_psm_command(val);
    }
    else
    {
        val = 1;
    ...
    HCI_TRACE_DEBUG("send psm command successfully");
}
#endif
...
/* BLE Commands sComplete*/
        ...
        case HCI_SET_RESERVED_LT_ADDR:
            ...
            break;
        case HCI_SET_CLB:
            btm_set_connectionless_slave_broadcast_complete(p);
            break;
        case HCI_WRITE_CLB_DATA:
            btm_set_connectionless_slave_broadcast_data_complete(p);
            break;
        case HCI_WRITE_SYNC_TRAIN_PARAM:
            btm_write_synchronization_train_parameter_complete(p);
            break;
#endif
        default:
            if ((opcode & HCI_GRP_VENDOR_SPECIFIC) == HCI_GRP_VENDOR_SPECIFIC)
                                         ...
                                         void *p_vsc_status_cback)
{
    ...
    switch (opcode)
    {
        ...
        case HCI_START_SYNC_TRAIN:
            btu_hcif_start_sync_train_status_evt(status);
            break;
#endif
        default:
...
static void btu_hcif_triggered_clock_capture_evt(UINT8 *p)
{
    tBTM_CMPL_CB   *p_cb = btm_cb.devcb.p_tcc_evt_cb;
    HCI_TRACE_DEBUG("btu_hcif_triggered_clock_capture_evt");
...
static void btu_hcif_sync_train_comp_evt(UINT8 *p)
{
    tBTM_CMPL_CB   *p_cb = btm_cb.devcb.p_st_cmpl_cb;
    HCI_TRACE_DEBUG("btu_hcif_sync_train_comp_evt");

    btm_cb.devcb.p_st_cmpl_cb = NULL;
...
static void btu_hcif_start_sync_train_status_evt(UINT8 status)
{
    tBTM_CMPL_CB   *p_cb = btm_cb.devcb.p_sst_status_cb;
    HCI_TRACE_DEBUG("btu_hcif_start_sync_train_status_evt");

    btm_cb.devcb.p_sst_status_cb = NULL;
...
static void btu_hcif_csb_timeout_evt(UINT8 *p)
{
    tBTM_CMPL_CB   *p_cb = btm_cb.devcb.p_csbt_evt_cb;
    HCI_TRACE_DEBUG("btu_hcif_csb_timeout_evt");
...
static void btu_hcif_inquiry_response_notify_evt(UINT8 *p)
{
    HCI_TRACE_DEBUG("btu_hcif_inquiry_response_notify_evt");
//#if defined(B3DS_INCLUDED) && (B3DS_INCLUDED == TRUE)
    // b3ds_proc_inquiry_response_notify_evt();
//#endif
}

static void btu_hcif_csb_channel_map_change_evt(UINT8 *p)
{
    tBTM_CMPL_CB   *p_cb = btm_cb.devcb.p_csbcmc_evt_cb;
    HCI_TRACE_DEBUG("btu_hcif_csb_channel_map_change_evt");
...
static void btu_hcif_slave_page_response_timeout_evt(void)
{
    tBTM_CMPL_CB   *p_cb = btm_cb.devcb.p_sprt_evt_cb;
    HCI_TRACE_DEBUG("btu_hcif_slave_page_response_timeout_evt");
...
static void btu_hcif_3d_ref_proto_assoc_notify_evt(UINT8 *p)
{
    HCI_TRACE_DEBUG("btu_hcif_legacy_assoc_notify_evt, addr:0x%02X,0x%02X,0x%02X,0x%02X,0x%02X,0x%02X", p[0], p[1], p[2], p[3], p[4], p[5]);
    B3DS_ProcRefProtoAssocNotifyEvt(p);
}
#endif
#endif
