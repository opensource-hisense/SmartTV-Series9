...
void avdt_ccb_hdl_getcap_cmd(tAVDT_CCB *p_ccb, tAVDT_CCB_EVT *p_data)
{
    ...
    if (interop_mtk_match_addr_name(INTEROP_MTK_A2DP_NOT_SET_SCMT, (const bt_bdaddr_t *)&p_ccb->peer_addr))
    {
        p_data->msg.svccap.p_cfg->num_protect = 0;
        AVDT_TRACE_DEBUG("avdt_ccb_hdl_getcap_cmd: IOT device, re-set protect number to be zero on rsp msg");
    }
#endif
