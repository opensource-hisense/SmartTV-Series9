...
/*    BTM_EIR_UUID_SERVCLASS_HDP_PROFILE                */
    ...
    BTM_EIR_UUID_SERVCLASS_3D_DISPLAY,
    BTM_EIR_UUID_SERVCLASS_3D_GLASSES,
    BTM_EIR_UUID_SERVCLASS_3D_SYNCHRONIZATION,
#endif
...
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
#define BTM_EIR_3D_INFORMATION_TYPE         HCI_EIR_3D_INFORMATION_TYPE         /* 0x3D */ /* B3DS_INCLUDED */
#endif
...
typedef struct
{
    ...
    UINT16 txCodecFrameSize;
    UINT16 rxCodecFrameSize;
    UINT32 inputBandwith;
    UINT32 outputBandwith;
    BtAudioCodingFormat inputCoding;
    BtAudioCodingFormat outputCoding;
    UINT16 inputCodedDataSize;
    UINT16 outputCodedDataSize;
    UINT8 inputPCMDataFormat;
    UINT8 outputPCMDataFormat;
    UINT8 inputPCMSamplePayloadMSBPos;
    UINT8 outputPCMSamplePayloadMSBPos;
    UINT8 inputDataPath;
    UINT8 outputDataPath;
    UINT8 inputTransportUnitSize;
    UINT8 outputTransportUnitSize;
    UINT16 max_latency;
    UINT16 packet_types;
    UINT8 retrans_effort;
}tBTM_ENH_ESCO_PARAMS;
#endif
...
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
#define BTM_SEC_SERVICE_3D_DISPLAY      51 // B3DS_INCLUDED
#define BTM_SEC_SERVICE_3D_GLASSES      52
#define BTM_SEC_SERVICE_3D_SYNCHRONIZATION  53

#undef BTM_SEC_SERVICE_FIRST_EMPTY
...
extern tBTM_STATUS BTM_SetLocalDeviceLeName (char *p_name);
extern tBTM_STATUS BTM_ReadLocalDeviceLeName (char **p_name);
#endif
...
/*******************************************************************************
**
**Function       BTM_SetPSM_State
**
**
**Description     This function is controller to up and down the PSM

**                 mode which design by MTK
**
**Returns      BTM_SUCCESS if already in specified state.
**                BTM_MODE_UNSUPPORTED if local device does not support PSM Mode
*******************************************************************************/
extern tBTM_STATUS BTM_SetPSM_State (BOOLEAN new_state);
#endif
...
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
extern tBTM_STATUS BTM_WriteSyncTrainPara(UINT16 interval_min, UINT16 interval_max,
    UINT32 synchronization_train_to, UINT8 service_data, tBTM_CMPL_CB *p_cb);
extern tBTM_STATUS BTM_StartSyncTrain(tBTM_CMPL_CB *p_status_cb, tBTM_CMPL_CB *p_cmpl_cb);
extern tBTM_STATUS BTM_SetTriggeredClockCapture (UINT16 handle, UINT8 enable, UINT8 which_clock,
    UINT8 lpo_allowed, UINT8 num_clock_captures_to_filter);
extern tBTM_STATUS BTM_SetReservedLtAddr (UINT8 lt_addr);
extern tBTM_STATUS BTM_SetCsb(UINT8 enable, UINT8 lt_addr, UINT8 lpo_allowed, UINT16 packet_type,
    UINT16 interval_min, UINT16 interval_max, UINT16 csb_supervision_to, tBTM_CMPL_CB *p_cb);
extern tBTM_STATUS BTM_SetCsbData(UINT8 lt_addr, UINT8 fragment, UINT8 data_length, UINT8 *p_data);
extern tBTM_CMPL_CB *BTM_RegisterForTriggeredClockCaptureEvt(tBTM_CMPL_CB *p_cb);
extern tBTM_CMPL_CB *BTM_RegisterForCsbTimeoutEvt(tBTM_CMPL_CB *p_cb);
extern tBTM_CMPL_CB *BTM_RegisterForCsbChannelMapChangeEvt(tBTM_CMPL_CB *p_cb);
extern tBTM_CMPL_CB *BTM_RegisterForSlavePageResponseTimeoutEvt(tBTM_CMPL_CB *p_cb);
#endif
