...
{
#if defined(MTK_LINUX_GAP_PTS_TEST) && (MTK_LINUX_GAP_PTS_TEST == TRUE)
    if (stack_config_get_interface()->get_pts_disable_discover())
    {
        GATT_TRACE_DEBUG("For GAP PTS, disable GATT discover operation");
        return;
    }
#endif
...
#if defined(MTK_FORCE_HANDLE_BLE_DATA_LENGTH_EXTENSION_PACKET) && (MTK_FORCE_HANDLE_BLE_DATA_LENGTH_EXTENSION_PACKET == TRUE)
