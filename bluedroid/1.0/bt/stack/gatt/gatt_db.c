                                                ...
                                                UINT16 handle, UINT16 offset, UINT32 trans_id)
{
    ...
    if ((BT_TRANSPORT_LE == p_tcb->transport && GATT_TRANSPORT_BR_EDR == p_sreg->transport)
         || (BT_TRANSPORT_BR_EDR == p_tcb->transport && GATT_TRANSPORT_LE == p_sreg->transport))
    {
        GATT_TRACE_ERROR("%s invalid transport!\n", __func__);
        return (tGATT_STATUS) GATT_NO_RESOURCES;
    }
#endif
    if (trans_id == 0)
    {
