                                 ...
                                 tGATT_TRANSPORT sup_transport)
{
    ...
    p_sreg->transport = sup_transport;
#endif
...
#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
void GATTC_Set_Pts_Flag(UINT8 pts_flag)
{
    gattc_pts_flag = pts_flag;
    GATT_TRACE_API ("GATTC_Set_Pts_Flag gattc_pts_flag =%d", gattc_pts_flag);
}
#endif
...
void GATT_SetIdleTimeout (BD_ADDR bd_addr, UINT16 idle_tout, tBT_TRANSPORT transport)
{
    ...
    if ((p_tcb = gatt_find_tcb_by_addr (bd_addr, transport)) != NULL)
    {
        ...
        {
#if defined(MTK_LINUX_GATTC_PTS_TEST) && (MTK_LINUX_GATTC_PTS_TEST == TRUE)
            L2CA_DisconnectReq (p_tcb->att_lcid);
#endif
