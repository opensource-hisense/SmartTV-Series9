                                   ...
                                   BOOLEAN check_acl_link)
{
    ...
    if (is_add) {
        ...
        else
        {
            ...
            {
#if defined(MTK_LINUX_GAP_PTS_TEST) && (MTK_LINUX_GAP_PTS_TEST == TRUE)
                if (stack_config_get_interface()->get_pts_extend_idle_timeout() && (gatt_if < 5))
                {
                    /*set idle timeout to 30s for GAP PTS*/
                    GATT_TRACE_DEBUG("GATT starts link idle timer =%d sec", GATT_LINK_EXTEND_IDLE_TIMEOUT_WHEN_NO_APP);
                    GATT_TRACE_DEBUG("GAP PTS extend idle timeout from 1s to 30s");
                    GATT_SetIdleTimeout(p_tcb->peer_bda, GATT_LINK_EXTEND_IDLE_TIMEOUT_WHEN_NO_APP, p_tcb->transport);
                }
                else
                {
