...
#if defined(MTK_AVRCP_TG_15) && defined(MTK_STACK_CONFIG_BL)
#if (MTK_AVRCP_TG_15 == TRUE && MTK_STACK_CONFIG_BL == TRUE)
/*******************************************************************************
**
** Function         sdp_avrcp_blacklist_check
**
** Description      This function check if
**                       1. The device address matches the avrcp 1.5 blacklist
**                       2. The attribute id is profile desciptor
**                       3. The profile uuid is AVRCP
**
** Returns          BOOLEAN; if true, change avrcp version to be 1.3
**
*******************************************************************************/
void sdp_avrcp_blacklist_check(tSDP_ATTRIBUTE *p_attr, UINT8 *p_attr_start, BD_ADDR addr)
{
    SDP_TRACE_EVENT("sdp_avrcp_blacklist_check");

    if ((p_attr->id == ATTR_ID_BT_PROFILE_DESC_LIST) && (p_attr->type == DATA_ELE_SEQ_DESC_TYPE) &&
        (p_attr_start[MTK_AVRCP_VERSION_SDP_POSITION] ==( (UUID_DESC_TYPE << 3) | SIZE_TWO_BYTES)) &&
        ((p_attr_start[MTK_AVRCP_VERSION_SDP_POSITION+1] << 8 | p_attr_start[MTK_AVRCP_VERSION_SDP_POSITION+2]) == UUID_SERVCLASS_AV_REMOTE_CONTROL)) {

        if (interop_mtk_match_addr_name(INTEROP_MTK_AVRCP_15_TO_13, (const bt_bdaddr_t *)&addr)){
            SDP_TRACE_WARNING("SDP - AVRCP 1.5 IOT device, fallback the version to 1.3");
            p_attr_start[MTK_AVRCP_VERSION_SDP_POSITION+5] = MTK_AVRCP_13_VERSION;
        }
        else if (interop_mtk_match_addr_name(INTEROP_MTK_AVRCP_15_TO_14, (const bt_bdaddr_t *)&addr)){
            SDP_TRACE_WARNING("SDP - AVRCP 1.5 IOT device, fallback the version to 1.4");
            p_attr_start[MTK_AVRCP_VERSION_SDP_POSITION+5] = MTK_AVRCP_14_VERSION;
        }
    }

    return;
}
#endif /* if (MTK_AVRCP_TG_15 == TRUE && MTK_STACK_CONFIG_BL == TRUE) */
#endif
...
#if defined(MTK_AVRCP_TG_15) && defined(MTK_STACK_CONFIG_BL)
#if (MTK_AVRCP_TG_15 == TRUE && MTK_STACK_CONFIG_BL == TRUE)
            if ((UINT8)(p_rsp - p_attr_start) >= (MTK_AVRCP_VERSION_SDP_POSITION+5))
            {
                sdp_avrcp_blacklist_check(p_attr, p_attr_start, p_ccb->device_address);
            }
#endif
#endif
    ...
    for (p_rec = sdp_db_service_search (p_ccb->cont_info.prev_sdp_rec, &uid_seq); p_rec; p_rec = sdp_db_service_search (p_rec, &uid_seq))
    {
        ...
        for (xx = p_ccb->cont_info.next_attr_index; xx < attr_seq.num_attr; xx++)
        {
            ...
            if (p_attr)
            {
                ...
                {
                    if ((p_attr_start[6] << 8 | p_attr_start[7]) == UUID_SERVCLASS_AV_REM_CTRL_TARGET)
                    {
                        isAVCTP_TG = TRUE;
                    }
                }
                if (p_attr->id == ATTR_ID_SUPPORTED_FEATURES && isAVCTP_TG)
                {

                    if (interop_mtk_match_addr_name(INTEROP_MTK_AVRCP_15_TO_13, (const bt_bdaddr_t *)&p_ccb->device_address)
                            || interop_mtk_match_addr_name(INTEROP_MTK_AVRCP_15_TO_14, (const bt_bdaddr_t *)&p_ccb->device_address))
                    {
                        SDP_TRACE_WARNING("Reset value for AVRCP 1.3");
                        p_attr_start[4] = 0x00;
                        p_attr_start[5] = p_attr_start[5] & 0x1F;
                    }
                }
                if ((UINT8)(p_rsp - p_attr_start) >= (MTK_AVRCP_VERSION_SDP_POSITION+5))
                {
