...
BOOLEAN l2c_ucd_check_pending_in_sec_q(tL2C_CCB  *p_ccb)
{
    ...
    if (p_buf != NULL)
    {
        ...
        if (psm == BT_PSM_B3DS)
        {
            L2CAP_TRACE_DEBUG("l2c_ucd_check_pending_in_sec_q, enqueue the l2cap data");
            fixed_queue_enqueue(p_ccb->p_lcb->ucd_in_sec_pending_q, (void*)p_buf);
        }
#endif
        btm_sec_l2cap_access_req (p_ccb->p_lcb->remote_bd_addr, psm,
