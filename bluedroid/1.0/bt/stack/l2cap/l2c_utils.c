...
*******************************************************************************/
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
void l2cu_release_ccb (tL2C_CCB *p_ccb, UINT8 timeout_flag)
#else
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
    if (0 == timeout_flag) //0: not timeout   1:timeout
    {
        /* Free the timer */
        alarm_free(p_ccb->l2c_ccb_timer);
    ...
    for (int xx = 0; xx < L2CAP_NUM_FIXED_CHNLS; xx++)
    {
        ...
        if (l2cb.fixed_reg[xx].pL2CA_FixedConn_Cb != NULL)
        {
            ...
            else
            {
                ...
                {
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                    l2cu_release_ccb(p_lcb->p_fixed_ccbs[xx], 0);
#else
    ...
    for (int xx = 0; xx < L2CAP_NUM_FIXED_CHNLS; xx++)
    {
        if (p_lcb->p_fixed_ccbs[xx])
        {
            if (p_lcb->p_fixed_ccbs[xx] != p_lcb->p_pending_ccb)
            {
                ...
                p_lcb->p_fixed_ccbs[xx] = NULL;
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
                l2cu_release_ccb(p_l2c_chnl_ctrl_block, 0);
#else
