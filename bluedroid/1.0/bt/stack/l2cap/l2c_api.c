...
**
*******************************************************************************/
BOOLEAN L2CA_IsFixedChnlPending (UINT16 fixed_cid, BD_ADDR rem_bda)
{
    tL2C_LCB *p_lcb;
    tBT_TRANSPORT transport = BT_TRANSPORT_BR_EDR;
    ...
    if (((p_lcb = l2cu_find_lcb_by_bd_addr (rem_bda, transport)) != NULL) && (p_lcb->p_pending_ccb != NULL))
    {
        if(p_lcb->p_pending_ccb == p_lcb->p_fixed_ccbs[fixed_cid - L2CAP_FIRST_FIXED_CHNL])
        {
            L2CAP_TRACE_API  ("%s() CID: 0x%04x  BDA: %08x%04x, the fixed channel connection is pending", __func__, fixed_cid,
                  (rem_bda[0]<<24)+(rem_bda[1]<<16)+(rem_bda[2]<<8)+rem_bda[3], (rem_bda[4]<<8)+rem_bda[5]);
            return (TRUE);
        }
    }
