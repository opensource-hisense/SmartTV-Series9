...
BOOLEAN L2CA_EnableUpdateBleConnParams (BD_ADDR rem_bda, BOOLEAN enable)
{
    ...
    {
        if (stack_config_get_interface()->get_pts_disable_ble_connection_update())
        {
            p_lcb->conn_update_mask &= ~L2C_BLE_CONN_UPDATE_DISABLE;
        }
        else
...
static void l2cble_start_conn_update (tL2C_LCB *p_lcb)
{
...
#if (defined(MTK_BLE_CONN_MIN_CE_LEN) && defined(MTK_BLE_CONN_MAX_CE_LEN))
                                                  slave_latency, supervision_tout, MTK_BLE_CONN_MIN_CE_LEN, MTK_BLE_CONN_MAX_CE_LEN);
#else
    ...
    else
    {
        ...
        {
#if defined(MTK_STACK_CONFIG_BL) && (MTK_STACK_CONFIG_BL == TRUE)
            if(interop_mtk_match_addr_name(INTEROP_MTK_LE_CONN_TIMEOUT_ADJUST, (const bt_bdaddr_t *)&p_lcb->remote_bd_addr))
            {
                p_lcb->timeout = BTM_BLE_CONN_TIMEOUT_DEF;
                L2CAP_TRACE_ERROR("change the LE supervision timeout as default : %d", p_lcb->timeout);
            }
#endif
             /* if both side 4.1, or we are master device, send HCI command */
            if (p_lcb->link_role == HCI_ROLE_MASTER
...
#if (defined(MTK_BLE_CONN_MIN_CE_LEN) && defined(MTK_BLE_CONN_MAX_CE_LEN))
                    p_lcb->max_interval, p_lcb->latency, p_lcb->timeout, MTK_BLE_CONN_MIN_CE_LEN, MTK_BLE_CONN_MAX_CE_LEN);
#else
        ...
        case L2CAP_CMD_BLE_UPDATE_REQ:
            ...
            if (p_lcb->link_role == HCI_ROLE_MASTER)
            {
                ...
                if (min_interval < BTM_BLE_CONN_INT_MIN_LIMIT
                    && !interop_mtk_match_addr_name(INTEROP_MTK_LE_CONN_INT_MIN_LIMIT_ACCEPT, (const bt_bdaddr_t *)&p_lcb->remote_bd_addr))
#else
                if (min_interval < BTM_BLE_CONN_INT_MIN_LIMIT)
#endif
                ...
                {
#if defined(MTK_STACK_CONFIG_BL) & (MTK_STACK_CONFIG_BL == TRUE)
                    if(interop_mtk_match_addr_name(INTEROP_MTK_HOGP_CONN_INT_UPDATE_ADJUST, (const bt_bdaddr_t *)&p_lcb->remote_bd_addr))
                    {
                        l2cu_send_peer_ble_par_rsp (p_lcb, L2CAP_CFG_UNACCEPTABLE_PARAMS, id);
                        p_lcb->min_interval = p_lcb->max_interval = BTM_BLE_CONN_INT_MIN_LIMIT;
                        break;
                    }
#endif
...
#if (defined(MTK_BLE_CONN_MIN_CE_LEN) && defined(MTK_BLE_CONN_MAX_CE_LEN))
                                        MTK_BLE_CONN_MIN_CE_LEN, /* UINT16 min_len       */
                                        MTK_BLE_CONN_MAX_CE_LEN))/* UINT16 max_len       */
#else
                                        0,                       /* UINT16 min_len       */
                                        0))                      /* UINT16 max_len       */
#endif
