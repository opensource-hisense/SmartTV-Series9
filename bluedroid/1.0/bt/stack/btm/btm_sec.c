...
tBTM_SEC_SERV_REC *btm_sec_find_first_serv (CONNECTION_TYPE is_originator, UINT16 psm);
#else   // MTK_B3DS_SUPPORT
...
void btm_sec_encrypt_change (UINT16 handle, UINT8 status, UINT8 encr_enable)
{
    ...
    BTM_TRACE_DEBUG ("handle= %d, p_dev_rec->hci_handle = %d", handle,p_dev_rec->hci_handle);
#endif
...
#if defined(MTK_LINUX) && defined(MTK_COMMON) && (MTK_COMMON == TRUE)
            /*when in the pairing process, the peer device disconnect(eg. power off),
            then will free p_dev_sec within p_auth_complete_callback. so here get p_dev_res again.*/
            p_dev_rec = btm_find_dev_by_handle (handle);
            if (!p_dev_rec)
                return;
#endif
