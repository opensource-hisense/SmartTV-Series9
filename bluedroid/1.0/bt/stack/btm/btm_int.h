...
typedef char tBTM_LOC_BD_LE_NAME[MTK_LINUX_GATT_MAX_LE_NAME_LEN + 1];
#endif
#endif
...
BT_OCTET16 ble_encryption_key_value; /* BLE encryption key */
    ...
    tBTM_CMPL_CB        *p_sst_status_cb;    /* BTM_StartSyncTrain function status         */
    tBTM_CMPL_CB        *p_st_cmpl_cb;       /* Sync Train Complete        */

    alarm_t             *wstp_timer;
    tBTM_CMPL_CB        *p_wstp_cmpl_cb;    /* Callback function to be called when  */
                                            /* BTM_WriteSyncTrainPara function completes         */
    alarm_t             *scsb_timer;
    tBTM_CMPL_CB        *p_scsb_cmpl_cb;    /* Callback function to be called when  */
                                            /* BTM_SetCsb function completes         */
    alarm_t             *scsbd_timer;
    tBTM_CMPL_CB        *p_scsbd_cmpl_cb;    /* Callback function to be called when  */
                                            /* BTM_SetCsbData function completes         */

    tBTM_CMPL_CB        *p_tcc_evt_cb;    /* Callback function to be called when  */
                                            /* BTM_RegisterForTriggeredClockCaptureEvt         */

    tBTM_CMPL_CB        *p_csbt_evt_cb;    /* Callback function to be called when  */
                                            /* BTM_RegisterForCsbTimeoutEvt         */

    tBTM_CMPL_CB        *p_csbcmc_evt_cb;    /* Callback function to be called when  */
                                            /* BTM_RegisterForCsbChannelMapChangeEvt         */

    tBTM_CMPL_CB        *p_sprt_evt_cb;    /* Callback function to be called when  */
                                            /* BTM_RegisterForSlavePageResponseTimeoutEvt         */
#endif
...
#define BTM_SEC_NO_LAST_SERVICE_ID      0
    ...
    UINT8 sec_status;                               /*status for pin_or_key_missing*/
#endif
#endif
...
#if defined(MTK_LINUX_GATT_MAX_LE_NAME_LEN) && (MTK_LINUX_GATT_MAX_LE_NAME_LEN > 0)
    UINT8            le_name_len;
    tBTM_LOC_BD_LE_NAME bd_le_name;                    /* local Bluetooth device le name */
#endif
#endif
...
extern void btm_set_connectionless_slave_broadcast_complete (UINT8 *p);
extern void btm_set_connectionless_slave_broadcast_data_complete (UINT8 *p);
extern void btm_write_synchronization_train_parameter_complete (UINT8 *p);
#endif
