...
void BTM_ReadDevInfo (BD_ADDR remote_bda, tBT_DEVICE_TYPE *p_dev_type, tBLE_ADDR_TYPE *p_addr_type)
{
    ...
    *p_addr_type = BLE_ADDR_PUBLIC;
    ...
    else /* there is a security device record exisitng */
    {
        ...
        /* new inquiry result, but ble part has been format in security device record after unpair */
        if (!memcmp(p_dev_rec->ble.pseudo_addr, bd_addr_null, BD_ADDR_LEN))
        {
            *p_dev_type = p_dev_rec->device_type;
            *p_addr_type = p_dev_rec->ble.ble_addr_type;
        }
...
tBTM_STATUS btm_ble_set_encryption (BD_ADDR bd_addr, tBTM_BLE_SEC_ACT sec_act, UINT8 link_role)
{
    ...
    switch (sec_act)
    {
        case BTM_BLE_SEC_ENCRYPT:
            ...
            {
#if defined(MTK_LINUX_GAP_PTS_TEST) && (MTK_LINUX_GAP_PTS_TEST == TRUE)
                if (stack_config_get_interface()->get_pts_passive_encrypt())
                {
                    /* do nothing */
                    BTM_TRACE_DEBUG ("GAP PTS, DUT as master do not start encrypt active")
                }
                else
                {
...
#if defined(MTK_LINUX_GAP_PTS_TEST) && (MTK_LINUX_GAP_PTS_TEST == TRUE)
            if (stack_config_get_interface()->get_pts_passive_security_request())
            {
                /* do nothing */
                BTM_TRACE_DEBUG ("GAP PTS, DUT as slave do not send security_request active");
            }
            else
            {
...
#if (defined(MTK_COMMON) && MTK_COMMON == TRUE) && (BLE_PRIVACY_SPT == FALSE)
            if (enhanced)
            {
                STREAM_TO_BDADDR   (local_rpa, p);
                STREAM_TO_BDADDR   (peer_rpa, p);
            }
#endif /* (defined(MTK_BLUEDROID_PATCH) && MTK_BLUEDROID_PATCH == TRUE) && (BLE_PRIVACY_SPT == FALSE) */
