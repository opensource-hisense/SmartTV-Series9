...
#if defined(MTK_LINUX_GAP_PTS_TEST) && (MTK_LINUX_GAP_PTS_TEST == TRUE)
        if (stack_config_get_interface()->get_pts_non_resolvable_private_address())
        {
            btm_gen_non_resolvable_private_addr((tBTM_BLE_ADDR_CBACK *)btm_gen_non_resolve_paddr_cback, NULL);
        }
        else
        {
...
{
#if defined(MTK_BTM_LE_SCAN_PARM_IGNORE) && (MTK_BTM_LE_SCAN_PARM_IGNORE == TRUE)
    /** Framework will set BLE full scan params as follow:
     *  windows : 5000ms
     *  interval: 5000ms
     *  This will lead to some problems, ex.LE create connection slow, inquiry can not find BR/EDR devices.
     *      So we use default LE 11.25/80 scan params
     */
    BTM_TRACE_EVENT ("%s: Ignore framework setting", __func__);
    return ;
#endif /* MTK_BTM_LE_SCAN_PARM_IGNORE */
    ...
    *p_flag = NULL;
    ...
    if (data_mask)
    {
        ...
        if (len > MIN_ADV_LENGTH && data_mask & BTM_BLE_AD_BIT_DEV_NAME)
        {
            ...
            if (btm_cb.cfg.le_name_len > 0)
            {
                BTM_TRACE_DEBUG("bd_le_name=%s", btm_cb.cfg.bd_le_name);
                if (strlen(btm_cb.cfg.bd_le_name) > (UINT16)(len - MIN_ADV_LENGTH))
                {
                    *p++ = len - MIN_ADV_LENGTH + 1;
                    *p++ = BTM_BLE_AD_TYPE_NAME_SHORT;
                    ARRAY_TO_STREAM(p, btm_cb.cfg.bd_le_name, len - MIN_ADV_LENGTH);
                }
                else
                {
                    cp_len = (UINT16)strlen(btm_cb.cfg.bd_le_name);
                    *p++ = cp_len + 1;
                    *p++ = BTM_BLE_AD_TYPE_NAME_CMPL;
                    ARRAY_TO_STREAM(p, btm_cb.cfg.bd_le_name, cp_len);
                }
...
**
** Description     Add by MTK: free resources of the ble control block.
**
** Returns          void
**
*******************************************************************************/
void btm_ble_free(void)
{
    ...
    alarm_free(p_cb->inq_var.inquiry_timer);
    p_cb->inq_var.inquiry_timer = NULL;

    alarm_free(p_cb->addr_mgnt_cb.refresh_raddr_timer);
    p_cb->addr_mgnt_cb.refresh_raddr_timer = NULL;
}
#endif
