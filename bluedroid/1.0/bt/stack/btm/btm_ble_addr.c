...
static void btm_gen_resolve_paddr_cmpl(tSMP_ENC *p)
{
    ...
    if (p)
    {
        ...
        BTM_TRACE_DEBUG("random_addr:%02x:%02x:%02x:%02x:%02x:%02x",
                        p_cb->private_addr[0],p_cb->private_addr[1],
                        p_cb->private_addr[2],p_cb->private_addr[3],
                        p_cb->private_addr[4],p_cb->private_addr[5]);
#endif
...
/*******************************************************************************
**
** Function         btm_gen_non_resolve_paddr_cback
**
** Description      This function set a non resolvable private address to controller.
**
** Returns          void
**
*******************************************************************************/
void btm_gen_non_resolve_paddr_cback(BD_ADDR_PTR static_random, void *p)
{
#if (BLE_INCLUDED == TRUE && SMP_INCLUDED == TRUE)
    tBTM_LE_RANDOM_CB *p_cb = &btm_cb.ble_ctr_cb.addr_mgnt_cb;
    BTM_TRACE_EVENT ("btm_gen_non_resolve_paddr_cback");

    if (!p)
    {
        /* set hash to be LSB of rpAddress */
        p_cb->private_addr[5] = static_random[5];
        p_cb->private_addr[4] = static_random[4];
        p_cb->private_addr[3] = static_random[3];
        p_cb->private_addr[2] = static_random[2];
        p_cb->private_addr[1] = static_random[1];
        p_cb->private_addr[0] = static_random[0];
        /* set it to controller */
        BTM_TRACE_DEBUG("random_addr:%02x:%02x:%02x:%02x:%02x:%02x",
                       p_cb->private_addr[0],p_cb->private_addr[1],
                       p_cb->private_addr[2],p_cb->private_addr[3],
...
static BOOLEAN btm_ble_match_random_bda(void *data, void *context)
{
    ...
    if (!(p_dev_rec->device_type & BT_DEVICE_TYPE_BLE) ||
        ...
        return TRUE;
#if defined(MTK_LINUX_GAP_PTS_TEST) && (MTK_LINUX_GAP_PTS_TEST == TRUE)
        BTM_TRACE_DEBUG("btm_ble_match_random_bda ble.keys.irk is 0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
                        p_dev_rec->ble.keys.irk[15], p_dev_rec->ble.keys.irk[14],p_dev_rec->ble.keys.irk[13], p_dev_rec->ble.keys.irk[12],
                        p_dev_rec->ble.keys.irk[11], p_dev_rec->ble.keys.irk[10],p_dev_rec->ble.keys.irk[9], p_dev_rec->ble.keys.irk[8],
                        p_dev_rec->ble.keys.irk[7], p_dev_rec->ble.keys.irk[6],p_dev_rec->ble.keys.irk[5], p_dev_rec->ble.keys.irk[4],
                        p_dev_rec->ble.keys.irk[3], p_dev_rec->ble.keys.irk[2],p_dev_rec->ble.keys.irk[1], p_dev_rec->ble.keys.irk[0]);
#endif
    /* generate X = E irk(R0, R1, R2) and R is random address 3 LSO */
    SMP_Encrypt(p_dev_rec->ble.keys.irk, BT_OCTET16_LEN,
                &rand[0], 3, &output);
