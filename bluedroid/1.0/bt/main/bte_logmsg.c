...
#endif
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
  {BTTRC_ID_STK_B3DS, BTTRC_ID_STK_B3DS, B3DS_SetTraceLevel, "TRC_B3DS", DEFAULT_CONF_TRACE_LEVEL},
#endif
  /* LayerIDs for BTA, currently everything maps onto appl_trace_level.
   */
...
EXPORT_SYMBOL void set_trace_log_level(const char *trc_name, UINT8 trace_level)
{
    for (tBTTRC_FUNC_MAP *functions = &bttrc_set_level_map[0]; functions->trc_name; ++functions) {
        if (0 == strcmp(functions->trc_name, trc_name))
        {
            ...
            if (trace_level != -1)
            {
                functions->trace_level = trace_level;
            }
