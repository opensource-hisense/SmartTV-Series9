...
#if defined(MTK_LINUX_AUDIO_LOG_DEFINE) && (MTK_LINUX_AUDIO_LOG_DEFINE == TRUE)
/* Define trace levels */
#define AUDIO_BT_TRACE_LEVEL_NONE    0          /* No trace messages to be generated    */
#define AUDIO_BT_TRACE_LEVEL_ERROR   1          /* Error condition trace messages       */
#define AUDIO_BT_TRACE_LEVEL_WARNING 2          /* Warning condition trace messages     */
#define AUDIO_BT_TRACE_LEVEL_API     3          /* API traces                           */
#define AUDIO_BT_TRACE_LEVEL_EVENT   4          /* Debug messages for events            */
#define AUDIO_BT_TRACE_LEVEL_DEBUG   5          /* Full debug messages                  */
#define AUDIO_BT_TRACE_LEVEL_VERBOSE 6          /* Verbose debug messages               */

#define AUDIO_MAX_TRACE_LEVEL        6
static uint8_t audio_a2dp_trace_level = AUDIO_BT_TRACE_LEVEL_WARNING;

#define AUDIOLOGWRAPPER(tag, fmt, args...) fprintf(stderr, "%s: " fmt "\n", tag, ## args)

#define AUDIO_LOG_DEBUG(...) AUDIOLOGWRAPPER(__VA_ARGS__)

#define FNLOG()             {if (audio_a2dp_trace_level >= AUDIO_BT_TRACE_LEVEL_DEBUG) AUDIO_LOG_DEBUG(LOG_TAG, "%s", __FUNCTION__);}
#define DEBUG(fmt, ...)     {if (audio_a2dp_trace_level >= AUDIO_BT_TRACE_LEVEL_DEBUG) AUDIO_LOG_DEBUG(LOG_TAG, "%s: " fmt,__FUNCTION__, ## __VA_ARGS__);}
#define INFO(fmt, ...)      {if (audio_a2dp_trace_level >= AUDIO_BT_TRACE_LEVEL_API) AUDIO_LOG_DEBUG(LOG_TAG, "%s: " fmt,__FUNCTION__, ## __VA_ARGS__);}
#define WARN(fmt, ...)      {if (audio_a2dp_trace_level >= AUDIO_BT_TRACE_LEVEL_WARNING) AUDIO_LOG_DEBUG(LOG_TAG, "%s: " fmt,__FUNCTION__, ## __VA_ARGS__);}
#define ERROR(fmt, ...)     {if (audio_a2dp_trace_level >= AUDIO_BT_TRACE_LEVEL_ERROR) AUDIO_LOG_DEBUG(LOG_TAG, "%s: " fmt,__FUNCTION__, ## __VA_ARGS__);}
#else
...
#if defined(MTK_LINUX_AUDIO_LOG_DEFINE) && (MTK_LINUX_AUDIO_LOG_DEFINE == TRUE)
EXPORT_SYMBOL uint8_t audio_set_trace_level(uint8_t new_level)
{
    ERROR("log level:%d", new_level);

    if (new_level != 0xFF)
    {
        audio_a2dp_trace_level = new_level;
    }

    return audio_a2dp_trace_level;
}
#endif

#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL int adev_open_output_stream(struct audio_hw_device *dev,
...
#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL void adev_close_output_stream(struct audio_hw_device *dev,
                                     struct audio_stream_out *stream)
#else
...
#if defined(MTK_LINUX_AUDIO_LOG_DEFINE) && (MTK_LINUX_AUDIO_LOG_DEFINE == TRUE)
    WARN("closing output (state %d)", out->common.state);
#else
...
#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL int adev_open_input_stream(struct audio_hw_device *dev,
                                  audio_io_handle_t handle,
                                  audio_devices_t devices,
                                  struct audio_config *config,
...
#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL void adev_close_input_stream(struct audio_hw_device *dev,
                                   struct audio_stream_in *stream)
#else
...
#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL int adev_open(const hw_module_t* module, const char* name,
                         hw_device_t** device)
