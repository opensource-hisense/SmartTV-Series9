...
typedef struct {
  ...
  BT_HDR *(*make_set_cmd_from_ext)(uint16_t opcode, uint8_t *ary, uint8_t len); /* MTK_STACK_CONFIG_LOG */
#endif
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
  BT_HDR *(*make_set_event_mask_2)(const bt_event_mask_t *event_mask); /* B3DS_INCLUDED */
#endif
