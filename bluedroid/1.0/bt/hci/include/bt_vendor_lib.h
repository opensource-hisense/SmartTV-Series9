...
/* MTK vendor opcode
 * A share build flag "MTK_BT_COMMON" for both vendor driver and bluedroid stack
 * Both above layers should define the flag in each makefile respectively
 *
 * 1. stack includes this file in vendor.h
 * 2. vendor driver includes this file in both bt_mtk.h and bt_drv.c
 */
 ...
 */
 ...
 /* [operation]
 *      Stack call to the vendor module to set PSM control.
 *  [input param]
 *      A pointer to uint8_t type which indicate on/off PSM
 *      Typecasting conversion: (uint8_t *) param.
 *  [return]
 *      0          - default, don't care.
 *      -1         - kernel driver not ready.
 *      -EBUSY     - stack on/off PSM control failed.
 *  [callback]
 *      None.
 */
    BT_VND_OP_SET_PSM_CONTRL,

/*  [operation]
 *      Call to the vendor module so that it can perform some vendor-specific
 *      operations on some vendor specific hci event/acl data/sco data.
 *  [input param]
 *      A pointer to bt_vendor_op_handle_vendor_msg_t containing vendor message.
 *  [return]
 *      0  - default, dont care.
 *  [callback]
 *      None
 */
    BT_VND_OP_HANDLE_VENDOR_MESSAGE,
#endif
} bt_vendor_opcode_t;
...
typedef struct {
    ...
    uint8_t   type;                         /* data type (serial_data_type_t) */
    uint8_t   *data;                        /* data content*/
} bt_vendor_op_handle_vendor_msg_t;
#endif
