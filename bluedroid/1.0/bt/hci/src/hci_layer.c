...
#if defined(MTK_LINUX_RESET) && (MTK_LINUX_RESET == TRUE)
hci_state_callback_t *hci_state_cback;
#endif
...
// Interface functions
#if defined(MTK_HCI_POWERSAVING_MODE) && (MTK_HCI_POWERSAVING_MODE == TRUE)
static void send_psm_command(int enable) {
  LOG_INFO(LOG_TAG, "%s enable:%d", __func__, enable);
#if defined(MTK_VENDOR_OPCODE) && (MTK_VENDOR_OPCODE == TRUE)
  {
    int en = enable;
    vendor->send_command(VENDOR_SET_PSM_CONTROL, &en);
  }
#endif
}
#endif
...
#if defined(MTK_LINUX_RESET) && (MTK_LINUX_RESET == TRUE)
static void set_hci_state_callback(hci_state_callback_t cback)
{
    LOG_WARN(LOG_TAG, "%s", __func__);
    hci_state_cback = cback;
}
#endif
// Start up functions
...
#if defined(MTK_LINUX_RESET) && (MTK_LINUX_RESET == TRUE)
  LOG_ERROR(LOG_TAG, "%s call the hci_timeout_callback", __func__);
  if (NULL != hci_state_cback)
    (*hci_state_cback)(BT_HCI_TIMEOUT);
  else
  LOG_ERROR(LOG_TAG, "%s have not register hci_timeout_callback", __func__);
#else
  ...
  while (hal->read_data(type, &byte, 1) != 0) {
    ...
    if (incoming->state == FINISHED) {
      ...
      if (type != DATA_TYPE_COMMAND) {
        // Pass all hci event/acl data/sco data to vendor library.
        bt_vendor_op_handle_vendor_msg_t vend_msg;
        vend_msg.len = incoming->buffer->len;
        vend_msg.type = type;
        vend_msg.data = incoming->buffer->data;
        vendor->send_async_command(VENDOR_HANDLE_VENDOR_MESSAGE, &vend_msg);
      }
#endif
...
#if defined(MTK_HCI_POWERSAVING_MODE) && (MTK_HCI_POWERSAVING_MODE == TRUE)
    interface.send_psm_command = send_psm_command;
#endif
...
#if defined(MTK_LINUX_RESET) && (MTK_LINUX_RESET == TRUE)
    interface.set_hci_state_callback = set_hci_state_callback;
#endif
