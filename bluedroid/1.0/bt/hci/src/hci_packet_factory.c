...
static BT_HDR *make_set_cmd_from_ext(uint16_t opcode, uint8_t *ary, uint8_t len) {
  uint8_t *stream;
  uint8_t parameter_size = len;
  BT_HDR *packet = make_command(opcode, parameter_size, &stream);

  ARRAY_TO_STREAM(stream, ary, len);
  return packet;
}
#endif
...
/* B3DS_INCLUDED */
static BT_HDR *make_set_event_mask_2(const bt_event_mask_t *event_mask) {
  uint8_t *stream;
  uint8_t parameter_size = sizeof(bt_event_mask_t);
  BT_HDR *packet = make_command(HCI_SET_EVENT_MASK_PAGE_2, parameter_size, &stream);
...
static const hci_packet_factory_t interface = {
  ...
  make_set_event_mask,
#if defined(MTK_STACK_CONFIG_LOG) && (MTK_STACK_CONFIG_LOG == TRUE)
  make_set_cmd_from_ext,    /* MTK_STACK_CONFIG_LOG */
#endif
#if defined(MTK_B3DS_SUPPORT) && (MTK_B3DS_SUPPORT == TRUE)
  make_set_event_mask_2,    /* B3DS_INCLUDED */
#endif
