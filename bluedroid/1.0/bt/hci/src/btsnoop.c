...
#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
#include "twrite.h"
#endif
...
static void update_logging() {
  ...
  BTSNOOP_LOG_MODE log_mode = stack_config->get_btsnoop_log_mode();
#else
  BTSNOOP_LOG_MODE log_mode = BTSNOOP_LOG_MODE_DEFAULT;
#endif
  bool should_log_MTK = module_started && enable_mtk_btsnoop();
  if (!should_log && should_log_MTK) {
    log_mode = BTSNOOP_LOG_MODE_ALONE;
    should_log = should_log_MTK;
    LOG_INFO(LOG_TAG, "%s twrite log mode %d.", __func__, log_mode);
  }
#endif
...
#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
    if (twrite_init(log_path, log_mode, &logfile_fd)) {
      LOG_INFO(LOG_TAG, "%s twrite log thread is started!", __func__);
    } else {
      LOG_INFO(LOG_TAG, "%s create twrite log thread fails, there's no hci log!", __func__);
...
#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
  if ((int)length != twrite_write(data, length))
    LOG_ERROR(LOG_TAG, "%s twrite_write data failed", __func__);
#else
  if (logfile_fd != INVALID_FD)
...
#if defined(MTK_BTSNOOPLOG_THREAD) && (MTK_BTSNOOPLOG_THREAD == TRUE)
  length = length_he;
  twrite_write_packet(btsnoop_write, type, packet, length_he, length, flags,
    drops, time_hi, time_lo);
#else
