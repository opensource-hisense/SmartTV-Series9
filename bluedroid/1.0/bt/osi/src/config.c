...
EXPORT_SYMBOL config_t *config_new(const char *filename) {
#else
...
#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL int config_get_int(const config_t *config, const char *section, const char *key, int def_value) {
#else
...
#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL bool config_get_bool(const config_t *config, const char *section, const char *key, bool def_value) {
#else
...
#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL const char *config_get_string(const config_t *config, const char *section, const char *key, const char *def_value) {
#else
...
bool config_save(const config_t *config, const char *filename) {
  ...
  if (fflush(fp) == EOF)
  {
      LOG_WARN(LOG_TAG, "%s unable to fflush file '%s': %s", __func__, temp_filename, strerror(errno));
  }
#endif
...
bool config_override(config_t *dest_config, const char *filename) {
  assert(filename != NULL);
  ...
  config_parse(fp, dest_config);
  fclose(fp);
  return true;
}

#if defined(MTK_LINUX_EXPORT_API) && (MTK_LINUX_EXPORT_API == TRUE)
EXPORT_SYMBOL void config_dump(config_t *config) {
#else
void config_dump(config_t *config) {
#endif
  for (const list_node_t *node = list_begin(config->sections); node != list_end(config->sections); node = list_next(node)) {
    const section_t *section = (const section_t *)list_node(node);
    LOG_INFO(LOG_TAG, "[%s]", section->name);

    for (const list_node_t *enode = list_begin(section->entries); enode != list_end(section->entries); enode = list_next(enode)) {
      const entry_t *entry = (const entry_t *)list_node(enode);
      LOG_INFO(LOG_TAG, "\t%s = %s", entry->key, entry->value);
    }
  }
}
