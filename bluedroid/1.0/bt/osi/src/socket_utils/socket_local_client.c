...
#if defined(MTK_LINUX_OSI_SOCKET) && (MTK_LINUX_OSI_SOCKET == TRUE)
#define LOG_TAG "bt_sock_client"
...
int osi_socket_make_sockaddr_un(const char *name, int namespaceId,
  ...
  switch (namespaceId) {
    ...
    memcpy(p_addr->sun_path, name, namelen);
    LOG_WARN(LOG_TAG, "name %s line:%d", p_addr->sun_path, __LINE__);
#else
