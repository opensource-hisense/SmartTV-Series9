...
//#define CC_USE_KERNEL_CRYPTO  // use kernel crypto api
...
static noinline void _TzSendSMC(TZ_SMC_ARG_T *prSmcArg)
{
    ...
    asm volatile (
        ...
        "MOV    %0,x1\n"
        : "=r" (prSmcArg->u4Cmd)
        : "r" (prSmcArg->u4Cmd), "r" (prSmcArg->u4Addr), "r" (prSmcArg->u4Size)
        : "x0","x1","x2","x3", "memory"
    );
#else
    asm volatile (
...
static BOOL _TZ_CTL(UINT32 u4Cmd, UINT32 u4PAddr, UINT32 u4Size)
{
...
BOOL TZ_CTL(UINT32 u4Cmd, UINT32 u4PAddr, UINT32 u4Size)
{
    dma_addr_t src_addr;
    UPTR u4VAddr;
    BOOL fgRet;
...
BOOL TZ_CTL_EXTERN(UINT32 u4Cmd, UINT32 u4PAddr, UINT32 u4Size)
{
    return TZ_CTL(u4Cmd,u4PAddr,u4Size);
}

EXPORT_SYMBOL(TZ_CTL_EXTERN);
...
BOOL TZ_GCPU_FlushInvalidateDCache(void)
{
    ...
    fgRet = TZ_CTL(TZCTL_GCPU_CLEAN_DCACHE, 0, 0);
...
static BOOL GCPU_Init(UINT32 u4Mode)
{
    ...
    if (!_prGcpuHwCmd)
    {
        ...
        _u4GcpuHwCmdAddr = (UPTR)kmalloc((DCACHE_LINE_SIZE + _u4GcpuHwCmdSize), GFP_KERNEL | GFP_DMA);
        if (!_u4GcpuHwCmdAddr)
        {
        ...
        _u4GcpuTzParamAddr = (UPTR)kmalloc((DCACHE_LINE_SIZE + _u4GcpuTzParamSize), GFP_KERNEL | GFP_DMA);
...
#ifndef CC_USE_KERNEL_CRYPTO
//#if !defined(CONFIG_CRYPTO_AES_ARM_BS) && !defined(CONFIG_CRYPTO_AES_ARM64_CE)
BOOL NAND_AES_INIT(void)
{
    GCPU_Init(0);
...
//	printk("%s - In=0x%x, Out=0x%x, size=%u\n", __FUNCTION__, u4InBufStart, u4OutBufStart, u4BufSize);
...
static struct crypto_blkcipher *gblkcipher = NULL;
struct blkcipher_desc gdesc;
int mtk_armv8ce_aes_close(void)
{
    if (gblkcipher) {
        crypto_free_blkcipher(gblkcipher);
    }
}

int mtk_armv8ce_aes_init(const void *key, int key_len)
{
#if 1 //1: aes_ecb, 0:aes_cbc
    char *cipher = "__ecb-aes-ce";
#elif 1 //1: armv8 (64-bit), 0: armv7 (32-bit)
    char *cipher = "__cbc-aes-ce";
#else
    char *cipher = "__cbc-aes-neonbs";
#endif
    ...
    gblkcipher = crypto_alloc_blkcipher(cipher, 0, 0);
    if (IS_ERR(gblkcipher)) {
        printk("could not allocate gblkcipher handle for %s\n", cipher);
        return -PTR_ERR(gblkcipher);
    }

    if (crypto_blkcipher_setkey(gblkcipher, charkey, key_len)) {
        printk("key could not be set\n");
        mtk_armv8ce_aes_close();
        return -EAGAIN;
    }

    ivsize = crypto_blkcipher_ivsize(gblkcipher);
    if (ivsize) {
        if (ivsize != AES_BLOCK_SIZE) {
            printk("IV length differs from expected length. It should be : %d\n",ivsize);
        }
        crypto_blkcipher_set_iv(gblkcipher, iv, ivsize);
    }
    gdesc.flags = 0;
    gdesc.tfm = gblkcipher;

    return -EFAULT;
}

int mtk_armv8ce_aes(void *dst, size_t *dst_len, const void *src, size_t src_len, int encrypt)
{
#if 0
    struct timeval rStartTime, rEndTime, rDiffTime;
#endif
    struct scatterlist sg_out;
    char iv[AES_BLOCK_SIZE];
    unsigned int ivsize = 0;
    sg_init_one(&sg_out, src, src_len);

    if (encrypt == 1) {
#if 0
        printk("encrypting... (input value: %d)\n", encrypt);
        do_gettimeofday(&rStartTime);
#endif
        crypto_blkcipher_encrypt(&gdesc, &sg_out, &sg_out, src_len);
    } else if (encrypt == 0) {
#if 0
        printk("decrypting... (input value: %d)\n", encrypt);
        do_gettimeofday(&rStartTime);
#endif
        crypto_blkcipher_decrypt(&gdesc, &sg_out, &sg_out, src_len);
    } else {
        printk("encrypt: 1, decrypt: 0, others: NG (input value: %d)\n", encrypt);
        mtk_armv8ce_aes_close();
    }

#if 0
    do_gettimeofday(&rEndTime);
    if (timeval_subtract(&rDiffTime, &rEndTime, &rStartTime)) {
        printk("%s(%d)The difference is negative. (rDiffTime.tv_sec=%d, rDiffTime.tv_usec=%d)\n", __FUNCTION__, __LINE__, rDiffTime.tv_sec, rDiffTime.tv_usec);
    }
    printk("%s(%d)rDiffTime.tv_sec=%d, rDiffTime.tv_usec=%d\n", __FUNCTION__, __LINE__, rDiffTime.tv_sec, rDiffTime.tv_usec);
#endif

    memcpy(dst, src, sg_out.length);
    ...
    *dst_len = sg_out.length;
    ...
    if (ivsize) {
        ...
        crypto_blkcipher_get_iv(gblkcipher, iv, ivsize);
        crypto_blkcipher_set_iv(gblkcipher, iv, ivsize);
    }
#endif
    return 1;
...
struct hash_desc g_hash_desc;
int mtk_armv8ce_sha1_init(void)
{
    g_hash_desc.tfm = crypto_alloc_hash("sha1", 0, CRYPTO_ALG_TYPE_SHASH); //CRYPTO_ALG_ASYNC
    crypto_hash_init(&g_hash_desc);
}

int mtk_armv8ce_sha1_update(const void *src, size_t src_len)
{
    struct scatterlist sg;

    sg_init_one(&sg, (char*)src, src_len);
    crypto_hash_update(&g_hash_desc, &sg, src_len);
}

int mtk_armv8ce_sha1_final(void *dst, const void *src, size_t src_len)
{
    char hashtext[41];

    if (src_len != 0) {
        mtk_armv8ce_sha1_update(src, src_len);
    }
    crypto_hash_final(&g_hash_desc, hashtext);
    crypto_free_hash(g_hash_desc.tfm);

    memcpy(dst, hashtext, 20);
}

struct hash_desc g_hash_desc_sha256;
int mtk_armv8ce_sha256_init(void)
{
    g_hash_desc_sha256.tfm = crypto_alloc_hash("sha256", 0, CRYPTO_ALG_TYPE_SHASH);
    crypto_hash_init(&g_hash_desc_sha256);
}

int mtk_armv8ce_sha256_update(const void *src, size_t src_len)
{
    struct scatterlist sg;

    sg_init_one(&sg, (char*)src, src_len);
    crypto_hash_update(&g_hash_desc_sha256, &sg, src_len);
}

int mtk_armv8ce_sha256_final(void *dst, const void *src, size_t src_len)
{
    char hashtext[41];

    if (src_len != 0) {
        mtk_armv8ce_sha256_update(src, src_len);
    }
    crypto_hash_final(&g_hash_desc_sha256, hashtext);
    crypto_free_hash(g_hash_desc_sha256.tfm);

    memcpy(dst, hashtext, 32);
}

BOOL NAND_AES_INIT(void)
{
    UINT8 key[16]={0xff, 0xee, 0xdd, 0xcc, 0xbb, 0xaa, 0x99, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x00};
    mtk_armv8ce_aes_init(key, 16); //128-bit key
}

BOOL NAND_AES_Encryption(UINT32 u4InBufStart, UINT32 u4OutBufStart, UINT32 u4BufSize)
{//arguments of physical address
    size_t dst_len;
    return mtk_armv8ce_aes(phys_to_virt((UPTR)u4OutBufStart), &dst_len, phys_to_virt((UPTR)u4InBufStart), (size_t)u4BufSize, 1);
}

BOOL NAND_AES_Decryption(UINT32 u4InBufStart, UINT32 u4OutBufStart, UINT32 u4BufSize)
{//arguments of physical address
    size_t dst_len;
    return mtk_armv8ce_aes(phys_to_virt((UPTR)u4OutBufStart), &dst_len, phys_to_virt((UPTR)u4InBufStart), (size_t)u4BufSize, 0);
}

EXPORT_SYMBOL(NAND_AES_INIT);
EXPORT_SYMBOL(NAND_AES_Encryption);
EXPORT_SYMBOL(NAND_AES_Decryption);
EXPORT_SYMBOL(mtk_armv8ce_sha1_init);
EXPORT_SYMBOL(mtk_armv8ce_sha1_update);
EXPORT_SYMBOL(mtk_armv8ce_sha1_final);
EXPORT_SYMBOL(mtk_armv8ce_sha256_init);
EXPORT_SYMBOL(mtk_armv8ce_sha256_update);
EXPORT_SYMBOL(mtk_armv8ce_sha256_final);

#endif // end of CC_USE_KERNEL_CRYPTO
