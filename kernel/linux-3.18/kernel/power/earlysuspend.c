...
extern void PDWNC_PowerStateNotify(UINT32 u4PowerState, UINT32 u4Arg1, UINT32 u4Arg2, UINT32 u4Arg3, UINT32 u4Arg4);
#endif
////////


/////// Registration for early suspend/late resume notificaton
static void (*_early_suspend_notify_f)(void) = 0;
static void (*_late_resume_notify_f)(void) = 0;
void RegisterEarlySuspendNotification(void (*f)(void))
{
    _early_suspend_notify_f = f;
}
void RegisterLateResumeNotification(void (*f)(void))
{
    _late_resume_notify_f = f;
}
//////
...
static void early_suspend(struct work_struct *work)
{
    ...
    pr_warn("@@@@@@@@@@@@@@@@@@@@@@@\n@@@__early_suspend__@@@\n@@@@@@@@@@@@@@@@@@@@@@@\n");
    printk("@@@__early_suspend__start @@@ pid=%d\n", current->pid);
    ...
    if ( _early_suspend_notify_f )
    {
        printk("Send Early Suspend Notification to BT driver\n");
        _early_suspend_notify_f();
    }
...
static void late_resume(struct work_struct *work)
{
    ...
    pr_warn("@@@@@@@@@@@@@@@@@@@@@@@\n@@@__late_resume__@@@\n@@@@@@@@@@@@@@@@@@@@@@@\n");
    printk("@@@__late_resume__start @@@ pid=%d\n", current->pid);

	pm_autosleep_set_state(PM_SUSPEND_ON);

    if ( _late_resume_notify_f )
    {
        printk("Send Late Resume Notification to BT driver\n");
        _late_resume_notify_f();
    }
