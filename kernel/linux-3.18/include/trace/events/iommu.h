...
TRACE_EVENT(map,
	...
	TP_PROTO(void *domain,unsigned long iova, phys_addr_t paddr, size_t size),
	...
	TP_printk("IOMMU:dom=%p  iova=0x%016llx paddr=0x%016llx size=%zu",
			__entry->dom,__entry->iova, __entry->paddr, __entry->size
	)
);

TRACE_EVENT(unmap,

	TP_PROTO(void *domain,unsigned long iova, size_t size, size_t unmapped_size),
	...
	TP_printk("IOMMU: dom=%p iova=0x%016llx size=%zu unmapped_size=%zu",
			__entry->dom,__entry->iova, __entry->size, __entry->unmapped_size
	)
);
