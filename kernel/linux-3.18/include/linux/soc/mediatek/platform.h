...
#define CPU_DBGPCSRlo           (0xA0)
#define CPU_DBGPCSRhi           (0xAC)
#define CPU_OSLAR               (0x300)
#define CPU_EDLAR               (0xFB0)
#define CPU_EDLAR_KEY           (0xC5ACCE55)
...
#define REG_IOBUS_TOUT_DBG  0x0320	// IOBUS TIME OUT DEBUG REGISTER
    #if defined(CONFIG_MACH_MT5886) ||  defined(CONFIG_MACH_MT5893)
    #define IOBUS_TOUT          (1U << 30)
    #define IOBUS_TOUT_WRITE    (1U << 29)
    #define IOBUS_TOUT_ADR_MSK  (0x3FFFFFF)
    #else
    #define IOBUS_TOUT          (1U << 28)
    #define IOBUS_TOUT_WRITE    (1U << 27)
    #define IOBUS_TOUT_ADR_MSK  (0xFFFFFF)    
    #endif
...
#define REG_CPU_CLK_CTL		(0x0400)	// CKDIV
#define REG_CPU0_PWR_CTL	(0x0404)	// GIC CONTROL
...
#define REG_STANDBY_STATUS  (0x0034)    // Standby status
#define REG_SMPC_PWR_CTL    (0x0038)    // SMPC power control
    #define PWR_ON          (1U << 0)
    #define PWR_ACK         (1U << 1)
    #define PWR_RST         (1U << 2)
#endif
...
#if defined(CONFIG_MACH_MT5891) || defined(CONFIG_MACH_MT5886) || defined(CONFIG_MACH_MT5863) || defined(CONFIG_MACH_MT5893)
...
#define __pdwnc_readl(REG)           __raw_readl(pPdwncIoMap + (REG))
#define __pdwnc_writel(VAL, REG)     __raw_writel(VAL, pPdwncIoMap + (REG))
#define __ckgen_readl(REG)           __raw_readl(pCkgenIoMap + (REG))
//TODO: should reverse the order
#define __ckgen_writel(VAL, REG)     __raw_writel(VAL, pCkgenIoMap + (REG))
