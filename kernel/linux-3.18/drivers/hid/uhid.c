...
#define SKIP_BT_POWER_KEY

#ifdef SKIP_BT_POWER_KEY
...
int uhid_getkeyvalue= 0;
module_param(uhid_getkeyvalue, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(uhid_getkeyvalue,"uhid get key value");
...
#ifdef SKIP_BT_POWER_KEY
static int first_power_key;
static int is_power_key(u8 *data, int size)
{
    /* The code for power key from BT is 03 00 02 */
    return (3 == size && 0x03 == data[0] && 0x0 == data[1] && 0x02 == data[2]);
}

static int is_release_key(u8 *data, int size)
{
    /* The code for release key from BT is 03 00 00 */
    return (3 == size && 0x03 == data[0] && 0x0 == data[1] && 0x00 == data[2]);
}

static int is_bt_wakeup(void)
{
    typedef unsigned int (*PDWNC_ReadWakeupReason)(void);
    typedef unsigned int (*PDWNC_MTK_GetBTGPIODetected)(void);
    unsigned int reason, status;
    char *func_name = "PDWNC_ReadWakeupReason";
    char *func2_name = "PDWNC_MTK_GetBTGPIODetected";

    PDWNC_ReadWakeupReason pfunc = NULL;
    PDWNC_MTK_GetBTGPIODetected pfunc2 = NULL;

    pfunc = (PDWNC_ReadWakeupReason)kallsyms_lookup_name(func_name);
    if (pfunc) {
        /* PDWNC Wakeup Reason is 37 (PDWNC_WAKE_UP_REASON_BT) */
        reason = pfunc();
        printk("Wakeup Reason %d\n", reason);
        if (reason == 37){
            return 1;
        } else if (reason == 2 || reason ==36){
            pfunc2 = (PDWNC_MTK_GetBTGPIODetected)kallsyms_lookup_name(func2_name);
            if (pfunc2){
                status = pfunc2();
                printk("Detect BT GPIO: %d\n", status);
                if (status) {
                    return 1;
                }
            } else {
                printk("%s Error :No Exported Func Found [%s]\n", __func__, func2_name);
            }
        }
...
static void dump_input_key(u8 *data, int size)
{
    int i;
    for (i = 0; i < size; i++) {
        if (i && (i % 16 == 0)) {
...
static unsigned int need_skip_release_key = 0;
#endif
				...
				size_t count, loff_t *ppos)
{
	...
	case UHID_INPUT:
#ifdef SKIP_BT_POWER_KEY
        data = uhid->input_buf.u.input.data;
        size = uhid->input_buf.u.input.size;
        curr_cnt = pre_cnt;
        pre_cnt = mt53xx_pm_suspend_cnt;
		if(uhid_getkeyvalue)
		{
        	printk("Dump UHID INPUT Key...\n");
        	dump_input_key(data, size);
		}
        if (is_power_key(data, size)) {
            //printk("===>is Power Key, curr_cnt = %u, mt53xx_pm_suspend_cnt = %u\n",
            //        curr_cnt, mt53xx_pm_suspend_cnt);
            if (curr_cnt != mt53xx_pm_suspend_cnt && is_bt_wakeup()) {
                 need_skip_release_key = 1;
                 printk("Filter out Power Key from BT device\n");
                 break;
            }
        }
        if (is_release_key(data, size)) {
            //printk("===>is Release Key\n");
            if (need_skip_release_key) {
                 need_skip_release_key = 0;
                 printk("Filter out Release Power Key from BT device\n");
                 break;
            }
        }
#endif
