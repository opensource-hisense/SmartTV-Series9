			      ...
			      int num_entries, struct io_pgtable_cfg *cfg)
{
	int i;
	#ifdef CONFIG_64BIT
           pr_debug("(peggy) [__arm_v7s_set_pte] pte : 0x%llx\n\n", pte);
	#else
           //pr_debug("(peggy) [__arm_v7s_set_pte] pte : 0x%x\n\n", pte);
	#endif
			 ...
			 int lvl, arm_v7s_iopte *ptep)
{
	...
	#ifdef CONFIG_64BIT
           pr_debug("(peggy) [__arm_v7s_map] ptep : 0x%llx\n", ptep);
           pr_debug("(peggy) [__arm_v7s_map] iova : 0x%llx\tpaddr: 0x%llx\n", iova, paddr);
	#else
           //pr_debug("(peggy) [__arm_v7s_map] ptep : 0x%x\n", ptep);
           //pr_debug("(peggy) [__arm_v7s_map] iova : 0x%x\tpaddr: 0x%x\n", iova, paddr);
	#endif
	ptep += ARM_V7S_LVL_IDX(iova, lvl);
	#ifdef CONFIG_64BIT
           pr_debug("(peggy) [__arm_v7s_map] ptep+iova_offset : 0x%llx\n", ptep);
	#else
           //pr_debug("(peggy) [__arm_v7s_map] ptep+iova_offset : 0x%x\n", ptep);
	#endif
						...
						void *cookie)
{
	...
	data->pgd = __arm_v7s_alloc_table(1, GFP_KERNEL, data);
        pr_debug("(peggy) [arm_v7s_alloc_pgtable] data->pgd = 0x%p\n", data->pgd);

	if (!data->pgd)
		goto out_free_data;
#if 0
#ifdef CONFIG_MTK_IOMMU
	/* pgd only allocated once for iommu device */
	pr_debug("(peggy) [arm_v7s_alloc_pgtable] devName: %s\tdev = %p\n", dev_name(cfg->iommu_dev), cfg->iommu_dev);
	MtkIommuSavePgd(virt_to_phys(data->pgd));
#endif
#endif
