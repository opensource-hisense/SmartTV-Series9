...
#ifndef USB_BULK_QMU_SUPPORT
//#define USB_BULK_QMU_SUPPORT
#endif

#define URB_IN_GPD	1
#define URB_NOT_IN_GPD	0

#ifndef CONFIG_USB_MTK_MUSB_QMU_BULK_RX_AUTO_SPLIT
#define CONFIG_USB_MTK_MUSB_QMU_BULK_RX_AUTO_SPLIT
#endif

#ifdef CONFIG_USB_MTK_MUSB_QMU_BULK_RX_AUTO_SPLIT
...
/*WIFI sometimes suspend/resume times isn't match, it cuases wifi resume flow abnormal.
 *Sometime system power down is not into suspend state because tasks busy. But when resumel,
 *the wifi and use consider it is wake up from suspend state. It sill cause some abnormal.
 *[Soluton]Now if in this case, when system resume, usb is not need disconnect (check device exist)/reconnect(reset usb phy)
 */
#define MUSB_SUPPORT_SKIP_SUSPEND (1)

/* Support BT data dump when disconnect */
//#define SUPPORT_WIRELESS_HCI_DUMP
...
#define USB_EVENTS_WIRELESS_DUMP      (0x01 << 8)
...
#define MGC_HSDMA_CHANNELS  8   // IC has 4 h/w DMA channel. Reseve the last one for Cmd Queue.
...
extern void MUT_VBUS_Monitor(void);
