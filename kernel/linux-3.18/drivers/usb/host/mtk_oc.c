...
int mgc_oc_dbg_enable = 0;
module_param(mgc_oc_dbg_enable, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(mgc_oc_dbg_enable,"Host oc logs enable");
...
#define OC_USB3_PORT_STA_BASE 0x10074430
#else
...
static int proc_oc_message(void __user *arg)
{
    ...
    *buf = 0;
	...
	if (mgc_oc_dbg_enable) {
	   printk("[USBOC][id=0x%x]proc_oc_message>>u1Is4K = %d, SONY_HUB_OCSTATUS = 0x%x, *buf = 0x%x\n", id, u1Is4K, SONY_HUB_OCSTATUS, *buf);
	}
...
#if defined(CC_LM_CUST) || defined(CC_PHX_CUST)
...
static int proc_insert_message(void __user *arg)
{
	...
	uint8_t size;
	uint8_t id = 0;
#if defined(CC_LM_CUST) || defined(CC_OX_CUST) || defined(CC_WK_CUST) || defined(CC_HK_CUST)
...
//	*((volatile uint32_t *)0xF000D724) &= ~0x10000000;
   ...
   do{
		...
		if(u1MUSBOCEnable)
		{
			...
			if (mgc_oc_dbg_enable) {
			   printk("[USBOC] u1MUSBOCPort = 0x%x, u1MUSBOCStatus = 0x%x.\n", u1MUSBOCPort, u1MUSBOCStatus);
			}
            if(u1MUSBOCPort != 0)
            {
                fgOCStatus = TRUE;
			...
			{	
				...
				if (!fgOCStatus)
				{
					printk("[USBOC]: Turn on Vbus GPIO on.\n");
