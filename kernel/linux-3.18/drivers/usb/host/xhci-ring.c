...
#ifndef USB_RESUME_TIMEOUT
#define USB_RESUME_TIMEOUT 25
#endif
...
//	flush_cache_all();//for DRAM agent from group2 to group1
...
#ifdef CONFIG_ARCH_MT5399
	if (temp & PORT_PLC){
        if((temp & PORT_CONNECT) == 1){
            xhci_dbg(xhci, "port connect, disable BC1.2 \n");
            //set CHGDT_EN=0x1
            addr = (int*) (SSUSB_U2_PHY_COM+U2_PHYBC12C);
            temp1 = readl(addr);
            temp1 &= ~(0x1);
            temp1 |= CHGDT_EN_Value;
            writel(temp1, addr);
            
        } else {
            xhci_dbg(xhci, "port dis-connect, enable BC1.2 \n");
            //set CHGDT_EN=0x0
            addr = (int*) (SSUSB_U2_PHY_COM+U2_PHYBC12C);
            temp1 = readl(addr);
            temp1 &= ~(0x1);
...
//					td, event_trb);
            if(!(usb_endpoint_xfer_isoc(&td->urb->ep->desc)&&(trb_comp_code == COMP_TX_ERR))){
                xhci_cleanup_halted_endpoint(xhci,
                    slot_id, ep_index, ep_ring->stream_id,
                    td, event_trb);
                }
		...
		struct xhci_transfer_event *event)
{
	...
	case COMP_STOP_INVAL:
		...
		break;
	case COMP_STALL:
		if(!(_mtk_logctl&LOGCTL_STALL))
		{
			_mtk_logctl|=LOGCTL_STALL;
...
cleanup:
		...
		if (ret) {
			...
			if ((urb->actual_length != urb->transfer_buffer_length &&
				...
				xhci_all(xhci, "Giveback URB %p, len = %d, "
						"expected = %d, status = %d\n",
...
static unsigned int count_sg_trbs_needed(struct xhci_hcd *xhci, struct urb *urb)
{
	...
	xhci_all(xhci, "count sg list trbs: num_sgs = %d\n", num_sgs);
	num_trbs = 0;
...
//    dma_map_single(NULL, start_trb, sizeof(struct xhci_generic_trb)*512, DMA_TO_DEVICE);
    for (i = 0; i < 4; i++)
    {
        Dummy_val+=start_trb->field[i];
    }
