...
MODULE_PARM_DESC(debug, "Debug xhci message level. Default = 0");
...
static void compliance_mode_recovery(unsigned long arg)
{
	...
	for (i = 0; i < xhci->num_usb3_ports; i++) {
	    ...
	    if (((temp & PORT_PLS_MASK) == USB_SS_PORT_LS_COMP_MOD)
		    || ((temp & PORT_PLS_MASK) == USB_SS_PORT_LS_SS_INACTIVE)
...
//		xhci_cleanup_msix(xhci);
	...
	xhci_info(xhci, "+[%s] set Run/Stop bit.\n", __FUNCTION__);
	/* step 4: set Run/Stop bit */
	command = xhci_readl(xhci, &xhci->op_regs->command);
	command |= CMD_RUN;
	xhci_writel(xhci, command, &xhci->op_regs->command);
...
int xhci_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
{
	...
	xhci_all(xhci, "xhci_urb_enqueue\n");
...
#ifdef CONFIG_USB_DELAY_RESUME_SUPPORT
    pPlatformDev->dev.power.is_delayresume = MUC_aDelayResumeUsing[2];       // USB3.0 user resume
#else
    pPlatformDev->dev.power.is_delayresume = FALSE;                          // USB3.0 default not support user resume
#endif
	#endif
