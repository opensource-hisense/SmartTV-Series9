...
int mgc_suspend_delay= 500;       // 2000ms
module_param(mgc_suspend_delay, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(mgc_suspend_delay,"Suspend Delay to wait disconnect done");


MODULE_DESCRIPTION("MTK 53xx USB Host Controller Driver");
MODULE_LICENSE("GPL");

#if defined(CC_S_PLATFORM)&&defined(CC_PHX_CUST)
#define WIFI_RST_GPIO 203
#endif
...
extern unsigned int xhci_connect_flag;
#endif
#if defined(CC_S_PLATFORM)&&(defined(CC_OX_CUST)||defined(CC_WK_CUST)||defined(CC_HK_CUST))
extern uint usb_HDDREGFlag;
#endif



extern bool Is_Logicool_Pad;
extern int Is_Logicool_Pad_port;

extern int mtk_get_usb_suspend(void);
extern void mtk_set_usb_suspend(int value);
//---------------------------------------------------------------------------
// Configurations
//---------------------------------------------------------------------------
...
#define MUSB_WIFI_PORT 2
#elif defined(CONFIG_ARCH_MT5865)
#define MUSB_WIFI_PORT 1
#else
...
#ifdef SUPPORT_WIRELESS_HCI_DUMP
static void MGC_WirelessEventDump(void);
#endif
//---------------------------------------------------------------------------
// Static variables
//---------------------------------------------------------------------------
...
struct wireless_event {
    ...
    u32 direction;
    char transfer_data[HCI_DUMP_BUF_SIZE];
};

struct wireless_event wireless_event_dump[HCI_DUMP_ENTRY_NUM];
	...
	void *dummy)
{
	uint8_t bPortNum;
#if defined(CC_S_PLATFORM)&&(defined(CC_OX_CUST)||defined(CC_WK_CUST)||defined(CC_HK_CUST))
        uint mask_usb_HDDREGFlag=0;
#else
	uint8_t bPortInsertFlag = 0;
	uint8_t bUnplugToSuspendWaitFlag = 0;
	MUSB_LinuxController *pController;
#endif
	if (event == PM_SUSPEND_PREPARE) {
        #if defined(CC_S_PLATFORM)&&(defined(CC_OX_CUST)||defined(CC_WK_CUST)||defined(CC_HK_CUST))
		printk(KERN_ERR "usb_HDDREGFlag = 0x%x\n", usb_HDDREGFlag);
		for(bPortNum=0; bPortNum< MUC_NUM_MAX_CONTROLLER; bPortNum++)
		{
			mask_usb_HDDREGFlag = mask_usb_HDDREGFlag | (1<<(bPortNum));
			if((Is_Logicool_Pad == 1) && (bPortNum == Is_Logicool_Pad_port))
			{
				if(MUC_aPwrGpio[bPortNum] != -1){
					MGC_VBusCtrl_pm(bPortNum, 0);
				    printk(KERN_ERR "[USB] TurnOff VBUS Port[%d] GPIO[%d] because of Logicool Pad Insert\n",bPortNum,MUC_aPwrGpio[bPortNum]);
					}
			}
		}
		usb_HDDREGFlag = usb_HDDREGFlag & mask_usb_HDDREGFlag;
		printk(KERN_ERR "Updated usb_HDDREGFlag = 0x%x\n", usb_HDDREGFlag);
		printk(KERN_ERR "[USB] PM_SUSPEND_PREPARE done\n");
        #else
		for(bPortNum=0; bPortNum< MUC_NUM_MAX_CONTROLLER; bPortNum++)
		{
		#ifdef USB_DEVICE_MODE_ENABLE_5V
			if(bPortNum==MUSB_WIFI_PORT)
			{
				printk(KERN_ERR "[USB] skip wifi port %d vbus control \n",bPortNum);
				continue;
			}
		#else
			if(((ssusb_adb_flag == 1) && (bPortNum == ssusb_adb_port))||(bPortNum==MUSB_WIFI_PORT))
				{
					printk(KERN_ERR "[USB] skip adb&wifi port %d vbus control \n",bPortNum);
					continue;
				}
		#endif
			if((bPortNum == 0)||(bPortNum == 1))
			{
				pController = MUCUST_GetControllerDev(bPortNum);
				...
				{
					bPortInsertFlag |= (pController->pThis->bInsert)<<bPortNum;
					printk(KERN_ERR "bPortInsertFlag = 0x%x  \n",bPortInsertFlag);
				}
			}
			if (time_after(jiffies, disconnect_done_time))
			{
				bUnplugToSuspendWaitFlag = 0;
				printk(KERN_ERR"USB device disconnect and wait time enough to suspend\n");
			}
			else
			{
				bUnplugToSuspendWaitFlag = 1;
				printk(KERN_ERR"USB device disconnect and wait time too short to suspend\n");
			}
			#if defined(CONFIG_ARCH_MT5886)
			#if defined(CC_USB_PHY_SWITCH)
                if((bPortNum !=1)&&(MUC_aPwrGpio[bPortNum] != -1))
			#else
				if((bPortNum !=2)&&(MUC_aPwrGpio[bPortNum] != -1))
			#endif
			#else
				if((MUC_aPwrGpio[bPortNum] != -1))
			#endif
		...
		usb_hotboot = 1;
		#endif
       printk("bPortInsertFlag = 0x%x\n",bPortInsertFlag);
	   #if defined(CONFIG_ARCH_MT5893)
       if((bPortInsertFlag)||((!bPortInsertFlag)&&(bUnplugToSuspendWaitFlag))||(xhci_connect_flag))
	   	#else
		//if((bPortInsertFlag)||((!bPortInsertFlag)&&(bUnplugToSuspendWaitFlag)))
	   #endif
		          msleep(mgc_suspend_delay); //wait for finish the usb device disconnect related action. //wifi need 450ms to finish disconnect
		...
		#ifndef USB_DEVICE_MODE_ENABLE_5V
			if(((ssusb_adb_flag == 1) && (bPortNum == ssusb_adb_port))||(bPortNum==MUSB_WIFI_PORT))
				continue;
		#endif
#if defined(CC_S_PLATFORM)&&(defined(CC_OX_CUST)||defined(CC_WK_CUST)||defined(CC_HK_CUST))
	       // Don't need to TurnOn VBUS under three condition
               // CaseB && with device insert && indicate Portx VBUS off
               if((usb_HDDREGFlag & (1<<bPortNum)) && (!(usb_HDDREGFlag & (1<<(bPortNum+8))))) {
                 printk(KERN_ERR "[USB] Don't need to TurnOn VBUS Port[%d] GPIO[%d] usb_HDDREGFlag[0x%x] \n",bPortNum,MUC_aPwrGpio[bPortNum],usb_HDDREGFlag);
               }
               else
#endif
               {
	        ...
	        #if defined(CC_S_PLATFORM)&&defined(CC_PHX_CUST)
                mtk_gpio_set_value(WIFI_RST_GPIO,1);
                printk(KERN_ERR "[USB] Set WIFI reset GPIO to 1\n");
            #endif
		printk(KERN_ERR "[USB] PM_POST_SUSPEND\n");
        // For Linux system, usb driver will resume the delay_resume module at the post_suspend stage.
 		// For Android system, usb driver will resume the delay_resume moudle at later resume stage.
 		// For Android system PVR case, resume delay_resume modult at post_suspend stage.
 		// Only Android system will define CONFIG_EARLYSUSPEND
#ifndef CONFIG_EARLYSUSPEND
		//if (pm_delayresume_enabled)
		{
			dpm_delay_resume(PMSG_RESUME);
		}
#endif
	}
...
static void mtk_usb_early_suspend(struct early_suspend *h)
{
	printk(KERN_ERR "[USB] Early suspend process \n");
	return;
}

/* tell userspace to start drawing */
static void mtk_usb_later_resume(struct early_suspend *h)
{
	printk(KERN_ERR "[USB] Later resume process \n");
	dpm_delay_resume(PMSG_RESUME);
	return;
}

static struct early_suspend mtk_usb_early_suspend_desc = {
	.level = 0,
	.suspend = mtk_usb_early_suspend,
	.resume = mtk_usb_later_resume,
};
...
static void mgc_save_context(MGC_LinuxCd *pThis)
{
	...
	pThis->context.dmamask = MGC_Read8(pBase, MGC_O_HSDMA_INTR_MASK);
        printk(KERN_ERR "[usb_dbg] mgc_save_context: M_REG_INTRLEVEL1EN = 0x%x\n",pThis->context.levelen);
	return;
}
...
static void mgc_restore_context(MGC_LinuxCd *pThis)
{
	...
	MGC_Write8(pBase, MGC_O_HDRC_DEVCTL,pThis->context.devctl);
        if ((0 == pThis->context.levelen) && (VBUS_ON == bPowerStatus[MGC_GetPortNum(pBase)]))
        {
#ifdef CONFIG_USB_QMU_SUPPORT
                            pThis->context.levelen |= 0x2f;
#else
                            pThis->context.levelen |= 0x0f;
#endif
        }
	...
	MGC_Write8(pBase,MGC_O_HSDMA_INTR_UNMASK_SET,pThis->context.dmamask);
        printk(KERN_ERR "[usb_dbg] mgc_restore_context: M_REG_INTRLEVEL1EN = 0x%x\n",pThis->context.levelen);
	// for DMA
...
static int mgc_suspend(struct device *dev)
{
    ...
    defined(CONFIG_ARCH_MT5863)||defined(CONFIG_ARCH_MT5893)||defined(CONFIG_ARCH_MT5865) ||\
    defined(CC_USB_PHY_SWITCH))
	uint32_t u4Reg = 0;
	uint8_t bReg = 0;
#endif
...
#if defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5893)
if (pThis->bPortNum == 3) {
	...
	if (pThis->bPortNum == 1) {
		...
		bReg |= MGC_M_POWER_SUSPENDM;
		MGC_Write8(pThis->pRegs, MGC_O_HDRC_POWER, bReg);
		/* Switch Port-B MAC clock to xtal clock 24M  */
		INFO("Switch standby MAC clock to xtal clock 24M\n");
		u4Reg = MGC_PHY_Read32(pUSBpdwncIoMap, 0xd8);
		u4Reg &= ~(0xe000);
		u4Reg |= (0x4000);
		MGC_PHY_Write32(pUSBpdwncIoMap, 0xd8, u4Reg);
		/* Switch Port-B to standby power domain  */
		INFO("Switch usb rst to pdwnc rst");
		u4Reg = MGC_PHY_Read32(pUSBpdwncIoMap, 0xd8);
		u4Reg |= (0x1 << 31);
		MGC_PHY_Write32(pUSBpdwncIoMap, 0xd8, u4Reg);

		u4Reg = MGC_PHY_Read32(pUSBpdwncIoMap, 0xe4);
		u4Reg |= (0x1 << 31);
		MGC_PHY_Write32(pUSBpdwncIoMap, 0xe4, u4Reg);
	} else
#endif
        ...
        if (pThis->bPortNum == 1) {
            ...
            INFO("Switch usb rst to pdwnc rst");
            u4Reg = MGC_PHY_Read32(pUSBpdwncIoMap, 0xdc);
            u4Reg |= (0x1 << 31);
            MGC_PHY_Write32(pUSBpdwncIoMap, 0xdc, u4Reg);

            INFO("Switch standby MAC clock to ref clock\n");
            u4Reg = MGC_PHY_Read32(pUSBpdwncIoMap, 0xe0);
            u4Reg |= (0x02);
            MGC_PHY_Write32(pUSBpdwncIoMap, 0xe0, u4Reg);
        } else
...
#if defined(CC_USB_PHY_SWITCH)
	if (pThis->bPortNum == 1) {
		/* Switch Port-B to core power domain  */
		INFO("Re-Switch Port-B ip rst\n");
		u4Reg = MGC_PHY_Read32(pUSBpdwncIoMap, 0xd8);
		u4Reg &= ~(0x1 << 31);
		MGC_PHY_Write32(pUSBpdwncIoMap, 0xd8, u4Reg);
		...
		/* Re-Switch Port-B MAC clock to 120M  */
		INFO("Re-Switch standby MAC clock to 120M clock\n");
		u4Reg = MGC_PHY_Read32(pUSBpdwncIoMap, 0xd8);
		u4Reg &= ~(0xe000);
		MGC_PHY_Write32(pUSBpdwncIoMap, 0xd8, u4Reg);
...
static void usb_events_timer_func (unsigned long _pThis)
{
...
#ifdef SUPPORT_WIRELESS_HCI_DUMP
    if(pThis->events & USB_EVENTS_WIRELESS_DUMP)
    {
      MGC_WirelessEventDump();
      pThis->events &= ~USB_EVENTS_WIRELESS_DUMP;
    }
#endif
	...
	if(pThis->events & USB_EVENTS_VBUS_ON)
	{
		...
		printk(KERN_ERR " [usb_dbg]USB_EVENTS_VBUS_ON\n");
    ...
    if(pThis->events & USB_EVENTS_HOST_SESSION_ON)
    {
        ...
        if (pController->MGC_pfVbusControl)
        {
            ...
            printk(KERN_ERR "[usb_dbg]wait 2000ms to turnon vbus\n");
...
static void MGC_WirelessEventDump(void)
{
    int event_index = 0;
    int i=0;
    printk("[%s][%d] --------- Start wireless event dump -------- \n",__FUNCTION__,__LINE__);
    for(event_index=0; event_index<HCI_DUMP_ENTRY_NUM; event_index++ )
    {
        printk("dump_index = 0x%x actual_length = 0x%x transfer_buffer_length = 0x%x direction = %s\n",
                wireless_event_dump[event_index].dump_index,
                wireless_event_dump[event_index].actual_length,
                wireless_event_dump[event_index].transfer_buffer_length,
                wireless_event_dump[event_index].direction?"CMD":"EVENT");
        for(i=0; i < HCI_DUMP_BUF_SIZE && i < wireless_event_dump[event_index].actual_length ;i++)
            printk("0x%02x ",wireless_event_dump[event_index].transfer_data[i]);
        printk("\n");
    }
...
static MGC_LinuxLocalEnd *MGC_FindEnd(MGC_LinuxCd *pThis, struct urb *pUrb)
{
	...
	if(((pDescriptor->idVendor==0x046d) &&(pDescriptor->idProduct==0xc21f))
		|| ((pDescriptor->idVendor==0x046d) &&(pDescriptor->idProduct==0xc22f)))
	{
		Is_Logicool_Pad=1;
		Is_Logicool_Pad_port = pThis->bPortNum;
		printk(KERN_ERR "Logicool pad insert in port[%d], set Is_Logicool_Pad = 1.\n",Is_Logicool_Pad_port);
	}
	...
	if(MGC_WIFIHotbootSupported(pUrb))
	{
	  ...
	  if(!udriver)
	  {
	    INFO("udriver is NULL\n");
	  }
	  else if(!(udriver->name))
	  {
	    INFO("udriver->name is NULL\n");
	  }
	  else {
	    INFO("udriver->name = %s\n", udriver->name);
	  }
	  ...
	  {
	    INFO("dev->dev.driver is  NULL\n");
	  }

#ifndef USB_FC_SUPPORT_V2
	  if(mtk_get_usb_suspend()&&(udriver)
	...
	INFO("Port-%d: %s EP %d  Get Fifo Addr = 0x%x, size = 0x%x\n",
...
#ifdef SUPPORT_WIRELESS_HCI_DUMP
    if(MGC_WIFIHotbootSupported(pUrb) && (pUrb->transfer_buffer_length != 0) && ((usb_pipein(pUrb->pipe) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==1)) || ((usb_pipeout(pUrb->pipe)) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==0))))
    {
        for(; event_index<HCI_DUMP_ENTRY_NUM; event_index++ )
        {
            memset(&wireless_event_dump[event_index], 0, sizeof(struct wireless_event));
            wireless_event_dump[event_index].dump_index = event_index;
        }

        if(event_index == HCI_DUMP_ENTRY_NUM)
        {
            DBG(2,"pUrb->transfer_buffer_length = %d buff = 0x%p address = 0x%lx\n virt_address = 0x%lx address = 0x%lx\n",pUrb->transfer_buffer_length,wireless_event_dump[event_index].transfer_data,
                    ((ulong)pUrb->transfer_dma),
                    __bus_to_virt((ulong)(pUrb->transfer_dma)),
                    ((ulong)pUrb->transfer_buffer));

            DBG(2,"pUrb->actual_length=0x%x pUrb->transfer_buffer_length=0x%x \n",pUrb->actual_length,pUrb->transfer_buffer_length);

            for(i=0;i<HCI_DUMP_ENTRY_NUM;i++)
            {
                    DBG(1,"i = %d  wireless_event_dump[i].dump_index = %d \n",i,wireless_event_dump[i].dump_index);

                    if(wireless_event_dump[i].dump_index == 0)
                    {
                            memset(&wireless_event_dump[i], 0, sizeof(struct wireless_event));

                            wireless_event_dump[i].dump_index = HCI_DUMP_ENTRY_NUM;        // index this is the latest data
                            wireless_event_dump[i].actual_length = pUrb->actual_length;
                            wireless_event_dump[i].transfer_buffer_length = pUrb->transfer_buffer_length;
                            if ( usb_pipein(pUrb->pipe) && ((uint8_t)usb_pipeendpoint(pUrb->pipe)==1) )
                                wireless_event_dump[i].direction = 0;
                            else
                                wireless_event_dump[i].direction = 1;

                            if((pUrb->transfer_buffer_length <= MGC_HSDMA_MIN_DMA_LEN)||(pUrb->transfer_dma==0))
                            {
                                for(pi=0; pi< pUrb->transfer_buffer_length;pi++)
                                    DBG(2,"0x%x \n", *(char *)(((ulong)pUrb->transfer_buffer+pi)));
                                memcpy((void *)wireless_event_dump[i].transfer_data, (const void *)((ulong)pUrb->transfer_buffer), HCI_DUMP_BUF_SIZE);
                            }
                            else
                            {
                                for(pi=0; pi< pUrb->transfer_buffer_length;pi++)
                                    DBG(2,"- 0x%x \n", *(char *)(__bus_to_virt((ulong)pUrb->transfer_dma+pi)));
                                memcpy(wireless_event_dump[i].transfer_data, (char *)__bus_to_virt((ulong)pUrb->transfer_dma), HCI_DUMP_BUF_SIZE);
                            }
                            //memcpy((void *)&wireless_event_dump[i].transfer_data[0], (const void *)pTranBuffer+pUrb->transfer_buffer_length-HCI_DUMP_BUF_SIZE, HCI_DUMP_BUF_SIZE);

                        DBG(2,"i = 0x%x dump_index = 0x%x actual_length = 0x%x transfer_buffer_length = 0x%x\n",
                            i,wireless_event_dump[i].dump_index,wireless_event_dump[i].actual_length,\
                            wireless_event_dump[i].transfer_buffer_length);
                        for(pi=0; pi< HCI_DUMP_BUF_SIZE;pi++)
                            DBG(2,"0x%x \n",wireless_event_dump[i].transfer_data[pi]);

                    }
            }

            for(j=0;j<HCI_DUMP_ENTRY_NUM;j++)
            {
                wireless_event_dump[j].dump_index--;
            }
...
static void MGC_KickCmdQ(MGC_LinuxCd *pThis, MGC_LinuxLocalEnd *pEnd, struct urb *pUrb)
{
    ...
    {
#ifndef CONFIG_USB_MTK_MUSB_QMU_BULK_RX_AUTO_SPLIT
        ...
        */
		DBG(2,"pUrb->transfer_buffer_length : %d\n",pUrb->transfer_buffer_length);
		...
		pUrb->error_count = URB_IN_GPD;		// indicate that the urb has inserted into GPD

#else
		DBG(3, "[%s][%d] pUrb=0x%p pUrb->transfer_buffer_length = 0x%x QMU_RX_SPLIT_THRE = 0x%x \n",__FUNCTION__,__LINE__,pUrb,pUrb->transfer_buffer_length,QMU_RX_SPLIT_THRE);
		/* Must be the bulk transfer type */
        pBuffer_addr = pUrb->transfer_dma;
		//reuse isoc urb->number_of_packets
		if((pUrb->transfer_buffer_length < QMU_RX_SPLIT_THRE)/*| usb_pipeout(pUrb->pipe)*/)
			{
			dwLength = pUrb->transfer_buffer_length;
			bIsIoc = TRUE;
			if (QMU_insert_task(pThis, bEnd, !bXmt, pBuffer_addr, dwLength, bIsIoc) < 0)
			{
				INFO("[USB] Insert Task Error !\n");
				return; //jgao@ fix  N/A->-1
			}
			pUrb->error_count = URB_IN_GPD; 	// indicate that the urb has inserted into GPD
		}else{
			//reuse isoc urb->unmber_of_packets
			pUrb->number_of_packets = ((pUrb->transfer_buffer_length) +QMU_RX_SPLIT_BLOCK_SIZE-1)/ (QMU_RX_SPLIT_BLOCK_SIZE);
			DBG(1,"QMU RX auto split,urb->transfer_buffer_length : %d, urb->number_of_packets :%d\n",pUrb->transfer_buffer_length,pUrb->number_of_packets);
			for(i=0; i<pUrb->number_of_packets; i++)
			{
				...
				/* If interrupt on complete ? */
				bIsIoc = (i == (pUrb->number_of_packets-1)) ? TRUE : FALSE;
				dwLength = (i == (pUrb->number_of_packets-1)) ? ((pUrb->transfer_buffer_length) %QMU_RX_SPLIT_BLOCK_SIZE) : dwLength;
				if(dwLength==0)
					dwLength = QMU_RX_SPLIT_BLOCK_SIZE;
				if (QMU_insert_task(pThis, bEnd, !bXmt,
								  pBuffer_addr+offset, dwLength, bIsIoc) < 0)
...
void MGC_KickUrb(MGC_LinuxCd *pThis, struct urb *pUrb, MGC_LinuxLocalEnd *pEnd)
{
    ...
    //pUrb->error_count = 0;
    pUrb->error_count = URB_NOT_IN_GPD;	// indicate that the urb not be inserted into GPD
...
static int MGC_ScheduleUrb(MGC_LinuxCd *pThis, MGC_LinuxLocalEnd *pEnd, struct urb *pUrb)
{
    ...
    DBG(3, "[USB]queued URB pUrb=0x%p (current %p) to end %d (bRemoteAddress=%d, bRemoteEnd=%d proto=%d) pEnd->bBusyCompleting=%d\n",
...
#ifdef CONFIG_USB_QMU_SUPPORT
	DBG(3, "[%s][%d] SupportQmu = %d pEnd->bStateGated = %d idle = %d\n",__FUNCTION__,__LINE__,(MGC_IsCmdQSupport(pThis, pEnd, pUrb)),pEnd->bStateGated,idle);
    if (MGC_IsCmdQSupport(pThis, pEnd, pUrb)
        && (!pEnd->bStateGated))
    {
...
static void MGC_CheckConnect(unsigned long pParam)
{
    ...
    if (pThis->bPortNum == 0)
    {
      ...
      dwReg = MGC_PHY_Read32(pPhyBase,M_REG_PHYACR5); //read M_REG_U2PHYACR5(0x24)
      dwReg &= ~0x1;
...
static irqreturn_t MUC_Irq(struct usb_hcd *hcd)
{
    ...
    if (bIntrUsbValue &MGC_M_INTR_CONNECT)
    {
        ...
        INFO("pThis->bInsert = %d, bPowerStatus[%d] = %d.\n", pThis->bInsert, bPort, bPowerStatus[bPort]);

       if ((dwLineState != LINESTATE_DISCONNECT)&&(bPowerStatus[bPort]==VBUS_ON))
        {
            pThis->bCheckInsert = 0;
    ...
    // disconnect event.
    {
        ...
        {
            pThis->bInsert = FALSE;
           /* if(bPort != MUSB_WIFI_PORT)
            {
                  disconnect_done_time = jiffies + msecs_to_jiffies(2000);
            }*/
        }
        #ifdef MGC_UNLINK_URB
	...
	if (pThis->bPortNum == 0)
	{
	  ...
	  //pull up RG_CHGDT_EN
	  u4Reg = MGC_PHY_Read32(pThis->pPhyBase,M_REG_PHYACR5); //read  M_REG_U2PHYACR5(0x24)
	  u4Reg |= 0x1;
    ...
    if (hep && hep->hcpriv)
    {
		...
		pEnd->wFIFOSize = 0;
                if (pThis->bPortNum !=MUSB_WIFI_PORT)
                {
                    ...
                    MGC_WriteCsr16((uint8_t*)pThis->pRegs, MGC_O_HDRC_TXCSR, pEnd->bEnd, MGC_M_TXCSR_FLUSHFIFO | MGC_M_TXCSR_TXPKTRDY);
                    MGC_WriteCsr16((uint8_t*)pThis->pRegs, MGC_O_HDRC_RXCSR, pEnd->bEnd, MGC_M_RXCSR_FLUSHFIFO | MGC_M_RXCSR_CLRDATATOG |MGC_M_RXCSR_RXPKTRDY);
                    MGC_WriteCsr16((uint8_t*)pThis->pRegs, MGC_O_HDRC_RXCSR, pEnd->bEnd, MGC_M_RXCSR_FLUSHFIFO | MGC_M_RXCSR_CLRDATATOG |MGC_M_RXCSR_RXPKTRDY);
                    printk(KERN_ERR " flush fifo port = [%d]\n",pThis->bPortNum);
                }
            }
...
#if defined(CONFIG_ARCH_MT5863) ||defined(CONFIG_ARCH_MT5865)
static void MUC_poweron_mac(void)     //power on crax usb port2 MAC or puppy usb port1
{
	uint32_t u4Reg;
	void __iomem *pUSBpdwncMapBase = pUSBpdwncIoMap;
#if defined(CONFIG_ARCH_MT5863)
    INFO("power on  U2 port mac on mt5863\n");
#else
    INFO("power on  U1 port mac on mt5865\n");
#endif
	u4Reg = MGC_PHY_Read32(pUSBpdwncMapBase, 0xe8);  //open mac power
	u4Reg |= (0x3 << 0);
	MGC_PHY_Write32(pUSBpdwncMapBase, 0xe8, u4Reg);

	u4Reg = MGC_PHY_Read32(pUSBpdwncMapBase, 0xe8);  //open sram power
	u4Reg &= ~(0x1 << 7);
	MGC_PHY_Write32(pUSBpdwncMapBase, 0xe8, u4Reg);

    u4Reg = MGC_PHY_Read32(pUSBpdwncMapBase, 0xe8);  //open clock enable
    u4Reg &= ~(0x1 << 3);
    MGC_PHY_Write32(pUSBpdwncMapBase, 0xe8, u4Reg);

	u4Reg = MGC_PHY_Read32(pUSBpdwncMapBase, 0xe8);  //shut down ISO
	u4Reg &= ~(0x1 << 4);
	MGC_PHY_Write32(pUSBpdwncMapBase, 0xe8, u4Reg);

	u4Reg = MGC_PHY_Read32(pUSBpdwncMapBase, 0xe8);  //release MAC reset
	u4Reg |= (0x1 << 2);
	MGC_PHY_Write32(pUSBpdwncMapBase, 0xe8, u4Reg);
...
static int MUC_hcd_probe(struct platform_device *pdev)
{
    ...
    if(pThis->pPhyBase){
        ...
        pThis->events |= USB_EVENTS_HOST_SESSION_ON;
        printk(KERN_ERR "[usb_dbg] USB_EVENTS_HOST_SESSION_ON\n");
    }
...
#define MTK_HCD_DMA_MASK 0xffffffffffffffff
#else
#define MTK_HCD_DMA_MASK 0xffffffff
#endif
#endif

bool is_user_resume_host(MUSB_LinuxController *data)
{
    if (data->is_delayresume)
    	{
        return true;
    	}
    return false;
...
static int __init MUC_init(void)
{
...
#if !defined(CONFIG_ARCH_MT5865)    //puppy have 1 normal port , have no local Arbiter control register
...
#if defined(CONFIG_ARCH_MT5863) ||defined(CONFIG_ARCH_MT5865)
	pUSBphyIoMap2 = of_iomap(np0,6);
#endif
#if defined(CONFIG_ARCH_MT5886) && defined(CC_USB_PHY_SWITCH)
	pUSBphyIoMapP1 = of_iomap(np0,6);
#endif
#if defined(CONFIG_ARCH_MT5886)||defined(CONFIG_ARCH_MT5863)||defined(CONFIG_ARCH_MT5893)
	pUSBphyIoMapEfuse = of_iomap(np0,7);
#endif
#else
...
#if defined(CONFIG_ARCH_MT5863) ||defined(CONFIG_ARCH_MT5865)
	MUC_poweron_mac(); //power on crax usb port2 MAC or puppy usb port1
#endif
    if (usb_disabled())
    {
        return USB_ST_REMOVED;
...
#elif defined(CONFIG_ARCH_MT5863)
		INFO("mgc MT5863 controller info reinitial \n");
		MUC_aLinuxController[1].bEndpoint_Rx_num = 13;
		MUC_aLinuxController[1].bEndpoint_Tx_num = 13;
#endif

	INFO("USB Port[%d] ADB enable = %d\n",ssusb_adb_port,ssusb_adb_flag);
	nCount = MUCUST_GetControllerNum();

    if(ssusb_adb_port > nCount)
        ssusb_adb_port = 0;

    printk("======== MUC_init USB port Common USB Driver*************=====\n");
    #ifdef CONFIG_USB_DELAY_RESUME_SUPPORT
    printk("MUC init: usb port delay resume flag %d %d %d\n",
    MUC_aDelayResumeUsing[0], MUC_aDelayResumeUsing[1], MUC_aDelayResumeUsing[2]);
    #endif
    ...
    for (i = 0; i < nCount; i++)
    {
	    ...
	    {
	     #ifdef CONFIG_USB_DELAY_RESUME_SUPPORT
			MUC_aLinuxController[i].is_delayresume=MUC_aDelayResumeUsing[i];
			INFO("[USB]Port[%d] Delay resume flag is %d\n",i,MUC_aLinuxController[i].is_delayresume);
         #endif
            pController = MUCUST_GetControllerDev(i);
            if (!pController)
            {
                return USB_ST_REMOVED;
            }
			...
			#endif
            pPlatformDev->dev.power.is_delayresume = is_user_resume_host(pController);
...
#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&mtk_usb_early_suspend_desc);
#endif
#endif
...
#if defined(CONFIG_PM) || defined(CC_SUPPORT_STR)
#ifdef CONFIG_EARLYSUSPEND
        unregister_early_suspend(&mtk_usb_early_suspend_desc);
#endif
