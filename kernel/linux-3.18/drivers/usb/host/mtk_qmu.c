...
static void  QMU_flush_q(MGC_LinuxCd *pThis, uint8_t bEnd, uint8_t isRx)
{
    ...
    if (isRx)
    {
        ...
        MGC_WriteCsr16(pBase, MGC_O_HDRC_RXCSR, bEnd, csr | MGC_M_RXCSR_CLRDATATOG);

        /* write 2x to allow double buffering */
        //CC: see if some check is necessary
        //MGC_WriteCsr16(pBase, MGC_O_HDRC_RXCSR, bEnd, MGC_M_RXCSR_FLUSHFIFO|MGC_M_RXCSR_CLRDATATOG|MUSB_RXCSR_RXPKTRDY);
        //MGC_WriteCsr16(pBase, MGC_O_HDRC_RXCSR, bEnd, MGC_M_RXCSR_FLUSHFIFO|MGC_M_RXCSR_CLRDATATOG|MUSB_RXCSR_RXPKTRDY);
    ...
    else
    {
        ...
        MGC_WriteCsr16(pBase, MGC_O_HDRC_TXCSR, bEnd, csr | MGC_M_TXCSR_CLRDATATOG);
...
static int QMU_set_gpd(MGC_LinuxCd *pThis, uint32_t pDataBuf_addr, uint32_t data_len, uint8_t EP_Num, uint8_t isIOC, uint8_t IsRx)
{
	...
	DBG(2,"[%s][%d] pWGpd = 0x%p pBuf = 0x%p pWGpd->pBuf_addr = 0x%x Buf_GetWritePhyAddr(pBuf) = 0x%x \n",
    ...
    if (isIOC)
    {
        TGPD_SET_IOC(pWGpd);		//  Gpd->flag = 1 means that this Gpd will trigger interrupt, else won't. The pre-condition is IOCDISR = 0
    }
    ...
    */
	...
	DBG(5, "GPD Info flag=0x%x chksum=0x%x DataBufferLen=0x%x bufLen=0x%x ExtLength=0x%x ZTepFlag=0x%x\n",
		pWGpd->flag,pWGpd->chksum,pWGpd->DataBufferLen,pWGpd->bufLen,pWGpd->ExtLength,pWGpd->ZTepFlag);
...
{   
    ...
    for (i=0; i<RXQ_NUM; i++)
    {
        ...
        pBuf->pLogicalBuf = 
            (void *)kzalloc((GPD_LEN_ALIGNED *MAX_URB_NUM), GFP_KERNEL |GFP_DMA);
...
int QMU_insert_task(MGC_LinuxCd *pThis, uint8_t EP_Num, uint8_t isRx, uint32_t buf_addr, uint32_t length, uint8_t isIOC)
{
    ...
    if (status == 0)
    {
		DBG(5,"[%s][%d] current GPD = 0x%x \n",__FUNCTION__,__LINE__,(MGC_ReadQMU32(base, MGC_O_QMU_RQCPR(EP_Num))));
    ...
    {        
        ...
        if (!MGC_IsEndIdle(pEnd))
        {
            if ((pEnd->bCmdQEnable) && (!pEnd->bStateGated))
            {
				...
				(void)MGC_GetNextUrb(pEnd);   
				pUrb = MGC_GetCurrentUrb(pEnd);
				if(pUrb && (pUrb->error_count == URB_NOT_IN_GPD))
...
wait_rx_ioc:
                    ...
                    {
						DBG(3, "pUrb=0x%p GPD Info flag=0x%x chksum=0x%x DataBufferLen=0x%x bufLen=0x%x ExtLength=0x%x ZTepFlag=0x%x\n",
							pUrb,pRGpd->flag,pRGpd->chksum,pRGpd->DataBufferLen,pRGpd->bufLen,pRGpd->ExtLength,pRGpd->ZTepFlag);
                        ...
                        pUrb->status = 0;
#else 
						 if(pUrb->transfer_buffer_length < QMU_RX_SPLIT_THRE)
						 	{
							 pUrb->actual_length = TGPD_GET_BUF_LEN(pRGpd);
							 pUrb->status = 0;
							 done = true;
						 }else{
							 //printk(KERN_ALERT "sdu->actual_length = %d,qh->iso_idx %d \n", sdu_result->actual_length,qh->iso_idx);
							 pUrb->actual_length += TGPD_GET_BUF_LEN(pRGpd);
							 DBG(2,"pUrb=0x%p GPD Info flag=0x%x chksum=0x%x DataBufferLen=0x%x bufLen=0x%x ExtLength=0x%x ZTepFlag=0x%x\n",
								 pUrb,pRGpd->flag,pRGpd->chksum,pRGpd->DataBufferLen,pRGpd->bufLen,pRGpd->ExtLength,pRGpd->ZTepFlag);
							if(--pUrb->number_of_packets == 0)
								{
								done = true;
								pUrb->status = 0;
							}
                    ...
                    if(done)
                	{
						...
						DBG(2, "[%s][%d] pEnd->bEnd = %d pUrb = 0x%p MGC_IsEndIdle(pEnd) = %d \n",__FUNCTION__,__LINE__,pEnd->bEnd,pUrb,MGC_IsEndIdle(pEnd));
                        if (!MGC_IsEndIdle(pEnd))
                        {
                            if ((pEnd->bCmdQEnable) && (!pEnd->bStateGated))
                            {
