...
int mtk_xhci_scheduler_add_ep(int dev_speed, int is_in, int isTT, int ep_type, int maxp, int interval, int burst
	...
	printk(KERN_ERR "add_ep parameters, dev_speed %d, is_in %d, isTT %d, ep_type %d, maxp %d, interval %d, burst %d, mult %d, ep 0x%x, ep_ctx 0x%x, sch_ep 0x%x\n", dev_speed, is_in, isTT, ep_type, maxp
		, interval, burst, mult, (unsigned int)ep, (unsigned int)ep_ctx, (unsigned int)sch_ep);
	if(isTT && ep_type == USB_EP_INT && ((dev_speed == USB_SPEED_LOW) || (dev_speed == USB_SPEED_FULL))){
		...
		ret = SCH_SUCCESS;
		if(sch_ep != NULL){
			printk(KERN_ERR "[Xhci]free the sch_ep for FULL/LOW speed INT\n");
			kfree(sch_ep);
			sch_ep = NULL;
		}
	}
		...
		ret = SCH_SUCCESS;
		if(sch_ep != NULL){
			printk(KERN_ERR "[Xhci]free the sch_ep for FULL speed ISOC\n");
			kfree(sch_ep);
			sch_ep = NULL;
		}
	}
		...
		ret = SCH_SUCCESS;
		if(sch_ep != NULL){
			printk(KERN_ERR "[Xhci]free the sch_ep for BULK/CONTROL\n");
			kfree(sch_ep);
			sch_ep = NULL;
		}
	}
