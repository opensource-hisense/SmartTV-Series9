					...
					unsigned int cycle_state, gfp_t flags)
{
	...
	if (!seg)
		return NULL;
	xhci_mem(xhci, "Allocating priv segment structure at %p\n", seg);
	...
	xhci_mem(xhci, "// Allocating segment at %p (virtual) 0x%llx (DMA)\n",
...
static void xhci_segment_free(struct xhci_hcd *xhci, struct xhci_segment *seg)
{
	...
	if (seg->trbs) {
		xhci_mem(xhci, "Freeing DMA segment at %p (virtual) 0x%llx (DMA)\n",
				seg->trbs, (unsigned long long)seg->dma);
		dma_pool_free(xhci->segment_pool, seg->trbs, seg->dma);
		seg->trbs = NULL;
	}
	xhci_mem(xhci, "Freeing priv segment structure at %p\n", seg);
	kfree(seg);
}
...
int xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags)
{
    ...
    #if 0
    temp = readl(&xhci->u3mac_regs->ltssm_ctrl);
    temp &= ~COMP_EN;
    writel(temp, &xhci->u3mac_regs->ltssm_ctrl);
    #endif
