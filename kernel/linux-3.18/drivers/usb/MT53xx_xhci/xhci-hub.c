...
unsigned int xhci_connect_flag = 0;

EXPORT_SYMBOL_GPL(xhci_connect_flag);
#endif
		...
		u16 wIndex, char *buf, u16 wLength)
{
	...
	spin_lock_irqsave(&xhci->lock, flags);
	xhci_dbg(xhci, "xhci_hub_control: typeReq = 0x%x\n", typeReq);
	switch (typeReq) {
	case GetHubStatus:
...
int xhci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	if (!status && !reset_change) {
		...
		xhci_dbg(xhci, "%s: status %d reset_change %d.\n", __func__, status, reset_change);
	}
	spin_unlock_irqrestore(&xhci->lock, flags);
