...
void mtu3_device_reset(struct mtu3 *mtu)
{
	void __iomem *ibase = mtu->ippc_base;
...
static void get_ep_fifo_config(struct mtu3 *mtu)
{
	...
	if (mtu->is_u3_ip) {
		fifosize = mtu3_readl(mbase, U3D_CAP_EPNTXFFSZ);
		tx_fifo = &mtu->tx_fifo;
		tx_fifo->base = 0;
		tx_fifo->limit = fifosize / MTU3_EP_FIFO_UNIT;
		bitmap_zero(tx_fifo->bitmap, MTU3_FIFO_BIT_SIZE);

		fifosize = mtu3_readl(mbase, U3D_CAP_EPNRXFFSZ);
		rx_fifo = &mtu->rx_fifo;
		rx_fifo->base = 0;
...
void mtu3_ep0_setup(struct mtu3 *mtu)
{
	...
	mtu3_writel(mbase, U3D_EPIESR, EP0ISR);
}
...
void mtu3_regs_init(struct mtu3 *mtu)
{
	...
	/* vbus detected by HW */
	mtu3_setbits(mbase, U3D_MISC_CTRL, VBUS_FRC_EN | VBUS_ON); //change vbus for mt53xx 
}
...
int mtu3_hw_init(struct mtu3 *mtu)
{
	void __iomem *ibase = mtu->ippc_base;
	u32 cap_dev;
	int ret;

	mtu->hw_version = mtu3_readl(ibase, U3D_SSUSB_HW_ID);

	cap_dev = mtu3_readl(ibase, U3D_SSUSB_IP_DEV_CAP);
...
int ssusb_gadget_init(struct ssusb_mtk *ssusb)
{
	...
	ssusb->u3d = mtu;
	mtu->ssusb = ssusb;
	mtu->max_speed = USB_SPEED_HIGH;//usb_get_maximum_speed(dev);mt53xx only surport high-speed
