		...
		struct pci_dev *companion, struct usb_hcd *companion_hcd)
{
	...
	if (is_ohci_or_uhci(companion)) {
		if (dev_get_drvdata(&pdev->dev)) {	/* Succeeded */
			mtk_usb_dbg(&pdev->dev, "HS companion for %s\n",
		...
		struct pci_dev *companion, struct usb_hcd *companion_hcd)
{
	if (is_ohci_or_uhci(pdev) && companion->class == CL_EHCI) {
		mtk_usb_dbg(&pdev->dev, "FS/LS companion for %s\n",
				dev_name(&companion->dev));
...
int usb_hcd_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	if (driver->flags & HCD_MEMORY) {
		...
		if (!request_mem_region(hcd->rsrc_start, hcd->rsrc_len,
				driver->description)) {
			mtk_usb_dbg(&dev->dev, "controller already in use\n");
			retval = -EBUSY;
			goto put_hcd;
		}
		hcd->regs = ioremap_nocache(hcd->rsrc_start, hcd->rsrc_len);
		if (hcd->regs == NULL) {
			mtk_usb_dbg(&dev->dev, "error mapping memory\n");
			retval = -EFAULT;
		...
		if (region == PCI_ROM_RESOURCE) {
			mtk_usb_dbg(&dev->dev, "no i/o regions available\n");
			retval = -EBUSY;
			goto put_hcd;
		}
	}
...
static int resume_common(struct device *dev, int event)
{
	...
	if (HCD_RH_RUNNING(hcd) ||
		...
		mtk_usb_dbg(dev, "can't resume, not suspended!\n");
		return 0;
	}
...
static int hcd_pci_suspend_noirq(struct device *dev)
{
	...
	if (retval == -EIO) {		/* Low-power not supported */
		mtk_usb_dbg(dev, "--> PCI D0 legacy\n");
		retval = 0;
	} else if (retval == 0) {
		mtk_usb_dbg(dev, "--> PCI %s\n",
...
static int hcd_pci_runtime_suspend(struct device *dev)
{
	...
	mtk_usb_dbg(dev, "hcd_pci_runtime_suspend: %d\n", retval);
	return retval;
}
...
static int hcd_pci_runtime_resume(struct device *dev)
{
	...
	mtk_usb_dbg(dev, "hcd_pci_runtime_resume: %d\n", retval);
	return retval;
}
