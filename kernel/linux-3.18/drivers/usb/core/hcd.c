...
static int rh_call_control (struct usb_hcd *hcd, struct urb *urb)
{
	...
	case DeviceOutRequest | USB_REQ_SET_ADDRESS:
		...
		mtk_usb_dbg (hcd->self.controller, "root hub device address %d\n",
			wValue);
		break;
	...
	case EndpointOutRequest | USB_REQ_SET_FEATURE:
		mtk_usb_dbg (hcd->self.controller, "no endpoint features yet\n");
		break;
...
error:
	...
	if (status < 0) {
		len = 0;
		if (status != -EPIPE) {
			mtk_usb_dbg (hcd->self.controller,
...
static int rh_queue_status (struct usb_hcd *hcd, struct urb *urb)
{
	...
	if (hcd->status_urb || urb->transfer_buffer_length < len) {
		mtk_usb_dbg (hcd->self.controller, "not queuing rh status urb\n");
		retval = -EINVAL;
		goto done;
	}
...
static int register_root_hub(struct usb_hcd *hcd)
{
	...
	if (retval != sizeof usb_dev->descriptor) {
		mutex_unlock(&usb_bus_list_lock);
		mtk_usb_dbg (parent_dev, "can't read %s device descriptor %d\n",
	...
	if (usb_dev->speed == USB_SPEED_SUPER) {
		...
		if (retval < 0) {
			mutex_unlock(&usb_bus_list_lock);
			mtk_usb_dbg(parent_dev, "can't read %s bos descriptor %d\n",
					dev_name(&usb_dev->dev), retval);
			return retval;
		}
	}
...
int usb_hcd_unlink_urb (struct urb *urb, int status)
{
	...
	if (retval == 0)
		...
		mtk_usb_dbg(&urb->dev->dev, "hcd_unlink_urb %p fail %d\n",
				urb, retval);
	return retval;
}
...
int hcd_bus_suspend(struct usb_device *rhdev, pm_message_t msg)
{
	...
	mtk_usb_dbg(&rhdev->dev, "bus %ssuspend, wakeup %d\n",
			(PMSG_IS_AUTO(msg) ? "auto-" : ""),
			rhdev->do_remote_wakeup);
	if (HCD_DEAD(hcd)) {
		mtk_usb_dbg(&rhdev->dev, "skipped %s of dead bus\n", "suspend");
		return 0;
	}
	...
	if (status == 0) {
		...
		if (rhdev->do_remote_wakeup) {
			...
			if (status != 0) {
				mtk_usb_dbg(&rhdev->dev, "suspend raced with wakeup event\n");
		...
		spin_unlock_irq(&hcd_root_hub_lock);
		mtk_usb_dbg(&rhdev->dev, "bus %s fail, err %d\n",
				"suspend", status);
	}
	return status;
}
...
int hcd_bus_resume(struct usb_device *rhdev, pm_message_t msg)
{
	...
	mtk_usb_dbg(&rhdev->dev, "usb %sresume\n",
			(PMSG_IS_AUTO(msg) ? "auto-" : ""));
	if (HCD_DEAD(hcd)) {
		mtk_usb_dbg(&rhdev->dev, "skipped %s of dead bus\n", "resume");
		return 0;
		...
		struct usb_hcd *primary_hcd)
{
	...
	if (!hcd) {
		mtk_usb_dbg (dev, "hcd alloc failed\n");
		return NULL;
	}
	if (primary_hcd == NULL) {
		...
		if (!hcd->bandwidth_mutex) {
			kfree(hcd);
			mtk_usb_dbg(dev, "hcd bandwidth mutex alloc failed\n");
			return NULL;
		}
		...
		unsigned int irqnum, unsigned long irqflags)
{
	...
	if ((retval = hcd_buffer_create(hcd)) != 0) {
		mtk_usb_dbg(hcd->self.controller, "pool alloc failed\n");
		goto err_create_buf;
	}
	...
	if (device_can_wakeup(hcd->self.controller)
		...
		mtk_usb_dbg(hcd->self.controller, "supports USB remote wakeup\n");
...
void usb_remove_hcd(struct usb_hcd *hcd)
{
	...
	mtk_usb_dbg(hcd->self.controller, "roothub graceful disconnect\n");
	spin_lock_irq (&hcd_root_hub_lock);
	hcd->rh_registered = 0;
	spin_unlock_irq (&hcd_root_hub_lock);
