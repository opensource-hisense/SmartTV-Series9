...
extern int usb_string_id(struct usb_composite_dev *cdev);
extern int usb_add_function(struct usb_configuration *config,struct usb_function *function);
extern void usb_remove_function(struct usb_configuration *c, struct usb_function *f);
extern void usb_put_function(struct usb_function *f);
...
extern struct usb_function *usb_get_function(struct usb_function_instance *fi);
extern int usb_add_config(struct usb_composite_dev *cdev,
		struct usb_configuration *config,
		int (*bind)(struct usb_configuration *));
extern void usb_remove_config(struct usb_composite_dev *cdev,
		      struct usb_configuration *config);
extern int config_ep_by_speed(struct usb_gadget *g,
				  ...
				  struct usb_composite_dev *cdev)
{
	...
	for (; (f = *functions++); index++) {
		...
		printk(KERN_ERR "[USB]%s: f->dev_name = %s, f->name = %s\n", __func__, f->dev_name, f->name);
		/* Added for USB Develpment debug, more log for more debuging help */
		...
		if (f->init) {
			...
			if (err) {
				...
				printk(KERN_ERR "[USB]%s: init %s success!!\n", __func__, f->name);
			}
		}
...
static int __init init(void)
{
	...
	err = android_create_device(dev);
printk(KERN_ERR "android_create_device end %d\n",err);
	if (err) {
		pr_err("%s: failed to create android device %d", __func__, err);
		goto err_create;
	}
