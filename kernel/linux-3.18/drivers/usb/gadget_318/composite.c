		...
		struct usb_function *function)
{
    ...
    {
        printk(KERN_ERR "function usb_interface_id get invalid value\n");
        return -ENODEV;
    }
...
int usb_composite_probe(struct usb_composite_driver *driver)
{
    ...
    if((ssusb_adb_flag == 1) && (2 != ssusb_adb_port))
    {
        printk("Call usb2_gadget_probe_driver\n");
    	return usb2_gadget_probe_driver(gadget_driver);         // For U2 gadget
    }

	printk("Call usb_gadget_probe_driver\n");
	return usb_gadget_probe_driver(gadget_driver);              // For U3 gadget
...
{
	if((ssusb_adb_flag == 1) && (2 != ssusb_adb_port))
    	usb2_gadget_unregister_driver(&driver->gadget_driver);
    else
