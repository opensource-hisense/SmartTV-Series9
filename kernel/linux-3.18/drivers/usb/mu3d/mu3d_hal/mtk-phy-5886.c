...
PHY_INT32 phy_init(struct u3phy_info *info){	
	...
	U3PhyWriteField32(((PHY_UINT32)&info->u3phyd_bank2_regs->b2_phyd_rxdet2)
		, RG_SSUSB_RXDET_STB2_SET_P3_OFST, RG_SSUSB_RXDET_STB2_SET_P3, 0x1A);	//rg_ssusb_rxdet_stb2_set_p3
...
PHY_INT32 eyescan_init(struct u3phy_info *info){
	//initial PHY setting
	U3PhyWriteField32(((PHY_UINT32)&info->u3phya_regs->reg5)
...
//	PHY_INT32 temp;
	...
	if(u4FmOut == 0){
		...
		// set reg = (1024/FM_OUT) * REF_CK * U2_SR_COEF_5886 / 1000 (round to the nearest digits)
		u4Tmp = (((1024 * REF_CK * U2_SR_COEF_5886) / u4FmOut) + 500) / 1000;
