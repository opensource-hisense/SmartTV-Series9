...
DEV_INT32 mu3d_hal_phy_scan(DEV_INT32 latch_val){
	...
	driving = 2;
	u3phy_ops->change_pipe_phase(u3phy, driving, driving);
	phase_val=0;
	count=0;
	fset_phase_val=TRUE;
	...
	while(TRUE){
		...
		if(U0_count>ENTER_U0_TH){//link up
			...
			printk("[PASS] Link Error Count=%d, Recovery Count=%d, PhaseDelay[0x%02x]\n", 
										link_error_count, recov_cnt, phase_val);
			phase_val++;
			fset_phase_val=TRUE;
		}
