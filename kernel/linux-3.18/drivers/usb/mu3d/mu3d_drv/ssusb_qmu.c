...
void qmu_tx_interrupt(struct musb *musb, u8 ep_num){
    ...
    TGPD* gpd_current = (TGPD*)(ulong)(os_readl(USB_QMU_TQCPR(ep_num)));
...
/*
	...
	if(4 == u4times)
	   while(1);
*/
//	mu3d_hal_stop_qmu(ep_num, USB_TX);
    ...
    os_printk(K_DEBUG,"Tx_gpd_last 0x%llu, gpd_current 0x%llu, gpd_end 0x%llu, \r\n",  (DEV_UINT32)gpd, (DEV_UINT32)gpd_current, (DEV_UINT32)Tx_gpd_end[ep_num]);
...
void qmu_rx_interrupt(struct musb *musb, u8 ep_num)
{
	...
	TGPD* gpd_current = (TGPD*)(ulong)(os_readl(USB_QMU_RQCPR(ep_num)));
	TBD*bd; 
...
void qmu_exception_interrupt(void __iomem *base, DEV_UINT32 wQmuVal){
    u32 wErrVal;
	int i = (int)wQmuVal;
