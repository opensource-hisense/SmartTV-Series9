...
int mtk_u3h_phy_init(void)
{
	...
	if(u3phy_ops->u2_slew_rate_calibration){
		printk(KERN_ERR "u2_slew_rate_calibration\n");
...
static int eof_cnt_table[][10] = {
	...
	{77, 19, 27, 10, 0, 4, 0, 2, 68, 0}
	};

static int frame_ref_clk[] = {60, 20, 24, 32, 48, 27};
 ...
 */
static int itp_delta_ratio[] = {0x8, 0x18, 0x14, 0xf, 0xa, 0x12};
...
void setInitialReg(struct device *dev)
{
	...
	printk("\033[0;34m function and line [%s][%d] u3_port_num = 0x%x \033[0m\n",__FUNCTION__,__LINE__,u3_port_num);
	u2_port_num = get_xhci_u2_port_num(dev);
	printk("\033[0;34m function and line [%s][%d] u2_port_num = 0x%x \033[0m\n",__FUNCTION__,__LINE__,u2_port_num);
	...
	if(u3_port_num){
		...
		printk("\033[0;34m function and line [%s][%d] addr = 0x%p data = 0x%x ck_factor = 0x%x\033[0m\n",__FUNCTION__,__LINE__,addr,data,ck_factor);
    	u3h_writelmsk(addr,data << ck_factor,RX_UX_EXIT_LFPS_REF);
		printk("\033[0;34m function and line [%s][%d] \033[0m\n",__FUNCTION__,__LINE__);
    	addr = usb3_csr_base + U3H_REF_CK_PARAMETER;
    	data = U3_REF_CK_VAL;
    	u3h_writelmsk(addr,data << ck_factor,REF_1000NS);
		printk("\033[0;34m function and line [%s][%d] \033[0m\n",__FUNCTION__,__LINE__);
    	//set SYS_CK
    	addr = usb3_csr_base + U3H_TIMING_PULSE_CTRL;
    	data = U3_SYS_CK_VAL;
    	u3h_writelmsk(addr,data << ck_factor,CNT_1US_VALUE);
...
#endif
	printk("\033[0;34m function and line [%s][%d] \033[0m\n",__FUNCTION__,__LINE__);
/* for later project, we put this in  set_frame_cnt_ck*/
#ifdef SSM_ENABLE
...
#if defined(CONFIG_ARCH_MT5893)||defined(CONFIG_ARCH_MT5886)
