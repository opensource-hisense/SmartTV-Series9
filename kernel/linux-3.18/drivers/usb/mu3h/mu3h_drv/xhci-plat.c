...
static int xhci_plat_probe(struct platform_device *pdev)
{
		...
		if (!request_mem_region(res_ippc->start, resource_size(res_ippc),
                                "ippc")) {
                dev_dbg(&pdev->dev, "error request_mem_region for ippc\n");
                return -EBUSY;
        }
...
static int xhci_plat_remove(struct platform_device *dev)
{
        ...
        usb_remove_hcd(hcd);
#if XHCI_MTK
		res_ippc= platform_get_resource_byname(dev, IORESOURCE_MEM, "ippc");
