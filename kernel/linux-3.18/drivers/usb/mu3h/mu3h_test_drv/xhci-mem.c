...
#include "xhci.h"
#include "../mu3h_lib/xhci-mtk-scheduler.h"
		...
		gfp_t mem_flags)
{
	...
	if (num_stream_ctxs > MEDIUM_STREAM_ARRAY_SIZE)
		...
		return dma_alloc_coherent(dev,
			sizeof(struct xhci_stream_ctx)*num_stream_ctxs, dma, mem_flags);
	else if (num_stream_ctxs <= SMALL_STREAM_ARRAY_SIZE)
...
int xhci_setup_addressable_virt_dev(struct xhci_hcd *xhci, struct usb_device *udev)
{
	...
	xhci_dbg(xhci, "ep0_ctx 0x%x 0x%x 0x%x 0x%x\n", ep0_ctx->ep_info, ep0_ctx->ep_info2, (unsigned int)ep0_ctx->deq, ep0_ctx->tx_info);
...
int xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags)
{
	...
	xhci->erst.entries = dma_alloc_coherent(dev,
			sizeof(struct xhci_erst_entry)*ERST_NUM_SEGS, &dma, flags);
	if (!xhci->erst.entries)
		goto fail;
