...
static volatile unsigned int print_flag = 0;
...
int f_test_lib_init(){
	...
	if (my_hcd != NULL){
		printk("Warning: xhc-mtk is already registered!\n");
		return RET_FAIL;
	}
...
int f_test_lib_cleanup(){
    ...
    u32 temp;
#endif
    u32 __iomem *u2_ctrl_addr;
    u32 __iomem *otg_sts_addr;
	...
	u2_ctrl_addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + U3H_SSUSB_U2_CTRL_0P +0*0x08);
	otg_sts_addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + U3H_SSUSB_OTG_STS);
    ...
    while((readl(otg_sts_addr) & 0x2000) == 0x2000){
    }
    printk(KERN_ERR "[OTG_H] can set dma to host\n");
#if 1
    temp = readl(u2_ctrl_addr);
    temp = temp | SSUSB_U2_PORT_HOST_SEL;
    writel(temp, u2_ctrl_addr);
#endif
#endif
...
//		xhci_err(xhci, "sleep %d msecs\n", randomSleep);
		...
		if(rreg_data->power_required){
			...
			enablePortClockPower(my_hcd->self.controller, port_id,rreg_data->speed);
		}
		//random access(read) MAC3/MAC2 regs
		randomIndex = get_random_int()%3;
		if(randomIndex == 0){
			addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET);
		}
		else if(randomIndex == 1){
			addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET + 0x200);
		}
		else if(randomIndex == 2){
			addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB2_CSR_OFFSET);
		}
	...
	do {
		...
		if (!(ep->ep_state & EP_HALT_PENDING) && !(ep->ep_state & SET_DEQ_PENDING)
			...
			while(g_stopping_ep)
				...
				msleep(1);
				if(print_flag)
					xhci_err(xhci, "stop endpoint thread hang ep_index = %d \n",ep_index);
			}
			g_stopping_ep = true;
...
int f_disconnect_port(int index){
	...
	xhci_err(xhci, "Waiting for device[%d] to disconnect\n", index);
 ...
 */
	...
	xhci_err(xhci, "enable slot command\n");
	xhci_ring_cmd_db(xhci);
	...
	xhci_err(xhci, "enable slot done\n");
...
int f_address_slot(char isBSR, struct usb_device *dev){
	...
	xhci_err(xhci, "address device done\n");
...
int f_evaluate_context(int max_exit_latency, int maxp0, int preping_mode, int preping, int besl, int besld){
	...
	int orig_maxp, new_maxp, orig_max_exit_latency, new_max_exit_latency, 
		new_preping_mode,  new_preping;
	int  new_besl,  new_besld;
	int ret;
...
int f_ctrlrequest(struct urb *urb, struct usb_device *udev){
	...
	ret = xhci_queue_ctrl_tx(xhci, GFP_ATOMIC, urb,	udev->slot_id, 0);
	spin_unlock_irqrestore(&xhci->lock, flags);
	xhci_err(xhci, "++++ queue ctr tx done! len=%d\n", urb->transfer_buffer_length);
	wait_not_event_on_timeout(&(urb->status), -EINPROGRESS, TRANS_TIMEOUT);
	if(urb->status == 0){
		if(urb->transfer_buffer_length > 0){
			dma_sync_single_for_cpu(dev,urb->transfer_dma,urb->transfer_buffer_length,DMA_BIDIRECTIONAL);
...
int f_config_ep(char ep_num,int ep_dir,int transfer_type, int maxp,int bInterval, int burst, int mult, struct usb_device *udev,int config_xhci){
	int ret;
	struct xhci_hcd *xhci;
	struct usb_host_endpoint *ep_tx;
...
int f_queue_urb(struct urb *urb,int wait, struct usb_device *dev){
	...
	if(usb_endpoint_xfer_bulk(&ep->desc)){
		spin_lock_irqsave(&xhci->lock, flags);
		ret = xhci_queue_bulk_tx(xhci, GFP_ATOMIC, urb, udev->slot_id, ep_index);
		spin_unlock_irqrestore(&xhci->lock, flags);
	}
	else if(usb_endpoint_xfer_int(&ep->desc)){
		spin_lock_irqsave(&xhci->lock, flags);
		ret = xhci_queue_intr_tx(xhci, GFP_ATOMIC, urb, udev->slot_id, ep_index);
		spin_unlock_irqrestore(&xhci->lock, flags);
	}
	else if(usb_endpoint_xfer_isoc(&ep->desc)){
		spin_lock_irqsave(&xhci->lock, flags);
		ret = xhci_queue_isoc_tx_prepare(xhci, GFP_ATOMIC, urb, udev->slot_id, ep_index);
		spin_unlock_irqrestore(&xhci->lock, flags);
	}
	...
	if(wait){
		...
		if(urb->status != 0){
			...
			if(urb->status == -EINPROGRESS){
				...
				xhci_err(xhci, "[ERROR] EPIPE: g_slot_id =%d, udev->slot_id =%d, ep_index =%d\n", g_slot_id, udev->slot_id, ep_index);					
				f_reset_ep(g_slot_id,ep_index);
				f_ring_set_tr_dequeue_pointer(g_slot_id, ep_index, urb);
				xhci_urb_free_priv(xhci, urb_priv);
				urb->hcpriv = NULL;
...
int f_data_in(int ep_in,int data_length,int start_add,int wait){
	...
	if(ep->desc.bmAttributes == EPATT_ISO){
		
		num_packets=(data_length/ep->desc.wMaxPacketSize);
	}
	else{
		num_packets=0;
	}
	urb_rx = usb_alloc_urb(num_packets, GFP_KERNEL);
	ret = f_fill_urb(urb_rx,ep_in,data_length,start_add,URB_DIR_IN, num_packets, 1024, NULL);
	if(ret){
		xhci_err(xhci, "[ERROR]fill rx urb Error!!\n");
		return RET_FAIL;	
	}
    ret = f_queue_urb(urb_rx,wait, NULL);
	if(ret){
		if(urb_rx->status == -EPIPE){
			g_event_stall = 1;
			xhci_err(xhci, "Rx Stall!!\n");
		}
		xhci_err(xhci, "[ERROR] Tx transfer error, status=%d\n", urb_rx->status);
...
int f_data_out(int ep_out,int data_length,int start_add,int wait){
	...
	urb_tx = usb_alloc_urb(num_packets, GFP_KERNEL);
	g_urb_tx=urb_tx;
 	ret = f_fill_urb(urb_tx,ep_out,data_length,start_add,URB_DIR_OUT,num_packets,1024, NULL);

	if(ret){
		xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
		return RET_FAIL;	
	}
    ret = f_queue_urb(urb_tx,wait, NULL);
	
	if(ret){
		if(urb_tx->status == -EPIPE){
			g_event_stall = 1;
			xhci_err(xhci, "Tx Stall!!\n");
		}
		xhci_err(xhci, "[ERROR] Tx transfer error, status=%d\n", urb_tx->status);
...
int f_loopback_loop_gpd(int ep_out, int ep_in, int data_length, int start_add, int gpd_length, struct usb_device *usbdev){
	...
	for(i=0; i<num_urbs; i++){
		...
		xhci_dbg(xhci, "[LOOPBACK]Tx round %d, urb_length %d, buffer 0x%x, mapping 0x%x\n"
			, i, tmp_urb_len, (unsigned int)(buffer_tx+total_running), mapping_tx+total_running);
		urb_tx = usb_alloc_urb(iso_num_packets, GFP_KERNEL);
...
//			printk(KERN_ERR "[Debug] max_esit_payload: %d\n", max_esit_payload);
		...
		xhci_dbg(xhci, "[LOOPBACK]Rx round %d, urb_length %d, buffer 0x%x, mapping 0x%x\n"
			, i, tmp_urb_len, (unsigned int)(buffer_rx+total_running), mapping_rx+total_running);
		...
		xhci_dbg(xhci, "urb_rx buffer 0x%x transfer_dma 0x%x\n", (unsigned int)urb_rx->transfer_buffer, urb_rx->transfer_dma);
		if(ret){
			xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
			return RET_FAIL;	
		}
	...
	for(i=0; i<data_length; i++){
		...
		if((*tmp1) != (*tmp2)){
			xhci_err(xhci, "[ERROR] buffer %d not match, tx buf 0x%x, rx buf 0x%x, tx 0x%x, rx 0x%x\n"
				, i, (unsigned int)tmp1, (unsigned int)tmp2, *tmp1, *tmp2);
			ret = RET_FAIL;
			break;
		}
	}
...
//	g_port_reset = false;
	//enablePortClockPower(port_index,0x3); // modify by mtk40184
	if(isWarmReset && port->port_speed == USB_SPEED_SUPER){
		enablePortClockPower(my_hcd->self.controller, port_index,0x3); // add by mtk40184
		//do warm reset
		addr = &xhci->op_regs->port_status_base + NUM_PORT_REGS*((port_id-1) & 0xff);
		temp = xhci_readl(xhci, addr);
		...
		enablePortClockPower(my_hcd->self.controller, port_index,0x2); // add by mtk40184
		//hot reset port
		addr = &xhci->op_regs->port_status_base + NUM_PORT_REGS*((port_id-1) & 0xff);
		temp = xhci_readl(xhci, addr);
		xhci_dbg(xhci, "before reset port %d = 0x%x\n", port_id-1, temp);
...
int f_power_suspend(){
	...
	if(!g_concurrent_resume){
		if(port->port_speed == USB_SPEED_SUPER){
			disablePortClockPower(my_hcd->self.controller, (g_port_id-1), 0x3);
		}
		else{
	...
	if(!g_concurrent_resume){
		...
		if(PORT_PLS_VALUE(temp) != 3){
			...
			xhci_info(xhci, "port enter U3 state\n");
			if(port->port_speed == USB_SPEED_SUPER){
			disablePortClockPower(my_hcd->self.controller, (g_port_id-1), 0x3);
		}
		else{
...
int f_power_remotewakeup(){
	...
	poll_event_on_timeout((volatile int *)&g_port_resume, 1, 10000);
	if(g_port_resume == 0){
...
int f_power_reset_u1u2_counter(int u_num){
	...
	if(u_num == 1){
		addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET + U3H_USB3_U1_STATE_INFO);
		u3h_writelmsk(addr,0x1<<16,CLR_USB3_U1_CNT)
	}
	else if(u_num == 2){
		addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET + U3H_USB3_U2_STATE_INFO);
		u3h_writelmsk(addr,0x1<<16,CLR_USB3_U2_CNT)
	}
	return RET_SUCCESS;
}
...
int f_power_get_u1u2_counter(int u_num){	
	...
	if(u_num == 1){		
		addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET + U3H_USB3_U1_STATE_INFO);
		data = readl(addr) & USB3_U1_CNT;
	}
	else if(u_num == 2){
		addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB3_CSR_OFFSET + U3H_USB3_U2_STATE_INFO);
		data = readl(addr) & USB3_U2_CNT;
	}
	return 0;
}
...
int f_power_reset_L1_counter(int direct){
	...
	if(direct == 1){
		temp = LPM_ENTRY_COUNT_RESET;
	}
	else if (direct == 2){
		temp = LPM_EXIT_COUNT_RESET;
	}
	addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB2_CSR_OFFSET + U3H_USB20_LPM_ENTRY_COUNT);
	writel(temp, addr);
	return 0;
}
...
int f_power_get_L1_counter(int direct){	
	...
	//direct: 1:ENTRY, 2:EXIT
	if(direct == 1){
		return (readl(addr) && LPM_ENTRY_COUNT);
	}
	else if (direct == 2){
		return ((readl(addr) && LPM_EXIT_COUNT) >> 16);
	}
	return 0;
}
...
u64 ts_config_lpm;
volatile int l1_exit_timer = 0x10;
...
//	int ret;	
	...
	dev = my_hcd->self.controller;
	u3h_hw = dev->platform_data;
	addr = (u32 __iomem *) (u3h_hw->u3h_virtual_base + SSUSB_USB2_CSR_OFFSET + U3H_POWER_MANAGEMENT);
	temp = xhci_readl(xhci, addr);
	temp &= ~SUSPENDM_ENABLE;
	xhci_writel(xhci, temp, addr);
	temp = xhci_readl(xhci, addr);
	
	//set bulk_nyet_active_mask & int_nak_active_mask
	addr = (u32 __iomem *)(my_hcd->regs + HSCH_CFG2);
	temp = xhci_readl(xhci, addr);
	//pdn
	...
	port_index = get_port_index(g_port_id);
	port = rh_port[port_index];
	addr = (u32 __iomem *)(my_hcd->regs + U2PORT_CFG);
	if(hle == 1 && rwe == 1){
		temp = 0xff;
		xhci_writel(xhci, temp, addr);
	}
	...
	num_u3_port = get_xhci_u3_port_num(my_hcd->self.controller);
	if(hle == 1){
		disablePortClockPower(my_hcd->self.controller, g_port_id-1-num_u3_port, 0x02);
	}
	else{
		enablePortClockPower(my_hcd->self.controller, g_port_id-1-num_u3_port, 0x02);
	}
#endif	
	return RET_SUCCESS;	
}
...
//#if KERNEL_3_4
	 ...
	 */
	//hdev->maxchild = 4;
	//hdev->children = kzalloc(hdev->maxchild *sizeof(struct usb_device *), GFP_KERNEL);
#endif
...
static int stress_ep0_thread(void *data){
	...
	do{
		if(print_count++ == 1000){
			print_count = 0;
			xhci_err(xhci, "[STRESS] stress_rdn_len_rx_done_thread ep_num %d\n", str_data->ep_num);
			}
		msleep(10);
	...
	do{
		...
		while(tx_status != URB_STATUS_IDLE){
			...
			if(count_print++ == 1000)
				{
				count_print = 0;
				xhci_err(xhci, "[STRESS][ERROR] stress_tx_thread error now!!!\n");
			}
			tx_status = urb_tx->status;
		}
	...
	do{
		...
		while(tx_status != 0){
			...
			if(count_print++ == 1000)
				{
				count_print = 0;
				xhci_err(xhci, "[STRESS][ERROR] stress_tx_done_thread error now!!!\n");
			}
			tx_status = urb_tx->status;
		}
		ep_rx = urb_tx->ep;
...
static int stress_rdn_len_tx_thread(void *data){
	...
	do{
		if(print_count++ == 1000){
			print_count = 0;
			xhci_err(xhci, "[STRESS] stress_rdn_len_tx_thread ep_num %d\n", str_data->ep_num);
			}
		urb_tx = str_data->urb_tx_list[tx_index];
		tx_status = urb_tx->status;
		while(tx_status != URB_STATUS_IDLE){
...
static int stress_rdn_len_tx_done_thread(void *data){
	...
	do{
		if(print_count++ == 1000){
			print_count = 0;
			xhci_err(xhci, "[STRESS] stress_rdn_len_tx_done_thread ep_num %d\n", str_data->ep_num);
			}
		urb_tx = str_data->urb_tx_list[tx_index];
		tx_status = urb_tx->status;
		while(tx_status != 0){
	...
	do{
		...
		if(str_data->isCompare){
			...
			if(((str_data->ep_num==1) && (str_data->loop_count[tx_index] == 300)) 
				...
				xhci_dbg(xhci, "[STRESS] comparing dev %d ep %d buffer %d\n", str_data->dev_num, str_data->ep_num, tx_index);
				//compare buffer data
				for(i=0; i<data_len; i++){
					tmp1 = urb_rx->transfer_buffer+i;
					tmp2 = str_data->buffer[tx_index]+i;
				...
				xhci_dbg(xhci, "[STRESS] comparing buffer dev %d ep %d %d done\n", str_data->dev_num, str_data->ep_num, tx_index);
				//reset loop_count
				str_data->loop_count[tx_index]=0;
			}
		}		
...
int otg_dev_B_hnp(void *data){
	...
	//HCRST should not be software reset for U2 MAC port0
   // writel((readl(0xf00409b4) & ~(0x10000)), 0xf00409b4);
    writel((readl(SSUSB_U3_XHCI_BASE+0x09b4) & ~(0x10000)), SSUSB_U3_XHCI_BASE+0x09b4);
	wait_event_on_timeout((volatile int *)&g_otg_hnp_become_host, true, TRANS_TIMEOUT);
	...
	//writel(SSUSB_CHG_A_ROLE_B, SSUSB_OTG_STS);
...
#endif
	wait_event_on_timeout((volatile int *)&g_otg_hnp_become_dev, true, TRANS_TIMEOUT);
...
//	char *tmp;
	...
	dr->bRequest = USB_REQ_SET_CONFIGURATION;
	#if TEST_PET
	dr->wValue = cpu_to_le16(0); //b_hnp_enable 1 if opt, pet use 0 
	#else
	dr->wValue = cpu_to_le16(1); //b_hnp_enable 1 if opt, pet use 0 
	#endif
	dr->wIndex = cpu_to_le16(0);
	dr->wLength = cpu_to_le16(0);
	urb = alloc_ctrl_urb(dr, NULL, udev);
...
TD_4_6:
	...
	if(g_port_resume == 1){
		...
		if(PORT_PLS_VALUE(temp) != 0){//(temp & PORT_PLS(15)) != PORT_PLS(0)){
			...
			printk(KERN_ERR "[OTG_H][FAIL]exit u3h_uut_A_host thread\n");
			return RET_FAIL;
		}
		else{
...
int otg_pet_uut_a(void *data){
	...
	u32 vbus_on;
       u32 __iomem *u2_ctrl_addr;
	u32 __iomem *otg_sts_addr;
	u32 __iomem *otg_ien_addr;
	u32 __iomem *u2_sys_addr;
	struct mtk_u3h_hw *u3h_hw;
	struct device *dev;
	...
	u3h_hw = dev->platform_data;
      ...
      u3h_hw = &temp4;
      u3h_hw->ippc_virtual_base = IPPC_BASE0;
      u3h_hw->u3h_virtual_base = U3H_BASE0;

#endif

	u2_ctrl_addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + U3H_SSUSB_U2_CTRL_0P +0*0x08);
	otg_sts_addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + U3H_SSUSB_OTG_STS);
	otg_ien_addr = (u32 __iomem *)(u3h_hw->ippc_virtual_base + U3H_SSUSB_OTG_INT_EN);
    u2_sys_addr = (u32 __iomem *)(u3h_hw->u3h_virtual_base + SSUSB_USB2_CSR_OFFSET);

	

	//enable OTG interrupt
	temp = readl(otg_ien_addr);
	temp = temp | SSUSB_ATTACH_A_ROLE_INT_EN | SSUSB_CHG_A_ROLE_A_INT_EN 
		| SSUSB_CHG_B_ROLE_A_INT_EN | SSUSB_SRP_REQ_INTR_EN;
    writel(temp, otg_ien_addr);
	//set host sel
	printk(KERN_ERR "[OTG_H] going to set dma to host\n");
	while((readl(otg_sts_addr) & 0x2000) == 0x2000){
	}
	...
	temp2 = readl(otg_ien_addr);

	writel(temp2, otg_ien_addr);
	//printk("[OTG_H] write back SSUSB_OTG_INT_EN 0x%x\n", temp2);
	//writel(SSUSB_ATTACH_A_ROLE, SSUSB_OTG_STS);
	...
	g_hs_block_reset = false;
	if(g_hs_block_reset){
		writel((readl(u2_sys_addr+0xc)|0x100), (u2_sys_addr+0xc));
	}
	else{
		writel((readl(u2_sys_addr+0xc)&(~0x100)), (u2_sys_addr+0xc));
    }
	...
	writel(0x3,(IPPC_BASE0+0xB0));
       writel(0xB0B0, (IPPC_BASE0+0xBC));
	writel(0xc, (IPPC_BASE0+0xB4));
       printk("[OTG_H] Probe data = 0x%x\n",(readl(IPPC_BASE0+0xc4)));
	printk("[OTG_H] SSUSB_OTG_STS = 0x%x\n",readl(otg_sts_addr));
	...
	writel((readl(IPPC_BASE0+0xc8) | 0x1fd0000),(IPPC_BASE0+0xc8));
	writel((readl(0xf004340c)|0x2fa00), 0xf004340c);

				
	while(g_stopped == false){
		temp3 = readl(otg_sts_addr);
		...
		if(g_otg_iddig != cur_iddig ){
			if(g_otg_iddig == 1){
				//turn off vbus	
				printk("[OTG_H] iddig up, turn off vbus\n");
				printk("[OTG_H] SSUSB_OTG_STS = 0x%x\n",readl(otg_sts_addr));
				printk("[OTG_H] Probe data = 0x%x\n",(readl(IPPC_BASE0+0xc4)));
#if 0				
				//set HOST_VBUSVALID_SEL=0
				...
				//set port_power=0
				// printk("[OTG_H] port_status_base = 0x%x\n",&xhci->op_regs->port_status_base);
				...
				//turn on vbus
				printk("[OTG_H] iddig down, turn on vbus\n");
				printk("[OTG_H] SSUSB_OTG_STS = 0x%x\n",readl(otg_sts_addr));
				printk("[OTG_H] Probe data = 0x%x\n",(readl(IPPC_BASE0+0xc4)));
#if 0				
		...
		if(g_otg_csc && g_otg_pet_status != OTG_HNP_DEV){
			...
			if(g_port_connect){
				...
				printk(KERN_ERR "[OTG_H]Port disonnected\n");
				if(g_otg_pet_status == OTG_HNP_SUSPEND){
					while(!g_otg_hnp_become_dev || (readl(otg_sts_addr)&SSUSB_HOST_DEV_MODE)){
						msleep(1);
					}
				}
				...
				if(g_otg_pet_status == OTG_HNP_SUSPEND){
					...
					temp2 = readl(otg_ien_addr);
					printk("[OTG_H] SSUSB_OTG_INT_EN: 0x%x\n", temp2);
					printk(KERN_ERR "[OTG_H] set CHG_B_ROLE_B\n");
					writel(SSUSB_CHG_B_ROLE_B, otg_sts_addr);
...
int otg_pet_uut_b(void *data){
	...
	g_stopped = false;
      ...
      //enable OTG2.0
      writel((readl(u3h_hw->ippc_virtual_base+0xc8) | 0x1fd0000),(u3h_hw->ippc_virtual_base+0xc8));
	...
	writel((readl(u3h_hw->u3h_virtual_base+0x09b4) & ~(0x10000)), u3h_hw->u3h_virtual_base+0x09b4);


	//printk(KERN_ERR "[OTG_H] after init U3_P1_SC = 0x%x\n",(readl(U3_P1_SC)));
	...
	while(g_stopped == false){
		...
		if(g_otg_pet_status == OTG_DEV){
			if(g_otg_hnp_become_host){
				...
				printk(KERN_ERR "[OTG_H] Become host\n");
				#if 0
				printk(KERN_ERR "[OTG_H] U3_P1_SC = 0x%x\n",(readl(U3_P1_SC)));
				if(readl(U3_P1_SC)==0x2e0)
					{
        printk(KERN_ERR "===enalble probe=======\n");	
	 writel(0x3, (SIFSLV_IPPC+0xB0));
       writel(0xB0B0, (SIFSLV_IPPC+0xBC));
	writel(0xC, (SIFSLV_IPPC+0xB4));
				 printk(KERN_ERR "[OTG_H] Probe data = 0x%x\n",(readl(SIFSLV_IPPC+0xc4)));
				 printk(KERN_ERR "[OTG_H] IPPC 00 = 0x%x\n",(readl(SIFSLV_IPPC+0x00)));
				 printk(KERN_ERR "[OTG_H] IPPC 04 = 0x%x\n",(readl(SIFSLV_IPPC+0x04)));
				 printk(KERN_ERR "[OTG_H] IPPC 50 = 0x%x\n",(readl(SIFSLV_IPPC+0x50)));
				 printk(KERN_ERR "[OTG_H] SSUSB_U3_XHCI_BASE 20= 0x%x\n",(readl(SSUSB_U3_XHCI_BASE+0x20)));		
				 printk(KERN_ERR "[OTG_H] SSUSB_U3_XHCI_BASE 24= 0x%x\n",(readl(SSUSB_U3_XHCI_BASE+0x24)));				 
					}
		//		printk(KERN_ERR "[OTG_H] SSUSB_U2_CTRL = 0x%x\n",readl(SSUSB_U2_CTRL(0)));
				#endif
				if(!g_otg_wait_con && !g_port_connect){
					...
					xhci_writel(xhci, ER_IRQ_DISABLE(temp),
							&xhci->ir_set->irq_pending);
					//set B_role_B
					while(readl(otg_sts_addr) & SSUSB_XHCI_MAS_DMA_REQ){
		...
		if((g_otg_pet_status != OTG_DEV) && (!g_port_connect)){
			...
			g_port_connect = false;
			g_port_reset = false;
			printk(KERN_ERR "[OTG_H]alert device2\n");
			//turn off irq_en
			temp = xhci_readl(xhci, &xhci->ir_set->irq_pending);
			xhci_writel(xhci, ER_IRQ_DISABLE(temp),
					&xhci->ir_set->irq_pending);
		...
		if(g_otg_pet_status == OTG_HNP_DISCONNECTED){
			...
			g_otg_hnp_become_host = false;
				...
				g_port_connect = false;
				g_port_reset = false;
				printk(KERN_ERR "[OTG_H]alert device3\n");
				//turn off irq_en
				temp = xhci_readl(xhci, &xhci->ir_set->irq_pending);
				xhci_writel(xhci, ER_IRQ_DISABLE(temp),
						&xhci->ir_set->irq_pending);
...
{
	xhci_info(xhci, "bDescriptorType = %02xH\n", value);	

	switch(value) {
	case MUSB_DT_DEVICE: 
		xhci_info(xhci, "\tDescriptorType is Device Descriptor.\n"); 
		break;
	case MUSB_DT_CONFIG: 
		xhci_info(xhci, "\tDescriptorType is Configuration Descriptor.\n"); 
		break;
	case MUSB_DT_STRING: 
		xhci_info(xhci, "\tDescriptorType is String Descriptor.\n"); 
		break;
	case MUSB_DT_INTERFACE: 
		xhci_info(xhci, "\tDescriptorType is Interface Descriptor.\n"); 
		break;
	case MUSB_DT_ENDPOINT: 
		xhci_info(xhci, "\tDescriptorType is Endpoint Descriptor.\n"); 
		break;
	case MUSB_DT_DEVICE_QUALIFIER: 
		xhci_info(xhci, "\tDescriptorType is Device_Qualifier Descriptor.\n"); 
		break;
	case MUSB_DT_OTHER_SPEED: 
		xhci_info(xhci, "\tDescriptorType is Other_Speed_Configuration Descriptor.\n"); 
		break;
	case MUSB_DT_INTERFACE_POWER: 
		xhci_info(xhci, "\tDescriptorType is Interface_Power Descriptor.\n"); 
		break;
	case MUSB_DT_OTG: 
		xhci_info(xhci, "\tDescriptorType is OTG Descriptor.\n"); 
		break;
	case MUSB_DT_HUB: 
		xhci_info(xhci, "\tDescriptorType is Hub Descriptor.\n"); 
		break;
	default:
		xhci_err(xhci, "\tUnknown DescriptorType!!!!!\n"); 
		break;
	}
...
u8 f_Descriptors_USB_Info(struct usb_device_descriptor *devDesc)
{
	...
	if ((devDesc->bDeviceClass==0x09) && (devDesc->bDeviceSubClass==0x00))
	{
		if (devDesc->bDeviceProtocol == 0x00) {
			xhci_info(xhci, "Device is Full speed Hub.\n"); 	
			u1USBClassCode = 0x09;
		}
		else if (devDesc->bDeviceProtocol == 0x01) {
			xhci_info(xhci, "Device is High speed Hub with single TT.\n"); 		
			u1USBClassCode = 0x09;		
		}
		else if (devDesc->bDeviceProtocol == 0x02) {
			xhci_info(xhci, "Device is High speed Hub with multiple TTs.\n"); 	
			u1USBClassCode = 0x09;
		}
		//else if (devDesc->bDeviceProtocol == 0x03) {
		//	xhci_info(xhci, "Device is Super speed Hub.\n"); 
		//}
	}		

	return u1USBClassCode;
}
