...
int dev_stall(char bdp,int length,int gpd_buf_size,int bd_buf_size)
{
	struct usb_device *udev, *rhdev;
	struct usb_ctrlrequest *dr;
	char *ptr;
	...
	dr = kmalloc(sizeof(struct usb_ctrlrequest), GFP_NOIO);
	ptr= kmalloc(2048, GFP_NOIO);	
	memset(ptr, 0, STALL_DATA_LENGTH);
	*ptr=0x55;
	*(ptr+1)=0xAA;
	*(ptr+2)=STALL_DATA_LENGTH&0xFF;
	*(ptr+3)=STALL_DATA_LENGTH>>8;
	*(ptr+4)=STALL&0xFF;
	*(ptr+5)=STALL>>8;
	*(ptr+6)=length&0xFF;
	*(ptr+7)=(length>>8)&0xFF;
	...
	*(ptr+16)=bdp;
	...
	dr->wValue = cpu_to_le16(0);
	dr->wIndex = cpu_to_le16(0);
	dr->wLength = cpu_to_le16(STALL_DATA_LENGTH);
...
int dev_get_status(void)
{
	...
	dr->bRequestType = USB_DIR_IN | USB_RECIP_ENDPOINT;;
	dr->bRequest = GET_STATUS;
	dr->wValue = 0;
	dr->wIndex = USB_DIR_OUT;
	dr->wLength = cpu_to_le16(USB_DT_STATUS_SIZE);

	urb = alloc_ctrl_urb(dr, ptr, udev);
	ret = f_ctrlrequest(urb, udev);
	memcpy(ptr, urb->transfer_buffer, USB_DT_STATUS_SIZE);
...
int dev_ep_halt(void){
	...
	dr->bRequestType = USB_DIR_OUT | USB_RECIP_ENDPOINT;;
