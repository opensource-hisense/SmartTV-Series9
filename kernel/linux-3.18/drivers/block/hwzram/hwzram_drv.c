...
#if 0 /* def CONFIG_MT_ENG_BUILD */
static void check_zero_page(struct bio *bio, bool should_be_zero)
{
	...
	map = (unsigned long *)kmap_atomic(page);
	if (!map) {
		pr_warn("%s: kmap_atomic failed\n", __func__);
		return;
	}

	for (pos = 0; pos != PAGE_SIZE / sizeof(*map); pos++) {
		if (map[pos]) {
			is_zero = false;
			break;
		}
...
static int __hwzram_make_request(struct hwzram *hwzram, struct bio *bio)
{
	...
	pr_devel("%s: bio bi_phys_segments %u bi_remaining = %u bi_vcnt = %u\n",
	       __func__, bio->bi_phys_segments, atomic_read(&bio->bi_remaining),
	       bio->bi_vcnt);
 ...
 */
static void hwzram_make_request(struct request_queue *queue, struct bio *bio)
{
