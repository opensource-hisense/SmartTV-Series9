 ...
 * 10.Use CINTERP_CTRL for the completion of compression.
 *
...
static bool hwzram_force_timer; /* false by default */
...
static void hwzram_impl_timer_fn(unsigned long data)
{
	...
	do {
		...
		hwzram_write_register(hwz, ZRAM_INTRP_STS_CMP, compr);
		update_complete_index(hwz, true);
				     ...
				     hwzram_clock_control hwctrl)
{
	...
	static void *local_hwzram_offset_ptr;
...
#define	LOCAL_HWZRAM_OFFSET	0x2000
	local_hwzram_offset_ptr = vmalloc(LOCAL_HWZRAM_OFFSET);
	if (!local_hwzram_offset_ptr)
		pr_warn("%s: no gap before iomap\n", hwz->name);

	pr_info("%s: gap is mapped at 0x%p\n", __func__, local_hwzram_offset_ptr);
