			  ...
			  u32 index, int offset, struct bio *bio)
{
	...
	if (unlikely(!zram->table[index].page)) {
		pr_debug("Read before write: sector=%lu, size=%u",
			 (ulong)(bio->bi_iter.bi_sector), bio->bi_iter.bi_size);
		handle_zero_page(bvec);
		return 0;
	}
...
static inline int valid_io_request(struct zram *zram, struct bio *bio)
{
	...
	/* unaligned request */
	if (unlikely(bio->bi_iter.bi_sector & (ZRAM_SECTOR_PER_LOGICAL_BLOCK - 1)))
		return 0;
...
#define FIX_ZRAM_MAJOR (253)
	zram_major = register_blkdev(FIX_ZRAM_MAJOR, "zram");
	if (zram_major < 0) {
		pr_warning("Unable to get major number\n");
		ret = -EBUSY;
		goto out;
	}
	zram_major = FIX_ZRAM_MAJOR;
