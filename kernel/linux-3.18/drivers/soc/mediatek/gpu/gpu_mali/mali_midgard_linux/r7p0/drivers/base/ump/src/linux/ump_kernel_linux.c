...
static int do_get_fb_ump_secureid(umpp_session * session, ump_k_get_secureid * params)
{    
	ump_dd_handle ump_wrapped_buffer;
	ump_dd_physical_block ump_memory_description;
	ump_secure_id secure_id;
	ump_memory_description.addr = params->addr;
	ump_memory_description.size = params->size;
	...
	*(params->secure_id)= secure_id;
	printk(KERN_WARNING "secure_id=%d\n", secure_id);
	return 0;
}
...
#endif
{
	...
	switch(cmd)
	{
		...
		case UMP_FUNC_RELEASE:
			...
			return 0;
		case UMP_FUNC_GET_SECUREID:
			if (size != sizeof(ump_k_get_secureid))
			{                
				return -ENOTTY;
			}
			if (copy_from_user(&msg, (void __user *)arg, size))
			...
			ret = do_get_fb_ump_secureid(session,(ump_k_get_secureid*) &msg);
			if (ret)
			{
				return ret;
			}
