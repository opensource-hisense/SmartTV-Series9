...
typedef struct ump_k_get_secureid
{
	uint64_t addr; /**< [in] framebuffer start addr */
	uint64_t size; /**< framebuffer size*/
	ump_secure_id * secure_id;/**< [in] Flags to use when allocating */
} ump_k_get_secureid;
...
#define UMP_FUNC_GET_SECUREID _IOWR(UMP_IOC_MAGIC, 8, ump_k_get_secureid)

/*max ioctl sequential number*/
#define UMP_IOC_MAXNR 8
