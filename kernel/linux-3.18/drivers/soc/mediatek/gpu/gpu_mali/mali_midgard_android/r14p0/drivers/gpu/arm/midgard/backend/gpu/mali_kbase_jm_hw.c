...
void kbase_job_done(struct kbase_device *kbdev, u32 done)
{
	...
	*/
	spin_lock_irqsave(&kbdev->job_mask_change, flags);
 	job_irq_mask = kbase_reg_read(kbdev, JOB_CONTROL_REG(JOB_IRQ_MASK), NULL);
 	kbase_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_MASK), 0, NULL);
 	spin_unlock_irqrestore(&kbdev->job_mask_change, flags);
...
#endif /* KBASE_GPU_RESET_EN */
	spin_lock_irqsave(&kbdev->job_mask_change, flags);
 	kbase_reg_write(kbdev, JOB_CONTROL_REG(JOB_IRQ_MASK), job_irq_mask, NULL);
 	spin_unlock_irqrestore(&kbdev->job_mask_change, flags);
