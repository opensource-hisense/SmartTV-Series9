...
/* Check if any group interrupt pending, return true.
** otherwise, return false and mask out relevant core's interrupt
*/
static mali_bool mali_group_pending_interrupt_or_mask(struct mali_group *group)
{
	mali_bool pending_interrupt = MALI_TRUE;
	...
	if (group->pp_core) {
		if (mali_group_is_virtual(group)) {
			struct mali_group *child, *temp;
			_MALI_OSK_LIST_FOREACHENTRY(child, temp, &group->group_list, struct mali_group, group_list) {
				/* It means the PP_call hasn't assert the interrupt yet, so it's safe to mask out the interrupt */
				if (MALI200_REG_VAL_IRQ_MASK_NONE == mali_pp_get_int_stat(child->pp_core)) {
					mali_pp_mask_all_interrupts(group->pp_core);
					if (MALI200_REG_VAL_IRQ_MASK_NONE != mali_pp_read_rawstat(child->pp_core))
						MALI_DEBUG_PRINT(2, ("Group PP job on core id %d has rawstatus 0x%x!!!\n", child->pp_core->core_id, mali_pp_read_rawstat(child->pp_core)));
					pending_interrupt = MALI_FALSE;
					break;
				}
			}
		} else {
			if (MALI200_REG_VAL_IRQ_MASK_NONE == mali_pp_get_int_stat(group->pp_core)) {
				mali_pp_mask_all_interrupts(group->pp_core);
				if (MALI200_REG_VAL_IRQ_MASK_NONE != mali_pp_read_rawstat(group->pp_core))
					MALI_DEBUG_PRINT(2, ("Physical group PP job on core id %d has rawstatus 0x%x!!!\n", group->pp_core->core_id, mali_pp_read_rawstat(group->pp_core)));
				pending_interrupt = MALI_FALSE;
			}
		}

		if (MALI_TRUE == pending_interrupt) {
			MALI_DEBUG_PRINT(2, ("PP %d has pending interrupt ++++++++, pp inttertupt status = 0x%x, pp status 0x%x\n", group->pp_core->core_id, mali_pp_get_int_stat(group->pp_core), mali_pp_read_status(group->pp_core)));
		}
	} else {
		if (MALI200_REG_VAL_IRQ_MASK_NONE == mali_gp_get_int_stat(group->gp_core)) {
			mali_gp_mask_all_interrupts(group->gp_core);
			if (mali_gp_read_rawstat(group->gp_core))
				MALI_DEBUG_PRINT(2, ("GP core has rawstatus 0x%x!!!\n", mali_gp_read_rawstat(group->gp_core)));
			pending_interrupt = MALI_FALSE;
		}

		if (MALI_TRUE == pending_interrupt) {
			MALI_DEBUG_PRINT(2, ("GP has pending interrupt +++++++0x%x\n", mali_gp_get_int_stat(group->gp_core)));
		}
	}
...
void mali_group_abort_gp_job(struct mali_group *group, u32 job_id)
{
	...
	while (MALI_TRUE == mali_group_pending_interrupt_or_mask(group)) {
		mali_group_unlock(group);
		MALI_DEBUG_PRINT(2, ("Aborting GP Job is waiting for GP interrupt handler +++++++\n"));
		msleep(5);
		mali_group_lock(group);
	}

	/* We may release the group lock above, so double confirm the group is running own Job */
	if (NULL == group->gp_running_job || mali_gp_job_get_id(group->gp_running_job) != job_id) {
		MALI_DEBUG_PRINT(2, ("Another thread has completed our GP Job, so just return !\n"));
		return ;
	}

	/* The group is hooking our job, abort it */
...
static void mali_group_abort_pp_job(struct mali_group *group, u32 job_id)
{
	...
	while (MALI_TRUE == mali_group_pending_interrupt_or_mask(group)) {
		mali_group_unlock(group);
		MALI_DEBUG_PRINT(2, ("Aborting PP Job is waiting for PP interrupt handler +++++++\n"));
		msleep(5);
		mali_group_lock(group);
	}

	/* We may release the group lock above, so double confirm the group is running own Job */
	if (NULL == group->pp_running_job || mali_pp_job_get_id(group->pp_running_job) != job_id) {
		MALI_DEBUG_PRINT(2, ("Another thread has completed our PP Job, so just return !\n"));
		return ;
	}
...
static void mali_group_bottom_half_mmu(void *data)
{
	...
	if (rawstat & (MALI_MMU_INTERRUPT_PAGE_FAULT | MALI_MMU_INTERRUPT_READ_BUS_ERROR)) {
		...
		if (MALI_FALSE == mali_group_pending_interrupt_or_mask(group)) {
			mali_group_mmu_page_fault_and_unlock(group);
		} else {
			MALI_DEBUG_PRINT(2, ("MMU bottom is waiting for core interrupt handler++++++++++++\n"));
			mali_group_unlock(group);
		}
...
static void mali_group_bottom_half_gp(void *data)
{
	...
	if (0 != irq_errors) {
		...
		if (MALI_FALSE == mali_group_pending_interrupt_or_mask(group)) {
			...
			MALI_DEBUG_PRINT(2, ("GP bottom 2 is waiting for core interrupt handler++++++++++++\n"));
			mali_group_unlock(group);
		}
		...
		if (!_mali_osk_timer_pending(group->timeout_timer) && NULL != group->gp_running_job) {
			...
			if (MALI_FALSE == mali_group_pending_interrupt_or_mask(group)) {
				...
				MALI_DEBUG_PRINT(2, ("GP bottom 3 is waiting for core interrupt handler++++++++++++\n"));
				mali_group_unlock(group);
			}
...
static void mali_group_bottom_half_pp(void *data)
{
	...
	if (0 != irq_errors) {
		...
		if (MALI_FALSE == mali_group_pending_interrupt_or_mask(group)) {
			...
			/* PP interrupt handler will schedule another bottom half */
			MALI_DEBUG_PRINT(2, ("PP bottom 2 is waiting for core interrupt handler++++++++++++\n"));
			mali_group_unlock(group);
		}
		...
		if (!_mali_osk_timer_pending(group->timeout_timer) && NULL != group->pp_running_job) {
			...
			if (MALI_FALSE == mali_group_pending_interrupt_or_mask(group)) {
				...
				/* PP interrupt handler will schedule another bottom half */
				MALI_DEBUG_PRINT(2, ("PP bottom 3 is waiting for core interrupt handler++++++++++++\n"));
				mali_group_unlock(group);
			}
		} else {
			mali_group_unlock(group);
