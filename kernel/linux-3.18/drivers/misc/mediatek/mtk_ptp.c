...
#define LOW_TEMP_OFT	    10
#define LOW_TEMP_THRESHOLD	33000
/*
...
#define VCO_VAL			0x20 //0.7 + 0x20 * 0.00625 = 0.9v
...
#if EN_ISR_LOG 
#define ptp_isr_info(fmt, args...)  pr_info(fmt, ##args)
#else
...
#define ptp_write(addr, val)	\
do {    \
	__raw_writel((val), (void __force __iomem *)((addr)));   \
	mb();  \
} while (0)
...
#else
static int ptp_log_en=1; 
#endif    
static int is_ptp_initializing=true;
static int cur_temperature = 35000;
/*=============================================================
...
#endif
#ifdef CC_PTPOD_SLT
    unsigned int reg_slt_status[NR_PTP_PHASE];
#endif
	/* slope */
	unsigned int MTS;
	unsigned int BTS;
...
static int base_ops_get_temp(struct ptp_det *det)
{
	...
	if (det_to_id(det) == PTP_DET_MCUSYS) {
		/* TS1 temp */
		temperature = get_immediate_ts_cpu_wrap();
#if SUPPORT_PTPOD_GPU
	} else if (det_to_id(det) == PTP_DET_GPUSYS) {
		/* TS3 temp */
		temperature = get_immediate_ts_gpu_wrap();
#endif
	}
#else
	temperature = 35000;
...
static int get_volt_cpu(struct ptp_det *det)
{
	...
	FUNC_ENTER(FUNC_LV_HELP);
	ret = regulator_get_voltage(det->reg); /* unit uv: micro voltage */
	FUNC_EXIT(FUNC_LV_HELP);
	return ret;
}
...
#if 1
	dev_pm_opp_adjust_voltage(det->dev, det->freq_table[opp_index]*1000,
...
extern struct device *get_mt_cpu_device(unsigned cpu);
static void get_freq_volt_table_cpu(struct ptp_det *det)
{
	...
	det->dev = get_mt_cpu_device(det->dev_id);
...
static int get_volt_gpu(struct ptp_det *det)
{
	...
	ret = mt_gpufreq_get_cur_volt(); /* unit  mv * 100 = 10uv */
...
static int set_volt_gpu(struct ptp_det *det)
{
	...
	ret = mt_gpufreq_update_volt(det->volt_tbl_pmic, det->num_freq_tbl);
	FUNC_EXIT(FUNC_LV_HELP);
...
static int limit_mcusys_env(struct ptp_det *det)
{
	...
	for (opp_index = 0; opp_index < det->num_freq_tbl; opp_index++) {
		...
		if (det->volt_table[opp_index] <= u_vboot &&
			...
			det->dvfs_max_freq_khz = det->freq_table[opp_index];
			det->dvfs_min_freq_khz = det->freq_table[opp_index];
			ret = dev_pm_opp_adjust_voltage(det->dev, det->freq_table[opp_index]*1000, u_vboot);
			if (ret)
...
#ifdef CC_PTPOD_SLT
    det->reg_slt_status[PTP_PHASE_INIT01]=false;
    det->reg_slt_status[PTP_PHASE_INIT02]=false;
    det->reg_slt_status[PTP_PHASE_MON]=false;
#endif
	/* init with devinfo */
...
static void ptp_set_ptp_volt(struct ptp_det *det)
{
	...
	if (ptp_log_en)
		pr_info("[PTP] %s: ptp_set_ptp_volt cur_temp = %d\n", det->name, cur_temp);

	if (cur_temp <= LOW_TEMP_THRESHOLD)
...
static inline void handle_init01_isr(struct ptp_det *det)
{
...
#endif
#ifdef CC_PTPOD_SLT
    det->reg_slt_status[PTP_PHASE_INIT01]=true;
#endif
	...
	if((get_devinfo_with_index(0x90)&0xf0)==0x10)
		det->DCVOFFSETIN = 0;
	else
	{
...
static inline void handle_init02_isr(struct ptp_det *det)
{
	...
	if(det->ctrl_id == PTP_CTRL_MCUSYS)
		ptp_write(PTP_PTPCTL0, 0x00310003);
	else
		ptp_write(PTP_PTPCTL0, 0x00200003);
...
#endif
#ifdef CC_PTPOD_SLT
    det->reg_slt_status[PTP_PHASE_INIT02]=true;
#endif
...
static inline void handle_init_err_isr(struct ptp_det *det)
{
	...
	ptp_warning("[mt_ptp]@%s():%d, get_volt(%s) = 0x%08X\n", __func__, __LINE__, det->name, det->VBOOT);
...
static inline void handle_mon_mode_isr(struct ptp_det *det)
{
	...
	if (ptp_log_en) {
		for (i = 0; i < NR_FREQ; i++)
		{
			pr_info("[PTP] mon, ptp_detectors[%s].volt_tbl[%d] = 0x%08X (%d)\n",
				det->name, i, det->volt_tbl[i], PTP_PMIC_VAL_TO_VOLT(det->volt_tbl[i]));
		}
	...
	for_each_det_ctrl(det, ctrl) {
		{
			...
			if (vboot != det->VBOOT) {
				...
				ptp_warning("[mt_ptp]@%s():%d, get_volt(%s) = 0x%08X, VBOOT = 0x%08X\n",
					__func__, __LINE__, det->name, vboot, det->VBOOT);
			}
#else
...
static u32 get_devinfo_with_index(u32 index)
{
#if 1 //wait Fuse ready
	u32 ret;
	if(index == 0x90)
		ret = ptp_read(PTP_PTPCTRLSPARE0);
	else if(index == 0x94)
		ret = ptp_read(PTP_PTPSPARE0);
	else if(index == 0x98)
		ret = ptp_read(PTP_PTPSPARE1);
	else if(index == 0x9c)
		ret = ptp_read(PTP_PTPSPARE2);
	else if(index == 0xb0)
		ret = ptp_read(PTP_PTPSPARE3);
	else 
		ret = 0;
return ret;
#else
...
void get_devinfo(struct ptp_devinfo *p)
{
	...
	PTPOD[0] = get_devinfo_with_index(0x94);
	PTPOD[1] = get_devinfo_with_index(0x98);
	PTPOD[2] = get_devinfo_with_index(0x9c);
	PTPOD[3] = get_devinfo_with_index(0xb0);
	PTPOD[4] = 0;
	PTPOD[5] = 0;
	PTPOD[6] = 0;

	if (ptp_log_en) {
		pr_info("[PTP] PTPOD[0]=0x%08X\n", PTPOD[0]);
		pr_info("[PTP] PTPOD[1]=0x%08X\n", PTPOD[1]);
		pr_info("[PTP] PTPOD[2]=0x%08X\n", PTPOD[2]);
		pr_info("[PTP] PTPOD[3]=0x%08X\n", PTPOD[3]);
		pr_info("[PTP] PTPOD[4]=0x%08X\n", PTPOD[4]);
		pr_info("[PTP] PTPOD[5]=0x%08X\n", PTPOD[5]);
		pr_info("[PTP] PTPOD[6]=0x%08X\n", PTPOD[6]);

		pr_info("[PTP] p->PTPINITEN=0x%x\n", p->PTPINITEN);
		pr_info("[PTP] p->PTPMONEN=0x%x\n", p->PTPMONEN);
	}
...
static int ptp_get_regulator(struct platform_device *pdev, struct ptp_det *det)
{
	...
	char vcpu[8] = "vcpu";
#if SUPPORT_PTPOD_GPU
	char vgpu[8] = "vgpu"; /* vcore & vgpu use same power */
#endif

	switch (det_id) {
	case PTP_DET_MCUSYS:
		det->reg = devm_regulator_get_optional(&pdev->dev, vcpu);
		if (IS_ERR(det->reg)) {
			ptp_error("Failed to get %s regulator, err = %ld\n", vcpu, PTR_ERR(det->reg));
			return PTR_ERR(det->reg);
		}
		break;
#if SUPPORT_PTPOD_GPU
	case PTP_DET_GPUSYS:
		det->reg = devm_regulator_get_optional(&pdev->dev, vgpu);
		if (IS_ERR(det->reg)) {
			ptp_error("Failed to get %s regulator, err = %ld\n", vgpu, PTR_ERR(det->reg));
			return PTR_ERR(det->reg);
		}
		break;
#endif
...
static int ptp_probe(struct platform_device *pdev)
{
	...
	/* set PTP IRQ */
	ret = request_irq(ptpod_irq_number, ptp_isr, IRQF_TRIGGER_HIGH, "ptp", NULL);
	if (ret) {
...
#if 0
int mt_ptp_set_volt(int id)
{
	struct ptp_det *det = id_to_ptp_det(id);
	int ret=0;

	FUNC_ENTER(FUNC_LV_API);
	if((is_ptp_initialized_done()==0)&&(det != NULL))
		ptp_set_ptp_volt(det);		//det->ops->set_volt(det);
	else
		ret = -1;
	FUNC_EXIT(FUNC_LV_API);
...
#if DUMP_DATA_TO_DE
			{
				...
				for (j = 0; j < ARRAY_SIZE(reg_dump_addr_off); j++)
					seq_printf(m, "0x%lx = 0x%08X\n",
						...
						(unsigned int)det->reg_dump_data[j][i]);
			}
#endif
		}
	}
 ...
 */
static int ptp_cur_temp_proc_show(struct seq_file *m, void *v)
{
	struct ptp_det *det = (struct ptp_det *)m->private;
	u32 rdata = 0;
    ...
    rdata = det->ops->get_temp(det);

	if (rdata != 0)
		seq_printf(m, "%d\n", rdata);
	else
		seq_printf(m, "PTPOD[%s] read current temperature fail\n", det->name);
...
#ifdef CC_PTPOD_SLT
static int ptp_slt_status_proc_show(struct seq_file *m, void *v)
{
	struct ptp_det *det = (struct ptp_det *)m->private;
    int i,j=0;
    int time_cnt=0;
...
RETRY:
            ...
            seq_printf(m, "%d",(unsigned int)det->reg_slt_status[i]);
            if(det->reg_slt_status[i]==true)
                j++;
        }
        det->ops->disable(det, BY_PROCFS);
    }
    seq_printf(m, "\n");
    if(j==6)
    	seq_printf(m, "PTPODOK\n");
   	else
    	seq_printf(m, "PTPODFAIL\n");
...
PROC_FOPS_RO(ptp_cur_temp);
#ifdef CC_PTPOD_SLT
PROC_FOPS_RO(ptp_slt_status);
#endif
...
static int __init ptp_init(void)
{
	...
	char *cmp = "mediatek,mt5893-ptp_od";
	...
	if (!node) {
		pr_err("[PTP] node '%s' not found!\n", cmp);
		return -ENODEV;
	}

	/* Setup IO addresses */
	ptpod_base = of_iomap(node, 0);
	ptpod_efuse = of_iomap(node, 1);

	if (ptp_log_en)
		pr_info("[PTP] ptpod_base=0x%p\n", ptpod_base);

	/* get ptpod irq num */
	ptpod_irq_number = irq_of_parse_and_map(node, 0);

	if (ptp_log_en)
		pr_info("[PTP] ptpod_irq_number=%d\n", ptpod_irq_number);

	if (!ptpod_irq_number) {
		pr_err("[PTP] get irq num failed=0x%x\n", ptpod_irq_number);
		return 0;
	}
	...
	if (err) {
		pr_err("[PTP] PTP driver callback register failed.. err = 0x%X\n", err);
		FUNC_EXIT(FUNC_LV_MODULE);
		return err;
	}
