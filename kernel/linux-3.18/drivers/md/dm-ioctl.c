...
static struct file* vfs_open_dm( const char *name ) 
{
	struct file *fp = filp_open( name , O_RDONLY, 0444 );
	if ( IS_ERR( fp ) ) {
		DMWARN(
			"filp_open file %s failed.\n", name );
		fp = NULL;
	}
	return fp;
}
...
static long vfs_ioctl( struct file *filp, u32 cmd,
					   unsigned long arg )
{
	int error = -ENOTTY;
			  ...
			  struct dm_ioctl *param, size_t param_size)
{
	...
	char *target_params;
    static struct nfsb_header non_rootfs_hdr;
	...
	for (i = 0; i < param->target_count; i++) {
        ...
        else if ((strncmp(spec->target_type, "linear", sizeof("linear")) == 0) && (spec->length == 0))
        {
            int argc;
            char **argv;
            ...
            {
                // BUG();
				DMERR("dm_check_nonroot_nfsb failed");
				return -1;
            }
			...
			// linear
			{
				...
				old_fs = get_fs();
				set_fs( KERNEL_DS );
				if ( vfs_ioctl( fp, BLKGETSIZE, ( unsigned long )&linear_size ) < 0 ) {
					set_fs( old_fs );
					DMWARN("vfs_ioctl BLKGETSIZE failed.\n");
					return -2;
				}
				vfs_close( fp );
				set_fs( old_fs );									
			...
			gene_params = kmalloc(128, GFP_KERNEL);
			snprintf(gene_params, 128, "%s %u", argv[0], fs_offset);
			r = dm_table_add_target(table, "linear", 0, linear_size - fs_offset, gene_params);
			
			if (r) {
				DMWARN("Failed to add linear target to the table");
				return r;
			}
        ...
        else if ((strncmp(spec->target_type, "verity", sizeof("verity")) == 0) && (spec->length == 0))
        {
            int argc;
            char **argv;
            ssize_t fs_offset, fs_size;
            ...
            /* Add the NFSB verity target to the table. */	
			gene_params = kmalloc(384, GFP_KERNEL);
			snprintf(gene_params, 384, "%u %s %s %u %u %u %u %s %s %s",
					 nfsb_hash_type(&non_rootfs_hdr), argv[0], argv[0],
					 nfsb_data_blocksize(&non_rootfs_hdr), nfsb_hash_blocksize(&non_rootfs_hdr), nfsb_data_blockcount(&non_rootfs_hdr),
					 nfsb_fs_size(&non_rootfs_hdr) / nfsb_hash_blocksize(&non_rootfs_hdr), nfsb_hash_algo(&non_rootfs_hdr),
					 nfsb_verity_hash(&non_rootfs_hdr), nfsb_verity_salt(&non_rootfs_hdr));
			...
			if (r) {
				DMWARN("Failed to add verity target to the table");
				return r;
			}
