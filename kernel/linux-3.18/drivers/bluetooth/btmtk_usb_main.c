					...
					size_t count, loff_t *f_pos)
{
	...
	if (count > HCI_MAX_COMMAND_BUF_SIZE) {
		BTUSB_ERR("%s: your command is larger than maximum length, count = %zd\n", __func__, count);
		goto exit;
	}

	for (i = 0; i < count; i++) {
		if (buf[i] != '=') {
			g_data->i_fwlog_buf[i] = buf[i];
			BTUSB_DBG("%s: tag_param %02x\n", __func__, g_data->i_fwlog_buf[i]);
		} else {
			/* for input log_lvl, ex echo log_lvl=4 > /dev/stpbtfwlog */
			val_param = &buf[i+1];
			if (strcmp(g_data->i_fwlog_buf, "log_lvl") == 0) {
				BTUSB_INFO("%s: btmtk_log_lvl = %d\n", __func__, val_param[0] - 48);
	...
	if (val_param != NULL)
		...
		BTUSB_ERR("%s: ERROR, val_param is NULL\n", __func__);
		return -1;
	}
	...
	BTUSB_DBG("%s: hci buff is %02x%02x%02x%02x%02x ", __func__, g_data->o_fwlog_buf[0], g_data->o_fwlog_buf[1],
		 g_data->o_fwlog_buf[2], g_data->o_fwlog_buf[3], g_data->o_fwlog_buf[4]);
	/* check HCI command length */
	if (length > HCI_MAX_COMMAND_SIZE) {
		BTUSB_ERR("%s: your command is larger than maximum length, length = %d\n", __func__, length);
		goto exit;
	}
	...
	ret = usb_control_msg(g_data->udev, usb_sndctrlpipe(g_data->udev, 0),
				0, DEVICE_CLASS_REQUEST_OUT, 0, 0,
				(void *)g_data->o_fwlog_buf, length, USB_CTRL_IO_TIMO);
	if (ret < 0) {
		BTUSB_ERR("%s: command send failed(%d)", __func__, ret);
		goto exit;
	}
	BTUSB_INFO("%s: write end\n", __func__);
exit:
	BTUSB_INFO("%s: exit write end\n", __func__);
	BTUSB_INFO("%s: total length is %d\n", __func__, length);
	return count;
}
