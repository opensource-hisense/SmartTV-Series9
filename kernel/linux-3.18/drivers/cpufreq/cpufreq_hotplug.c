...
static struct dbs_tuners {
    ...
    unsigned int hotplug_is_enabled;
} dbs_tuners_ins = {
	...
	.ignore_nice = 0,
	.io_is_busy = 0,
	.cpu_num_limit = NR_CPUS,
    .hotplug_is_enabled = 0,
};

void hp_limited_cpu_num(unsigned int num)
{
...
show_one(hotplug_is_enabled, hotplug_is_enabled);
				   ...
				   const char *buf, size_t count)
{
	...
	hp_limited_cpu_num(input);

	return count;
}

static ssize_t store_hotplug_is_enabled(struct kobject *a, struct attribute *b,
                const char *buf, size_t count)
{
    unsigned int input;
    int ret;
    ...
    mutex_lock(&dbs_mutex);
    dbs_tuners_ins.hotplug_is_enabled = ! !input;
    mutex_unlock(&dbs_mutex);
...
define_one_global_rw(hotplug_is_enabled);
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	// check if online cpus is over limit cpu
...
#ifdef DEBUG_CPUFREQ
	//printk("avg: %d hotplug_in:%d, hotplug_out:%d\n", avg_load, hotplug_in_avg_load, hotplug_out_avg_load);
#endif
	...
	if (avg_load < dbs_tuners_ins.down_threshold) {
		...
		if (policy->cur == policy->min) {
			...
			if (num_online_cpus() > 1 && hotplug_out_avg_load <
				...
				//mutex_unlock(&this_dbs_info->timer_mutex);
				g_next_hp_action = num_online_cpus() - 1;
		...
		/* bump down to the next lower frequency in the table */
		...
		if (policy->cur != this_dbs_info->freq_table[index].frequency) {
#ifdef DEBUG_CPUFREQ
			printk("policy->current: %d, next.frequency:%d\n",
			       policy->cur,
				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			dbs_tuners_ins.hotplug_load_history = kmalloc((sizeof
								       (unsigned
									int) *
								       max_periods),
								      GFP_KERNEL);
...
MODULE_DESCRIPTION
    ("'cpufreq_hotplug' - cpufreq governor for dynamic frequency scaling and CPU hotplugging");
MODULE_LICENSE("GPL");
