...
#endif //#if defined(CONFIG_ARCH_MT5863)

#if defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5891)
static int i4MacPhyZCALInit(StarDev *dev)
{
	int i4Ret = 0;
...
static int i4MacPhyZCALRemapTable(StarDev *dev, u8 value)
{
	int i4Ret = 0;
	u16 temp;
	StarPrivate *starPrv = dev->starPrv;
...
void mdcMdio_write(struct net_device *dev, int phy_id, int location, int val)
{
...
void vStar100mReturnLoss(StarDev *dev)
{
	...
	STAR_MSG(STAR_WARN, "100M Return Loss[updated]\n");
...
/*leilk add for phoniex verify */
int phy_initial_setting(StarDev *dev)
{
	int data;
    StarPrivate *starPrv = dev->starPrv;

    STAR_MSG(STAR_ERR, "enter phy_initial_setting\n");

	StarMdcMdioWrite(dev, starPrv->phy_addr, 0x1f, 0x2a30);

	/* set  MDI_CAL_FCE to 1 */
	data = StarMdcMdioRead(dev, starPrv->phy_addr, 0x14);
	data |= (1<<12);
	StarMdcMdioWrite(dev, starPrv->phy_addr, 0x14, data);

	/* set  MDI_CAL_VAL to 1000 */
	data = StarMdcMdioRead(dev, starPrv->phy_addr, 0x14);
    data &= ~(0xf<<8);
	data |= (0x8<<8);
	StarMdcMdioWrite(dev, starPrv->phy_addr, 0x14, data);
	...
	/* set  TST_RESERVE_INT[5:0] to 010000 */
...
#if defined(CONFIG_ARCH_MT5863)
static void Star_Phy_Init(StarDev *dev)
{
	u16 data;
	//StarPrivate *starPrv = dev->starPrv;

	StarClearBit(STAR_ETH_PDREG_SOFT_RST(dev->base), ETH_PHY_RST);
	star_mb();
	msleep(1);
	StarSetBit(STAR_ETH_PDREG_SOFT_RST(dev->base), ETH_PHY_RST);
	star_mb();
	msleep(4);

	/* DC calibration default value */
	/* R50OHM_RSEL_TX_A[6:0] = 7'b1000000 , R50OHM_RSEL_TX_B[6:0] = 7'b1000000 */
	StarMdcMdioWrite45(dev, 0, Dev1E_R50OHM_RSEL_CTRL_0.regAddr, Dev1E_R50OHM_RSEL_CTRL_0.devAddr, 0xC0C0);

	/* RG_REXT_TRIM[5:0] = 6'b100000 */
	data = StarMdcMdioRead45(dev, 0, Dev1E_ANACAL_REG_5.regAddr, Dev1E_ANACAL_REG_5.devAddr);
	data &= ~(RG_REXT_CTRL_MASK);
	data |= (0x2000);
	StarMdcMdioWrite45(dev, 0, Dev1E_ANACAL_REG_5.regAddr, Dev1E_ANACAL_REG_5.devAddr, data);

	/* RG_BG_RASEL[2:0] = 3'b100 */
	StarMdcMdioWrite45(dev, 0, Dev1F_BG_RASEL.regAddr, Dev1F_BG_RASEL.devAddr, 0x4);

	printk("PHY Init done.\n");
}

void star_power_down(StarDev *dev)
{
	STAR_MSG(STAR_ERR, "star_power_down flow\n");

	StarNICPdSet(dev, 0);
	star_mb();
	StarMdcMdioWrite45(dev, 0, 0x3e, 0x1e, 0xffff);
	STAR_MSG(STAR_ERR, "0x1e_3e %x\n", StarMdcMdioRead45(dev, 0, 0x3e, 0x1e));
	StarMdcMdioWrite45(dev, 0, 0x426, 0x1f, 0x0000);
	StarMdcMdioWrite45(dev, 0, 0x3d, 0x1e, 0xf8f8);
	STAR_MSG(STAR_ERR, "0x1e_3d %x\n", StarMdcMdioRead45(dev, 0, 0x3d, 0x1e));
	StarMdcMdioWrite45(dev, 0, 0x267, 0x1f, 0x0000);
	StarMdcMdioWrite45(dev, 0, 0x151, 0x1e, 0x1300);
	StarMdcMdioWrite45(dev, 0, 0x3c, 0x1e, 0xffff);
	StarMdcMdioWrite45(dev, 0, 0xe1, 0x1e, 0x0001);
	//StarSetReg(STAR_ETH_UP_MAC_CK((starDev)->base), 0x1);
	//STAR_MSG(STAR_ERR, "0x334 %x\n", StarGetReg(STAR_ETH_UP_MAC_CK((starDev)->base)));
	STAR_MSG(STAR_ERR, "mac 0x14 %x\n", StarGetReg((dev)->base + 0x14));
	STAR_MSG(STAR_ERR, "mac 0x18 %x\n", StarGetReg((dev)->base + 0x18));
	STAR_MSG(STAR_ERR, "mac 0x08 %x\n", StarGetReg((dev)->base + 0x08));
}
...
#if defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
...
#if defined (CONFIG_ARCH_MT5396) || defined(CONFIG_ARCH_MT5368) || defined(CONFIG_ARCH_MT5389) || defined(CONFIG_ARCH_MT5398) || defined(CONFIG_ARCH_MT5880)||defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5865)|| defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
        // configure LED 
...
//#elif defined (CONFIG_ARCH_MT5389)
		...
		StarMdcMdioWrite(dev, starPrv->phy_addr, 0x1F, 0x0000);  // main page
#elif defined (CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5865)|| defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886) 
        StarMdcMdioWrite(dev, starPrv->phy_addr, 0x1F, 0x2a30);  // test page
        //msleep(50);
...
#if defined(CONFIG_ARCH_MT5880) || defined(CONFIG_ARCH_MT5865)
        vSet100Amp(dev, 4);  
#endif
	...
	/* write PMA Dbg09 register to 0x18689 for 200m cable packet loss */
	StarMdcMdioWrite(dev, starPrv->phy_addr, 0x1F, 0x52b5);
	StarMdcMdioWrite(dev, starPrv->phy_addr, 0x11, 0x8689);
...
{    
...
#elif defined (CONFIG_ARCH_MT5398) || defined (CONFIG_ARCH_MT5880)||defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5865)|| defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863)
    ...
    */
    ...
    msleep(50);
#endif    
#if defined(CONFIG_ARCH_MT5880)||defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5865)|| defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863)
...
#if 0// defined(CONFIG_ARCH_MT5882)
if (IS_IC_MT5885())
{
    ...
    {
        #if defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863)
