										...
										size_t count, loff_t *ppos)
{
	...
	STAR_MSG(STAR_ERR, "read cl45 phy register useage:\n");
	STAR_MSG(STAR_ERR, "\t echo rcl45 devName phyReg > phyreg\n");

	STAR_MSG(STAR_ERR, "write cl45 phy register useage:\n");
	STAR_MSG(STAR_ERR, "\t echo wcl45 devName phyReg value > phyreg\n");
										...
										size_t count, loff_t *pos)
{
	...
	} else if (str_cmp_seq(&buf, "rcl45")) {
		if (2 == sscanf(buf, "%x %x", &phy_devname, &phy_reg)) {
			STAR_MSG(STAR_ERR, "devname phyreg(0x%x 0x%x):0x%x\n", phy_devname, phy_reg,
					StarMdcMdioRead45(starDev,starPrv->phy_addr,phy_reg,phy_devname));
		} else {
			STAR_MSG(STAR_ERR,"sscanf rcl45(%s) error\n", buf);
		}
	} else if (str_cmp_seq(&buf, "wcl45")){
		if (3 == sscanf(buf, "%x %x %x", &phy_devname, &phy_reg, &value)) {
			phy_val = StarMdcMdioRead45(starDev,starPrv->phy_addr,phy_reg,phy_devname);
			StarMdcMdioWrite45(starDev,starPrv->phy_addr,phy_reg,phy_devname,(u16)value);
			STAR_MSG(STAR_ERR, "devname phyreg(0x%x 0x%x): 0x%x --> 0x%x!\n",
					phy_devname, phy_reg, phy_val,
					StarMdcMdioRead45(starDev,starPrv->phy_addr,phy_reg,phy_devname));
		} else {
			STAR_MSG(STAR_ERR,"sscanf wcl45(%s) error\n", buf);
		}
					...
					char __user *buf, size_t count, loff_t *ppos)
{
	...
	wol = star_get_wol_flag(starPrv);
	if (wol < 0)
		return -1;

	STAR_MSG(STAR_ERR, "%ssupport WOL\n", (wol ? "" : "NOT "));
	STAR_MSG(STAR_ERR, "Use 'echo 1 > /proc/driver/star/wol' to enable wol\n");
	STAR_MSG(STAR_ERR, "Use 'echo 0 > /proc/driver/star/wol' to disable wol\n");

	return 0;
}

static ssize_t proc_write_wol_eth(struct file *file,
					const char __user *buffer, size_t count, loff_t *pos)
{
	...
	buf[count] = '\0';
	star_set_wol_flag(starPrv, (buf[0] - '0'));
