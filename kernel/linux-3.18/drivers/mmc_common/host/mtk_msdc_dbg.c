...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "-> 0)PIO 1)DMA \n");
	seq_printf(m, "-> echo 1 23 1 >msdc_bebug -> host[1] drv mode DMA. (dma when >= 512)\n");
...
static int msdc_debug_proc_write(struct file *file, const char *buf, unsigned long count, void *data)
{
	...
	if (cmd == RW_BIT_BY_BIT_COMPARE) {
		...
		while (count < p4) {
			...
			pr_err("\n call rwaccess_write \n");
			pr_err("partno = %x, offset = %x, size = %x\n", p1, p2, p3);
			msdc_partwrite(p1, p2, p3, temp_buf);
			count ++;
		}
	}
...
int msdc_debug_proc_init(void)
{
	...
	if (prEntry) {
	   ...
	   pr_info("[%s]: successfully create /proc/msdc_debug\n", __func__);
	} else {
	...
	if (prEntry) {
	   ...
	   pr_info("[%s]: successfully create /proc/msdc_help\n", __func__);
	} else {
	...
	if (prEntry) {
	   ...
	   pr_info("[%s]: successfully create /proc/msdc_tune\n", __func__);
	} else {
	...
	if (prEntry) {
	   ...
	   pr_info("[%s]: successfully create /proc/msdc_tune_flag\n", __func__);
	} else {
