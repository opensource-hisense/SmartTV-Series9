...
typedef enum {
	...
	SD_TOOL_SET_TUNE = 18,
	PRINT_CUR_STATUS = 0x20,
	MMC_ACCESS_READ = 0x21,
	MMC_ACCESS_WRITE = 0x22,
	SD_TOOL_DRV_MODE = 0x23
} msdc_dbg;
...
extern int msdc_partread(int partno, uint64_t offset, uint64_t size, void *pvmem);
extern int msdc_partwrite(int partno, uint64_t offset, uint64_t size, void *pvmem);
...
#define INIT_MSG ERR_MSG
#define IRQ_MSG ERR_MSG
