...
#if !defined(CC_TURSTY_WITHOUT_DTS)
...
#if defined(CC_TRUSTY_WITHOUT_DTS)
static struct platform_device trusty_dev= {
    .name="trusty",
};

static struct platform_device trusty_irq_dev= {
    .name="trusty-irq",
};

static struct platform_device trusty_virtio_dev= {
    .name="trusty-virtio",
};

static int __init trusty_driver_init(void)
{
	platform_driver_register(&trusty_driver);

    platform_device_register(&trusty_dev);

    platform_device_register(&trusty_irq_dev);
    trusty_irq_dev.dev.parent=&trusty_dev.dev;

    platform_device_register(&trusty_virtio_dev);
    trusty_virtio_dev.dev.parent=&trusty_dev.dev;

	dev_info(NULL, "trusty-virtio device register done \n");
    return 0;
}
#else
static int __init trusty_driver_init(void)
