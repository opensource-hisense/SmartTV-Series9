...
static char *newpart(char *s)
{
	...
	/*be sure the part number not overflow, the current maximum is 64 [1-64]*/
	BUG_ON(mtk_msdc_partition.nparts  > (1 << MMC_SHIFT));
...
int mtkmsdc_partition(struct parsed_partitions *state)
{
    ...
    for(partno = 1; partno <= mtk_msdc_partition.nparts; partno++)
    {
        ...
        info = &state->parts[partno].info;
        label_min = min_t(int, sizeof(info->volname) - 1,
                    sizeof(part->name));
        strncpy(info->volname, part->name, label_min);
