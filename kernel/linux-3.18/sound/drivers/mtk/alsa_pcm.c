...
#ifdef CC_TUNNELING_SUPPORT
extern INT32 AUD_TunnelingStart(UINT32 u4TunnelingId, USIZE usRingFifoSize);
extern INT32 AUD_TunnelingInit(UINT32 u4TunnelingId);
extern INT32 AUD_TunnelingDeinit(UINT32 u4TunnelingId);
extern INT32 AUD_TunnelingPcmSetParameters(UINT32 u4TunnelingId, UINT32 u4SampleRate, UINT32 u4Channels);
extern void AUD_TunnelingAdvanceRingfifoWritePtr(UINT32 u4TunnelingId, USIZE usSize);
extern USIZE AUD_TunnelingRingfifoReadPtrElapsed(UINT32 u4TunnelingId);
#endif
...
struct timer_list mt85xx_pcm_cap_timer;
static int snd_card_mt85xx_pcm_playback_prepare(struct snd_pcm_substream *substream)
{
    ...
    else
    {
        if (substream->pcm->device == ALSA_PCM_DECID)
        {
            AUD_TunnelingPcmSetParameters(substream->pcm->device, runtime->rate, runtime->channels);
        }
        AUD_TunnelingStart(substream->pcm->device, pcm->pcm_buffer_size);
    }
#endif
    return 0;
}
...
static void snd_card_mt85xx_pcm_playback_update_write_pointer(struct snd_mt85xx_pcm *pcm)
{
...
#ifdef CC_TUNNELING_SUPPORT
    unsigned int wptr_advanced_frames = 0;
#endif

    if (pcm->substream->pcm->device < ALSA_DSPDEC_ID)
    {
    ...
    {
        if (runtime->control->appl_ptr < pcm->pcm_prev_wptr)
        {
            wptr_advanced_frames = runtime->boundary + runtime->control->appl_ptr
                    - pcm->pcm_prev_wptr;
        }
        else
        {
            wptr_advanced_frames = runtime->control->appl_ptr - pcm->pcm_prev_wptr;
        }
        AUD_TunnelingAdvanceRingfifoWritePtr(pcm->substream->pcm->device,
                frames_to_bytes(runtime, wptr_advanced_frames));
        pcm->pcm_prev_wptr = runtime->control->appl_ptr;
    }
#endif
}
...
static void snd_card_mt85xx_pcm_playback_timer_function(unsigned long data)
{
    ...
    {
        bytes_elapsed = (unsigned int)AUD_TunnelingRingfifoReadPtrElapsed(pcm->substream->pcm->device);
        pcm->pcm_rptr += bytes_elapsed;
    }
#endif
...
static snd_pcm_uframes_t snd_card_mt85xx_pcm_playback_pointer(struct snd_pcm_substream *substream)
{
    ...
    if (substream->pcm->device < ALSA_DSPDEC_ID)
    {
        return bytes_to_frames(runtime, pcm->pcm_rptr);
    }
    else
    {
        return bytes_to_frames(runtime, pcm->pcm_rptr % pcm->pcm_buffer_size);
    }
}
...
static int snd_card_mt85xx_pcm_playback_open(struct snd_pcm_substream *substream)
{
   ...
   if ((substream->pcm->device == ALSA_TUNAL_AC3_DSPDEC_ID) | (substream->pcm->device == ALSA_NON_TUNAL_AC3_DSPDEC_ID))
    ...
    {
        AUD_TunnelingInit(substream->pcm->device);
    }
#endif
    return 0;
}
...
static int snd_card_mt85xx_pcm_playback_close(struct snd_pcm_substream *substream)
{
    ...
    {
        AUD_TunnelingDeinit(substream->pcm->device);
    }
#endif
    return 0;
}
...
extern UINT32 AUD_GetUploadWritePtr(void);
extern void AUD_InitALSARecordSpeaker(void);
extern void AUD_DeInitALSARecordSpeaker(void);
extern INT32 vAprocRecordPCM(UINT8 uploadId, UPTR u4BufAddr,
                              UINT32 u4BufSize, UINT32* u4ReadSize,
                               BOOL fgBlockRead, UINT8 UploadDataType);
extern UINT32 AUD_GetUploadWp(void);
extern UINT32 AUD_GetUploadRp(void);
...
static struct snd_pcm_hardware mtk_mt85xx_pcm_capture_hardware = {
    ...
    .rate_min =     USE_RATE_MIN,
    .rate_max =     USE_RATE_MAX,
    .channels_min =     USE_CHANNELS_MAX, // only support two channel here
    .channels_max =     USE_CHANNELS_MAX,
    .buffer_bytes_max = MAX_BUFFER_SIZE,
    .period_bytes_min = MIN_PERIOD_SIZE,
    .period_bytes_max = MAX_PERIOD_SIZE,
...
static void snd_card_mt85xx_pcm_capture_timer_function(unsigned long arg)
{
    struct snd_pcm_substream *substream = (void*)arg;
    struct snd_pcm_runtime *runtime = substream->runtime;
    UINT32 u4ABufPtr;
    snd_pcm_sframes_t u4_frames;
    mt85xx_pcm_cap_timer.expires = 2 + jiffies; // 10ms later

    add_timer(&mt85xx_pcm_cap_timer);

    u4ABufPtr = AUD_GetUploadWritePtr();
    u4_frames = bytes_to_frames(runtime,u4ABufPtr*3072);

#if ALSA_DBG_MSG
    printk("[ALSA]capture_timer_update frames %d!\n",(int)u4_frames);
#endif
...
static void snd_card_mt85xx_pcm_capture_timer_start(struct snd_pcm_substream *substream)
{
    mt85xx_pcm_cap_timer.expires = 2 + jiffies;
    mt85xx_pcm_cap_timer.function = (void*)&snd_card_mt85xx_pcm_capture_timer_function;
    mt85xx_pcm_cap_timer.data = (unsigned long)substream;

    init_timer(&mt85xx_pcm_cap_timer);
    add_timer(&mt85xx_pcm_cap_timer);

#if ALSA_DBG_MSG
    printk("[ALSA]snd_card_mt85xx_pcm_capture_timer_start!\n");
#endif

}

static void snd_card_mt85xx_pcm_capture_timer_stop(struct snd_pcm_substream *substream)
{
    del_timer(&mt85xx_pcm_cap_timer);

#if ALSA_DBG_MSG
    printk("[ALSA]snd_card_mt85xx_pcm_capture_timer_stop!\n");
#endif
}
...
{
    struct snd_pcm_runtime *runtime = substream->runtime;
    runtime->hw = mtk_mt85xx_pcm_capture_hardware;

    AUD_InitALSARecordSpeaker();

    snd_card_mt85xx_pcm_capture_timer_start(substream);

#if ALSA_DBG_MSG
    printk("[ALSA]snd_card_mt85xx_pcm_capture_open - start\n");
#endif
...
{
#if ALSA_DBG_MSG
    printk("[ALSA]snd_card_mt85xx_pcm_capture_hw_params - start\n");
#endif
...
{
#if ALSA_DBG_MSG
    printk("[ALSA]snd_card_mt85xx_pcm_capture_hw_free - start\n");
#endif
...
{
#if ALSA_DBG_MSG
    printk("[ALSA]snd_card_mt85xx_pcm_capture_prepare - start\n");
#endif
...
static int snd_card_mt85xx_pcm_capture_trigger(struct snd_pcm_substream *substream, int cmd)
{
    ...
    case SNDRV_PCM_TRIGGER_START:
        #if ALSA_DBG_MSG
        printk("[ALSA]snd_card_mt85xx_pcm_capture_trigger - start\n");
        #endif
        return 0;
    case SNDRV_PCM_TRIGGER_RESUME:
        #if ALSA_DBG_MSG
        printk("[ALSA]snd_card_mt85xx_pcm_capture_trigger - resume\n");
        #endif
        return 0;
    case SNDRV_PCM_TRIGGER_STOP:
        #if ALSA_DBG_MSG
        printk("[ALSA]snd_card_mt85xx_pcm_capture_trigger - stop\n");
        #endif
        return 0;
    case SNDRV_PCM_TRIGGER_SUSPEND:
        #if ALSA_DBG_MSG
        printk("[ALSA]snd_card_mt85xx_pcm_capture_trigger - suspend\n");
        #endif
        return 0;
    }
    return 0;
...
{
    snd_pcm_uframes_t u4Wp_frames = 0;
    UINT32 u4Wp;
    struct snd_pcm_runtime *runtime = substream->runtime;

    u4Wp = AUD_GetUploadWp();
    u4Wp_frames = bytes_to_frames(runtime,u4Wp*3072);

#if ALSA_DBG_MSG
    printk("[ALSA]pcm_capture_pointer %d\n",(int)u4Wp_frames);
#endif
...
#if ALSA_DBG_MSG
    printk("[ALSA]snd_card_mt85xx_pcm_capture_close - suspend\n");
#endif

    snd_card_mt85xx_pcm_capture_timer_stop(substream);
    return 0;
}

#define ALSA_UPLOAD_ID_2 2
#define UPLOAD_FRAME_SIZE 768
static int snd_card_mt85xx_pcm_capture_copy(struct snd_pcm_substream *substream,
            int channel,
            ...
            void __user *buf, snd_pcm_uframes_t count)
{
    ...
    vAprocRecordPCM(ALSA_UPLOAD_ID_2, (unsigned long)buf, (UINT32)frames_to_bytes(runtime, count),
                             &u4ReadSize, TRUE, 2);
...
static struct snd_pcm_ops snd_card_mt85xx_capture_ops = {
    ...
    .copy      = snd_card_mt85xx_pcm_capture_copy
};
