...
unsigned int (*mt53xx_mid_protect_all_dram)(unsigned int) = NULL;
void mt53xx_register_mid_protect_all_dram(void (*func)(unsigned int))
{
    mt53xx_mid_protect_all_dram = func;
}
EXPORT_SYMBOL(mt53xx_register_mid_protect_all_dram);
...
static int psci_system_suspend_enter(suspend_state_t state)
{
	...
	if (mt53xx_mid_protect_all_dram)
		mt53xx_mid_protect_all_dram(1);
	udelay(1000); // delay 1ms
	if (mt53xx_mid_protect_all_dram)
		mt53xx_mid_protect_all_dram(0);
...
static int cpu_psci_cpu_kill(unsigned int cpu)
{
	...
	pr_err("CPU%d may not have shut down cleanly (AFFINITY_INFO reports %d)\n",
			cpu, err);
