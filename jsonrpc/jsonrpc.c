...
json_t *jsonrpc_error_object(int code, json_t *data)
{
	...
	switch (code) {
		...
		case JSONRPC_INTERNAL_ERROR:
			...
			break;
		case JSONRPC_MTK_CANT_OPERATE:
		    message = "Cannot operation now";
		    break;
		case JSONRPC_MTK_COUNT_OVER:
		    message = "Count out of range";
		    break;
		case JSONRPC_MTK_INDEX_OVER:
		    message = "Index out of range";
		    break;
		case JSONRPC_MTK_UNSUPPORT_FORMAT:
		    message = "Unsupported format";
		    break;
		case JSONRPC_MTK_NOT_FOUND:
		    message = "Not found";
		    break;
		case JSONRPC_MTK_REQ_DUP:
		    message = "Request duplicated";
		    break;
		case JSONRPC_MTK_WRONG_UUID:
		    message = "Wrong uuid";
		    break;
		case JSONRPC_MTK_WRONG_NICKNAME:
		    message = "Wrong nickname";
		    break;
		case JSONRPC_MTK_NOT_SELECTED:
		    message = "Not selected";
		    break;
		case JSONRPC_MTK_FILE_SIZE_ERR:
		    message = "File size error";
		    break;
		case JSONRPC_MTK_ILLEGAL_STATE:
		    message = "Illegal State";
		    break;
		case JSONRPC_MTK_ILLEGAL_ARG:
		    message = "Illegal Argument";
		    break;
		case JSONRPC_MTK_REQ_RETRY:
		    message = "Request Retry";
		    break;
		case JSONRPC_MTK_CLIENT_OVER:
		    message = "Client Over Maximum";
		    break;
	}
...
json_t *jsonrpc_result_response(json_t *json_id, json_t *json_result)
{
	...
	json_t *response = json_pack("{s:s,s:o,s:o}",
		...
		"result", json_result);
#endif		
    json_t *response = json_pack("{s:o,s:o}",
		"id", json_id,
		"result", json_result);
	return response;
}
...
#endif
    rc = json_unpack_ex(json_request, &error, flags, "{s:s,s?o,s?o}",
		"method", str_method,
		"params", json_params,
		"id", json_id
	);
...
json_t *jsonrpc_handle_request_single(json_t *json_request, struct jsonrpc_method_entry_t method_table[])
{
	...
	if (is_notification) {
		...
		if (rc==0) {
			...
			json_response = jsonrpc_error_response(json_id,
					jsonrpc_error_object(/*JSONRPC_INTERNAL_ERROR*/rc, json_result));
		}
	}
...
char *jsonrpc_handler(const char *input, size_t input_len, struct jsonrpc_method_entry_t method_table[])
{
	...
	if (json_response)
		output = json_dumps(json_response, JSON_INDENT(0));
	return output;
}
